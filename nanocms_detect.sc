...
for dir in nasl_make_list_unique( "/cms", "/nanocms", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	url = NASLString( dir, "/data/nanoadmin.php" );
	...
	if(egrep( pattern: "NanoCMS Admin Login", string: buf, icase: TRUE )){
		vers = "unknown";
		files = make_list( "/gnu-license-mini.txt",
			 "/README%20FIRST.txt" );
		for file in files {
			url = dir + file;
			req = http_get( item: url, port: port );
		...
		log_message( data: build_detection_report( app: "NanoCMS", version: vers, install: install, cpe: cpe, concluded: version[0], concludedUrl: concUrl ), port: port );
		exit( 0 );
	}
}
exit( 0 );
