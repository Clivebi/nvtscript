if(description){
	...
	script_tag( name: "summary", value: "This script finds the running PHP Quick Arcade version." );
...
for dir in nasl_make_list_unique( "/", "/phpqa", "/phpqa/phpqa", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( rcvRes, ">PHP-Quick-Arcade<" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "PHP Quick Arcade", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
