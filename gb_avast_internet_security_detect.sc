...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( avastName, "Avast Internet Security" )){
		...
		if(avastVer){
			...
			log_message( data: build_detection_report( app: "Avast Internet Security", version: avastVer, install: avastPath, cpe: cpe, concluded: avastVer ) );
			exit( 0 );
		}
	}
}
