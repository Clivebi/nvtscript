...
if( !ret || ret == "" ){
	...
	if( IsMatchRegexp( ret, "Error: Unrecognized command found" ) ){
		...
		for section in sections {
			if(!IsMatchRegexp( section, "multicast\\s+ipv6\\s+routing-enable" ) || IsMatchRegexp( section, "Info: It will take a long time" ) || ContainsString( section, "section include multicast" )){
				continue;
			}
			if( IsMatchRegexp( section, "^\\s*multicast\\s+ipv6\\s+routing-enable\\s*" ) ){
