...
for dir in nasl_make_list_unique( "/", "/asbru", "/wcm", http_cgi_dirs( port: asbPort ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(IsMatchRegexp( asbRes, ">Asbru Web Content Management.*<" ) && ContainsString( asbRes, "www.asbrusoft.com" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Asbru Web Content Management System", version: version, install: install, cpe: cpe, concluded: version ), port: asbPort );
	}
}
exit( 0 );
