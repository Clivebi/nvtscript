if(description){
	...
	script_dependencies( "realty_web_base_detect.sc" );
...
if(!version = get_kb_item( NASLString( "www/", port, "/RealtyWebBase" ) )){
	exit( 0 );
}
...
if( !isnull( vers ) && !ContainsString( "unknown", vers ) ){
	...
	variables = NASLString( "user=%27%20or%20%271=1&password=%27%20or%20%271=1" );
	filename = NASLString( dir, "/admin/admin.php" );
