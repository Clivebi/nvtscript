if(description){
	...
	script_dependencies( "gb_v-webmail_detect.sc" );
...
if(vwmailVer[2] != NULL && ( !safe_checks() )){
	req = http_get( item: vwmailVer[2] + "/includes/mailaccess/pop3.php" + "?CONFIG[pear_dir]=[SHELL]", port: vwmailPort );
	res = http_send_recv( data: req, port: vwmailPort );
	if(!ContainsString( res, "SHELL" )){
		req = http_get( item: vwmailVer[2] + "/includes/prepend.php?" + "CONFIG[includes]=[SHELL]", port: vwmailPort );
		res = http_send_recv( data: req, port: vwmailPort );
	}
	if(ContainsString( res, "SHELL" ) && egrep( pattern: "^HTTP/1\\.[01] 200", string: res )){
