...
if( !isnull( vers ) && !ContainsString( "unknown", vers ) ){
	...
	hostnames = make_list( "localhost",
		 get_host_name() );
	for hostname in hostnames {
		request = NASLString( "/", file, "?recipient=foobar@", hostname, "&subject=1&return_link_url=javascript:alert(0815)&return_link_title=VT-Test" );
		url = dir + request;
		...
		if(IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) && egrep( pattern: "<a href=.javascript:alert\\(0815\\).>VT-Test</a>", string: buf )){
