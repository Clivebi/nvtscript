...
if(description){
	...
	script_dependencies( "gb_sendmail_detect.sc", "global_settings.sc", "os_detection.sc" );
...
ip = forge_ip_packet( ip_v: 4, ip_hl: 5, ip_tos: 0, ip_off: 0, ip_len: 20, ip_p: IPPROTO_TCP, ip_id: cipid, ip_ttl: cttl, ip_src: get_host_ip() );
tcp = forge_tcp_packet( ip: ip, th_sport: sport, th_dport: port, th_flags: 0x02, th_seq: cth_seq, th_ack: cth_ack, th_x2: 0, th_off: 5, th_win: cth_win, th_urp: 0 );
for(j = 0;j < nrpackets;j++){
	reply = send_packet( packet: tcp, pcap_active: FALSE );
	sleep( 1 );
}
sleep( 3 );
soc = open_sock_tcp( port );
