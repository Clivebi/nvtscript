...
ppPath = registry_get_sz( key: "SOFTWARE\\ComputerAssociates\\eTrustPestPatrol", item: "InstallPath" );
if(ppPath){
	share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: ppPath );
	file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: ppPath + "\\ppctl.dll" );
