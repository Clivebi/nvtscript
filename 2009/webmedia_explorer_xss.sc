...
if(description){
	...
	script_dependencies( "webmedia_explorer_detect.sc" );
...
url = NASLString( dir, "/index.php?search=%22%20onmouseover=alert(document.cookie)%20---" );
req = http_get( item: url, port: port );
buf = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
if(!buf){
	exit( 0 );
}
if(IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) && egrep( pattern: "<a href=.*onmouseover=alert\\(document\\.cookie\\) ---", string: buf )){
