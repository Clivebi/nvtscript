if(description){
	...
	script_dependencies( "gb_aol_detect.sc" );
...
if(!version = get_kb_item( "AOL/Ver" )){
	exit( 0 );
}
if(!IsMatchRegexp( version, "^9\\..*" )){
	exit( 0 );
}
appPath = registry_get_sz( key: "SOFTWARE\\America Online\\AOL\\CurrentVersion", item: "AppPath" );
if(appPath != NULL){
	share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: appPath );
	file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: appPath + "\\sb.dll" );
