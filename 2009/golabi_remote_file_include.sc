...
for dir in nasl_make_list_unique( "/cms", "/golabi", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	for pattern in keys( files ) {
		file = files[pattern];
		url = NASLString( dir, "/Templates/default/index_logged.php?main_loaded=1&cur_module=/" + file );
