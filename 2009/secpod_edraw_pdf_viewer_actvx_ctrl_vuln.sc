...
key = "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall" + "\\PDF Viewer Component_is1";
pdfViewName = registry_get_sz( key: key, item: "DisplayName" );
if(ContainsString( pdfViewName, "PDF Viewer Component" )){
	exePath = registry_get_sz( key: key, item: "InstallLocation" );
	if(!exePath){
		exit( 0 );
	}
	share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: exePath );
	file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: exePath + "pdfviewer.exe" );
