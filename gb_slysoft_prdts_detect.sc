...
anydvdPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\AnyDVD.exe", item: "Path" );
if(anydvdPath){
	...
	if(anydvdVer != NULL){
		...
		register_and_report_cpe( app: "AnyDVD", ver: anydvdVer, base: "cpe:/a:slysoft:anydvd:", expr: "^([0-9.]+)", insloc: anydvdPath );
	}
}
clonedvdPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\CloneDVD2.exe", item: "Path" );
if( clonedvdPath ){
	dvdVer = get_version( dllPath: clonedvdPath + "\\CloneDVD2.exe", string: "prod", offs: 332560 );
	if(dvdVer != NULL){
		set_kb_item( name: "Slysoft/Products/Installed", value: TRUE );
		set_kb_item( name: "CloneDVD/Ver", value: dvdVer );
		register_and_report_cpe( app: "CloneDVD", ver: dvdVer, base: "cpe:/a:slysoft:clonedvd:", expr: "^([0-9.]+)", insloc: clonedvdPath );
	}
}
else {
	clonedvdPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\CloneDVD.exe", item: "Path" );
...
clonecdPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\CloneCD.exe", item: "Path" );
if(clonecdPath){
	cdVer = get_version( dllPath: clonecdPath + "\\CloneCD.exe", string: "prod", offs: 332560 );
	if(cdVer != NULL){
		set_kb_item( name: "Slysoft/Products/Installed", value: TRUE );
		set_kb_item( name: "CloneCD/Ver", value: cdVer );
		register_and_report_cpe( app: "CloneCD", ver: cdVer, base: "cpe:/a:slysoft:clonecd:", expr: "^([0-9.]+)", insloc: clonecdPath );
	}
}
drivePath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\VCDPrefs.exe", item: "Path" );
if(drivePath){
	...
	if(driveVer != NULL){
		...
		register_and_report_cpe( app: "Virtual CloneDrive", ver: driveVer, base: "cpe:/a:slysoft:virtualclonedrive:", expr: "^([0-9.]+)", insloc: drivePath );
	}
}
exit( 0 );
