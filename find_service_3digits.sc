...
func read_answer( socket ){
	var r, answer, i, retry;
	retry = 2;
	for{
		for(i = 0;i <= retry;i++){
		...
		if( !r || IsMatchRegexp( r, "^[0-9]{3}[^-]" ) || strlen( answer ) > 1000000 ){
			break;
		}
	}
	return answer;
...
if(!IsMatchRegexp( help, "^50[0-9]" )){
	if(ContainsString( help, "ARTICLE" ) || ContainsString( help, "NEWGROUPS" ) || ContainsString( help, "XHDR" ) || ContainsString( help, "XOVER" ) || IsMatchRegexp( banner, "^[0-9]{3} .*(NNTP|NNRP)" )){
	...
	if(egrep( pattern: "^101", string: banner ) && ( egrep( pattern: "[a-zA-Z]+broker", string: banner, icase: TRUE ) || egrep( pattern: "portmapper tcp PORTMAPPER", string: banner ) )){
		service_register( port: port, proto: "imqbrokerd" );
		log_message( port: port, data: "A Message Queue broker is running at this port." );
		close( soc );
		exit( 0 );
	}
	if(ContainsString( help, "PORT" ) || ContainsString( help, "PASV" )){
		service_report( port: port, svc: "ftp", banner: banner );
		close( soc );
		exit( 0 );
	}
	if(IsMatchRegexp( help, "^220 .* SNPP " ) || egrep( string: help, pattern: "^214 .*PAGE" )){
		service_report( port: port, svc: "snpp", banner: banner );
		close( soc );
		exit( 0 );
	}
	if(egrep( string: help, pattern: "^214-? " ) && ContainsString( help, "MDMFMT" )){
...
if(IsMatchRegexp( date, "^111[ \t]+2[0-9]{3}[01][0-9][0-3][0-9][0-2][0-9][0-5][0-9][0-5][0-9]" )){
	service_report( port: port, svc: "nntp", banner: banner );
	close( soc );
	exit( 0 );
}
ftp_commands = make_list( "CWD",
	 "SYST",
	 "PORT",
	 "PASV" );
ko = 0;
...
if(egrep( pattern: "^220 Bot Server", string: help ) || ContainsString( help, raw_string( 0xb0, 0x3e, 0xc3, 0x77, 0x4d, 0x5a, 0x90 ) )){
	service_report( port: port, svc: "agobot.fo", banner: banner );
	exit( 0 );
}
if(ContainsString( help, "500 P-Error" ) && ContainsString( help, "220 Hello" )){
	service_report( port: port, svc: "unknown_irc_bot", banner: banner );
	exit( 0 );
}
if(ContainsString( help, "220 WinSock" )){
...
if(substr( banner, 0, 3 ) == "200 "){
	...
	if(soc){
		...
		banner = read_answer( socket: soc );
		send( socket: soc, data: NASLString( "USER ", vt_strings["lowercase"], "\\r\\n" ) );
		r = read_answer( socket: soc );
		if(strlen( r ) > 3 && substr( r, 0, 3 ) == "200 "){
			send( socket: soc, data: NASLString( "PASS ", vt_strings["lowercase_rand"], "\\r\\n" ) );
			r = read_answer( socket: soc );
