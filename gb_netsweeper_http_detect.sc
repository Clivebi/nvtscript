...
if(egrep( pattern: ">Netsweeper WebAdmin", string: res, icase: TRUE ) && ContainsString( res, "Set-Cookie: webadmin" )){
	install = "/";
	version = "unknown";
	url = "/webadmin/tools/systemstatus_remote.php";
	headers = make_list( "Content-Type",
	...
	os_register_and_report( os: "CentOS", cpe: "cpe:/o:centos:centos", desc: "Netsweeper Detection (HTTP)", runs_key: "unixoide" );
	cpe = build_cpe( value: version, exp: "^([0-9.]+)", base: "cpe:/a:netsweeper:netsweeper:" );
	if(!cpe){
		cpe = "cpe:/a:netsweeper:netsweeper";
	}
	register_product( cpe: cpe, location: install, port: port, service: "www" );
	log_message( data: build_detection_report( app: "Netsweeper", version: version, install: install, cpe: cpe, concluded: vers[0], concludedUrl: conclUrl ), port: port );
}
exit( 0 );
