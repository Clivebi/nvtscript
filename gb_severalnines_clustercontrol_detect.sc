...
if(ContainsString( res, "<title>Severalnines ClusterControl</title>" )){
	version = "unknown";
	...
	register_product( cpe: cpe, location: "/clustercontrol", port: port, service: "www" );
	log_message( data: build_detection_report( app: "Severalnines ClusterControl", version: version, install: "/clustercontrol", cpe: cpe, concluded: vers[0] ), port: port );
	exit( 0 );
}
exit( 0 );
