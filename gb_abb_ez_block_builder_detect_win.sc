if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of ABB EZ Block Builder." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "ABB EZ Block Builder" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:abb:ez_block_builder:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
