...
for dir in nasl_make_list_unique( "/fm", "/bds", "/aps", "/filestore", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( port: port, item: dir + "/Login.do" );
	if(( ContainsString( res, ">Biscom SFT<" ) || ContainsString( res, ">Biscom Secure File Transfer<" ) ) && ContainsString( res, "bds.uploader.js" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Biscom Secure File Transfer", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
