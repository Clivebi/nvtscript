if(description){
	...
	script_tag( name: "last_modification", value: "2021-05-28 07:20:53 +0000 (Fri, 28 May 2021)" );
...
res = http_keepalive_send_recv( port: port, data: req );
if(ContainsString( res, "id=\"gateone\"" ) && ContainsString( res, "GateOne.init()" )){
	version = "unknown";
	...
	os_register_and_report( os: "Linux/Unix", cpe: "cpe:/o:linux:kernel", runs_key: "unixoide", desc: "Gate One Detection (HTTP)" );
	log_message( data: build_detection_report( app: "Gate One", version: version, install: "/", cpe: cpe ), port: port );
	exit( 0 );
}
exit( 0 );
