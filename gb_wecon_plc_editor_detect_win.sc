...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Wecon PLC Editor" )){
			continue;
		}
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:we-con:plc_editor:", expr: "^([0-9.u]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
