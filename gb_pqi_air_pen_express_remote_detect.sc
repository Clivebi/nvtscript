...
if(res && ContainsString( res, "<TITLE>Air Pen express" )){
	install = "/";
	version = "unknown";
	set_kb_item( name: "pqi/air_pen_express/detected", value: TRUE );
	cpe = "cpe:/a:pqi:air_pen_express";
	register_product( cpe: cpe, location: install, port: port, service: "www" );
	log_message( data: build_detection_report( app: "PQI Air Pen Express", version: version, install: install, cpe: cpe ), port: port );
}
exit( 0 );
