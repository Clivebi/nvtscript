...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Zelio Soft" )){
			continue;
		}
		...
		register_and_report_cpe( app: "Schneider Electric " + appName, ver: version, concluded: concluded, base: "cpe:/a:schneider-electric:zelio_soft2:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
