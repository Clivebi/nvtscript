	...
	if(fileVer){
		...
		if(hotfix_check_sp( win2003: 3 ) > 0){
			...
			if( ContainsString( SP, "Service Pack 1" ) ){
				if(egrep( pattern: "^5\\.2\\.3790\\.([0-2]?[0-9]?[0-9]?[0-9]|3(0[0-9][0-9]" + "|1([0-8][0-9]|90)))$", string: fileVer )){
					security_message( port: 0, data: "The target host was found to be vulnerable" );
				}
				exit( 0 );
			}
				...
				if(ContainsString( SP, "Service Pack 2" )){
					if(egrep( pattern: "^5\\.2\\.3790\\.([0-3]?[0-9]?[0-9]?[0-9]|4([0-2][0-9]" + "[0-9]|3([0-4][0-9]|5[0-3])))$", string: fileVer )){
						security_message( port: 0, data: "The target host was found to be vulnerable" );
					}
					exit( 0 );
				}
