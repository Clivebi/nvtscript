...
winPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\COM3\\Setup", item: "Install Path" );
if(!winPath){
	exit( 0 );
}
winPath += "\\nslookup.exe";
...
if(egrep( pattern: "^5\\.(0?1\\.2600\\.([01]?[0-9]?[0-9]?[0-9]|20[0-9][0-9]|21[0-7]" + "[0-9]|2180)|0?0\\.2195\\.([0-5]?[0-9]?[0-9]?[0-9]|6[0-5][[0-9]" + "[0-9]|66[0-5][0-9]|666[0-3]))$", string: winVer )){
