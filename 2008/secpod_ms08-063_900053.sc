...
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: sysPath + "\\drivers\\Srv.sys" );
sysVer = GetVer( file: file, share: share );
if(!sysVer){
	exit( 0 );
}
if(hotfix_check_sp( win2k: 5 ) > 0){
	if(egrep( pattern: "^5\\.0\\.2195\\.([0-6]?[0-9]?[0-9]?[0-9]|7(0[0-9][0-9]|" + "1[0-6][0-9]|17[0-6]))$", string: sysVer )){
...
if(hotfix_check_sp( xp: 4 ) > 0){
	...
	if( ContainsString( SP, "Service Pack 2" ) ){
		if(egrep( pattern: "^5\\.1\\.2600\\.([0-2]?[0-9]?[0-9]?[0-9]|3([0-3][0-9][0-9]|" + "4([0-2][0-9]|3[0-5])))$", string: sysVer )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
		...
		if(ContainsString( SP, "Service Pack 3" )){
			if(egrep( pattern: "^5\\.1\\.2600\\.([0-4]?[0-9]?[0-9]?[0-9]|5([0-5][0-9][0-9]|" + "6([0-6][0-9]|70)))$", string: sysVer )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
			}
			exit( 0 );
		}
...
if(hotfix_check_sp( win2003: 3 ) > 0){
	...
	if( ContainsString( SP, "Service Pack 1" ) ){
		if(egrep( pattern: "^5\\.2\\.3790\\.([0-2]?[0-9]?[0-9]?[0-9]|3[01][0-9][0-9])$", string: sysVer )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
		...
		if(ContainsString( SP, "Service Pack 2" )){
			if(egrep( pattern: "^5\\.2\\.3790\\.([0-3]?[0-9]?[0-9]?[0-9]|4([0-2][0-9][0-9]|" + "3([0-5][0-9]|6[0-2])))$", string: sysVer )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
			}
			exit( 0 );
		}
