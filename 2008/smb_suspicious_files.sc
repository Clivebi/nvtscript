...
func check_reg( nname, url, key, item, exp ){
	var key_h, value, sz, report;
	key = "SOFTWARE\\Classes\\" + key;
	if(!registry_key_exists( key: key )){
		return 0;
	}
	for value in registry_enum_values( key: key ) {
		if( !isnull( value ) ){
			sz = value;
		}
		else {
		...
		if(exp == NULL || ContainsString( tolower( sz ), tolower( exp ) )){
			report = NASLString( "'", nname, "' is installed on the remote host.\\n", "Make sure that the user of the remote host intended to install
...
i = 0;
func fill_names( buffer ){
	var files, n, i, j, debug;
	nname = make_list();
	url = make_list();
	key = make_list();
	items = make_list();
	exp = make_list();
	files = split( keep: FALSE,buffer:buffer );
	n = max_index( files );
	i = 0;
	for(j = 0;j < n;i++){
		if(!( IsMatchRegexp( files[j], "^NAME" ) && IsMatchRegexp( files[j + 1], "^URL" ) && IsMatchRegexp( files[j + 2], "^KEY" ) && IsMatchRegexp( files[j + 3], "^ITEM" ) && IsMatchRegexp( files[j + 4], "^EXP" ) )){
			if(debug){
...
EXP=aclui.dll" );
for(i = 0;nname[i];i++){
	my_file = NASLString( rootfile, "\\", exp[i] );
	myread = smb_read_file( fullpath: my_file, offset: 0, count: 8 );
	if(myread){
		report = NASLString( "The dll ", nname[i], " (", my_file, ") is present on the remote host\\nSolution: ", url[i] );
