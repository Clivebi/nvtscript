...
for item in get_kb_list( "ssh/login/rpms" ) {
	if(( IsMatchRegexp( release, "^FC[7-9]" ) ) && ( IsMatchRegexp( item, "(O|o)pen(O|o)ffice.*" ) )){
		report = NASLString( "Fedora advisory based local check is available to verify if the package is up-to-date as per the vendor advisory. Please run the Fedora Local Checks to confirm." );
