...
if(( officeVer && IsMatchRegexp( officeVer, "^1[12]\\." ) ) || registry_key_exists( key: "SOFTWARE\\Microsoft\\Office" )){
	...
	if(sharedPath){
		...
		file5 = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: sharedPath + "OFFICE11\\msxml5.dll" );
	}
}
...
if(sysPath){
	...
	if(!isnull( share ) && !isnull( file6 )){
		if(egrep( pattern: "^([0-5]\\..*|6\\.([01]?[0-9]|20\\.(0?[0-9]?[0-9]?[0-9]|10([0-8]" + "[0-9]|9[0-8])))\\..*)$", string: GetVer( file: file6, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file6r ) && !isnull( share )){
		if(egrep( pattern: "^([0-5]\\..*|6\\.(0\\.([0-2]?[0-9]?[0-9]?[0-9]|3([0-7]" + "[0-9][0-9]|8[0-7][0-9]|88[0-2])))\\..*)$", string: GetVer( file: file6r, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file5 ) && !isnull( share2 )){
		if(egrep( pattern: "^([0-4]\\..*|5\\.([01]?[0-9]\\..*|20\\.([0-9]?[0-9]?[0-9]|10" + "([0-7][0-9]|8[0-6]))\\..*))$", string: GetVer( file: file5, share: share2 ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file4 ) && !isnull( share )){
		if(egrep( pattern: "^([0-3]\\..*|4\\.([01]?[0-9]\\..*|20\\.([0-8]?[0-9]?[0-9]?[0-9]" + "\\..*|9([0-7][0-9][0-9]|8[0-6][0-9])\\..*)))$", string: GetVer( file: file4, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file4a ) && !isnull( share )){
		if(egrep( pattern: "^([0-3]\\..*|4\\.([0-9]\\..*|10\\.([0-8]?[0-9]?[0-9]?[0-9]" + "\\..*|9([0-3][0-9][0-9]|40[0-3])\\..*)))$", string: GetVer( file: file4a, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file4r ) && !isnull( share )){
		if(egrep( pattern: "^([0-3]\\..*|4\\.([0-9]\\..*|10\\.([0-8]?[0-9]?[0-9]?[0-9]" + "\\..*|9([0-3][0-9][0-9]|40[0-3])\\..*)))$", string: GetVer( file: file4r, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
			exit( 0 );
		}
	}
	if(!isnull( file3 ) && !isnull( share )){
		if(egrep( pattern: "^([0-7]\\..*|8\\.([0-9]?[0-9]\\..*|100\\.([0-9]?[0-9]?[0-9]" + "\\..*|1(0[0-3][0-9]|04[0-7])\\..*)))$", string: GetVer( file: file3, share: share ) )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
...
if(dllVer || dllVer2 || dllVer3){
	if(hotfix_check_sp( winVista: 3, win2008: 3 ) > 0){
		if(version_in_range( version: dllVer, test_version: "8.0", test_version2: "8.100.1047.9" ) || version_in_range( version: dllVer2, test_version: "4.0", test_version2: "4.20.9869.9" ) || version_in_range( version: dllVer3, test_version: "6.0", test_version2: "6.20.1098.9" )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
	}
