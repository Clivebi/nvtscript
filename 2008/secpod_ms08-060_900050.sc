...
ntdsPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\COM3\\Setup", item: "Install Path" );
if(!ntdsPath){
	exit( 0 );
}
share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: ntdsPath );
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: ntdsPath + "\\ntdsa.dll" );
ntdsVer = GetVer( file: file, share: share );
if(!ntdsVer){
	exit( 0 );
}
if(ereg( pattern: "^5\\.0\\.2195\\.([0-6]?[0-9]?[0-9]?[0-9]|70[0-9][0-9]|" + "71([0-6][0-9]|7[0-7]))$", string: ntdsVer )){
