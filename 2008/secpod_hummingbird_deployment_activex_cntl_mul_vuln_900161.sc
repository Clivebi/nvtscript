...
wizPath = registry_get_sz( key: "SOFTWARE\\Hummingbird\\Deployment Wizard", item: "HomeDir" );
if(!wizPath){
	exit( 0 );
}
share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: wizPath );
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: wizPath + "DeployPkgShell.exe" );
name = kb_smb_name();
login = kb_smb_login();
...
close( soc );
if(wizVer){
	if(ereg( pattern: "^[0-9](\\..*)|10(\\.0(\\.0(\\.[0-3]?[0-9]|\\.4[0-4])?)?)($|[^.0-9])", string: wizVer )){
