...
close( soc );
func packet_construct( _ip_src, _ip_ttl ){
	_ip_id = rand() % 65535;
...
func packet_parse( _icmp, _ip_dst, _ttl ){
...
for(ttl = 1;ttl <= 50;ttl++){
	...
	responsepacket = send_packet( packet: requestpacket, pcap_active: TRUE, pcap_filter: packetfilter, pcap_timeout: 1 );
	if(responsepacket){
