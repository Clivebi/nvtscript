...
login_pkt_hdr = raw_string( 0x02, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00 );
login_pkt_hdr2 = raw_string( 0x02, 0x01, 0x00, 0x61, 0x00, 0x00, 0x00, 0x00 );
nul = raw_string( 0x00 );
pkt_src_hostname = crap( data: nul, length: 31 );
pkt_src_process = raw_string( "1", crap( data: nul, length: 29 ), 0x01 );
pkt_magic1 = raw_string( 0x03, 0x01, 0x06, 0x0a, 0x09, 0x01 );
...
pkt_magic10 = raw_string( 0x00, 0x00, 0x00, 0x00, 0xe2, 0x16, 0x00, 0x01, 0x09, 0x00, 0x00, 0x06, 0x6d, 0x7f, 0xff, 0xff, 0xff, 0xfe, 0x02, 0x09, 0x00, 0x00, 0x00, 0x00, 0x0a, 0x68, 0x00, 0x00, 0x00 );
func make_sql_login_pkt( database, username, password ){
	var dblen, dbuf, dlen, dpad, pblen, pbuf, plen, ppad, sql_packet, ublen, ubuf, ulen, upad;
	...
	dbuf = NASLString( database, crap( data: nul, length: dpad ) );
	ubuf = NASLString( username, crap( data: nul, length: upad ) );
	pbuf = NASLString( password, crap( data: nul, length: ppad ) );
	sql_packet = NASLString( login_pkt_hdr, pkt_src_hostname, ubuf, ublen, pbuf, pblen, pkt_src_process, pkt_magic1, pkt_bulk_copy, pkt_magic2, pkt_client, dbuf, dblen, pkt_magic3, pblen, pbuf, pkt_magic4, pkt_passwordlength_plus2, pkt_version, pkt_library, pkt_library_version, pkt_magic6, pkt_language, login_pkt_hdr2, pkt_language2, pkt_magic7, pkt_old_secure, pkt_encrypted, pkt_magic8, pkt_sec_spare, pkt_char_set, pkt_magic9, pkt_block_size, pkt_magic10 );
	return sql_packet;
}
...
close( soc );
if(strlen( r ) > 3 && ord( r[0x00] ) == 4 && ( ord( r[2] ) * 256 + ord( r[3] ) ) == strlen( r )){
	i = 8;
	for(;i < strlen( r );){
		type = ord( r[i] );
