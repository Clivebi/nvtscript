...
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: sysPath + "\\Netapi32.dll" );
dllVer = GetVer( file: file, share: share );
if(!dllVer){
	exit( 0 );
}
if(hotfix_check_sp( win2k: 5 ) > 0){
	if(egrep( pattern: "^5\\.0\\.2195\\.([0-6]?[0-9]?[0-9]?[0-9]|7([01][0-9][0-9]|" + "20[0-2]))$", string: dllVer )){
...
if(hotfix_check_sp( xp: 4 ) > 0){
	...
	if( ContainsString( SP, "Service Pack 2" ) ){
		if(egrep( pattern: "^5\\.1\\.2600\\.([0-2]?[0-9]?[0-9]?[0-9]|3([0-3][0-9][0-9]|" + "4([0-5][0-9]|6[01])))$", string: dllVer )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
		...
		if(ContainsString( SP, "Service Pack 3" )){
			if(egrep( pattern: "^5\\.1\\.2600\\.([0-4]?[0-9]?[0-9]?[0-9]|5([0-5][0-9][0-9]|" + "6([0-8][0-9]|9[0-3])))$", string: dllVer )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
			}
			exit( 0 );
		}
...
if(hotfix_check_sp( win2003: 3 ) > 0){
	...
	if( ContainsString( SP, "Service Pack 1" ) ){
		if(egrep( pattern: "^5\\.2\\.3790\\.([0-2]?[0-9]?[0-9]?[0-9]|3[01][0-9][0-9]|" + "32([01][0-9]|2[0-8]))$", string: dllVer )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
		...
		if(ContainsString( SP, "Service Pack 2" )){
			if(egrep( pattern: "^5\\.2\\.3790\\.([0-3]?[0-9]?[0-9]?[0-9]|4([0-2][0-9][0-9]|" + "3([0-8][0-9]|9[01])))$", string: dllVer )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
			}
			exit( 0 );
		}
