...
func cookie_decode( cookie, param ){
	var match;
...
func str_xor(  ){
	var nargs, result, len, arg1, arg2, i, j;
	...
	for(i = 1;i < nargs;++i){
		...
		result = "";
		for(j = 0;j < len;++j){
			result += raw_string( ord( arg1[j] ) ^ ord( arg2[j] ) );
		}
	}
	return result;
}
...
if(!keystream || strlen( keystream ) == 0){
	exit( 0 );
}
...
if(!guess || strlen( guess ) == 0){
	exit( 0 );
}
if(strleft( get_kb_item( "http/password" ), len ) != guess){
	exit( 0 );
}
report = NASLString( "Sensitive values, including the username and password, can be\\n", "decrypted by XORing the plaintext with the following fixed key\\n", "stream :\\n", "\\n", hexstr( keystream ), "..." );
