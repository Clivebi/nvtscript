...
if(hotfix_check_sp( win2k: 5 ) > 0){
	...
	if(ereg( pattern: "^6\\.0?0\\.2800\\.(0?[0-9]?[0-9]?[0-9]|1([0-8][0-9][0-9]|" + "9[0-2][0-9]|93[0-2]))$", string: vers )){
		security_message( port: 0, data: "The target host was found to be vulnerable" );
		exit( 0 );
	}
}
if(hotfix_check_sp( xp: 4 ) > 0){
	...
	if(ContainsString( SP, "Service Pack 2" )){
		if(ereg( pattern: "^6\\.0?0\\.2900\\.([0-2]?[0-9]?[0-9]?[0-9]|3([0-2][0-9][0-9]|" + "3[0-4][0-9]))$", string: vers )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
	if( ContainsString( SP, "Service Pack 3" ) ){
		if(ereg( pattern: "^6\\.0?0\\.2900\\.([0-4]?[0-9]?[0-9]?[0-9]|5([0-4][0-9][0-9]|" + "5[0-6][0-9]|57[0-8]))$", string: vers )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
...
if(hotfix_check_sp( win2003: 3 ) > 0){
	...
	if(ContainsString( SP, "Service Pack 1" )){
		if(ereg( pattern: "^6\\.0?0\\.3790\\.([0-2]?[0-9]?[0-9]?[0-9]|3(0[0-9][0-9]|" + "1[0-5][0-9]|16[0-7]))$", string: vers )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
	if( ContainsString( SP, "Service Pack 2" ) ){
		if(ereg( pattern: "^6\\.0?0\\.3790\\.([0-3]?[0-9]?[0-9]?[0-9]|4([0-2][0-9][0-9]|" + "3[01][0-9]|32[0-4]))$", string: vers )){
			security_message( port: 0, data: "The target host was found to be vulnerable" );
		}
		exit( 0 );
	}
