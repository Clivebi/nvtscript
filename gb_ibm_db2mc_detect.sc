...
for dir in nasl_make_list_unique( "/", "/DMC", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( rcvRes, "DB2 Monitoring Console" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "IBM DB2 Monitoring Console", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
