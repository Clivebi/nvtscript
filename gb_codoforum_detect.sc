...
for dir in nasl_make_list_unique( "/", "/codoforum", "/forum", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( rcvRes, ">Welcome to Codoforum<" ) && ContainsString( rcvRes, "content=\"codoforum" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Codoforum", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
