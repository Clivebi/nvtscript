if(description){
	...
	script_dependencies( "gb_argosoft_mailserver_http_detect.sc", "gb_argosoft_mailserver_smtp_detect.sc", "gb_argosoft_mailserver_pop3_detect.sc" );
...
for proto in make_list( "smtp",
	...
	for port in ports {
		...
		if( concluded == "" ) {
			...
			if(!ContainsString( concluded, protoupper )){
				concluded += ", " + protoupper;
			}
		}
		if(!isnull( concl )){
...
report = build_detection_report( app: "ArgoSoft Mail Server", version: version, cpe: CPE, concluded: concluded, extra: extra );
