if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of Schneider Electric EcoStruxure Machine Expert - HVAC." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Schneider Electric EcoStruxure[ -~] Machine Expert - HVAC" )){
			continue;
		}
		version = "unknown";
		concluded = appName;
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:schneider:ecostruxure_machine_expert:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
