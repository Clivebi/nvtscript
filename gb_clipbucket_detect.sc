...
for dir in nasl_make_list_unique( "/", "/clipbucket", "/ClipBucket", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( port: port, item: dir + "/admin_area/login.php" );
	if(ContainsString( res, "ClipBucket Copyright" ) && ( ContainsString( res, ">Sign in with your Clipbucket Account" ) || ContainsString( res, ">Username" ) ) && ( IsMatchRegexp( res, "<title>Admin Login - ClipBucket.*</title>" ) || ContainsString( res, "Arslan Hassan" ) )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "ClipBucket", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
