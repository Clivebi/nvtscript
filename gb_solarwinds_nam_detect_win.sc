...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "SolarWinds Network Automation Manager" )){
			continue;
		}
		version = "unknown";
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:solarwinds:network_automation_manager:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
