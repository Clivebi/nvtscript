...
for dir in nasl_make_list_unique( "/aproxengine", "/cms", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( buf, "AproxEngine" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "AproxEngine CMS", version: vers, install: install, cpe: cpe, concluded: version[0], concludedUrl: url ), port: port );
		exit( 0 );
	}
}
exit( 0 );
