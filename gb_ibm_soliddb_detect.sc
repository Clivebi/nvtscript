...
tcp = NASLString( "tcp ", host, " ", port );
vt_strings = get_vt_strings();
id = vt_strings["default"] + "(" + this_host() + ")";
set_byte_order( BYTE_ORDER_LITTLE_ENDIAN );
req = raw_string( 0x02, 0x00, 0x00, 0x00, 0x00, 0x0c, 0x00 ) + mkdword( 1 ) + mkdword( strlen( tcp ) ) + tcp + mkdword( strlen( user ) ) + user + mkdword( strlen( pass ) ) + pass + mkdword( 4 ) + mkdword( 3 ) + mkdword( 2 ) + mkdword( 1 ) + mkdword( 1 ) + mkdword( 0 ) + mkdword( strlen( id ) + 3 ) + raw_string( 0x04 ) + mkword( strlen( id ) ) + id;
...
len = strlen( ret );
if(( len == 35 || len >= 27 ) && hexstr( substr( ret, 0, 6 ) ) == "02000100000000" && hexstr( substr( ret, 6, 7 ) ) == "0001"){
	install = port + "/tcp";
	...
	req = raw_string( 0x02, 0x00, 0x00, 0x00, 0x00, 0x0b, 0x00 ) + mkdword( 2 ) + mkdword( a ) + mkdword( b ) + mkdword( 0x012d ) + mkdword( strlen( version_cmd ) ) + version_cmd;
	send( socket: soc, data: req );
	ret = recv( socket: soc, length: 1024 );
	close( soc );
	if(ContainsString( ret, "solidDB" )){
		s = 19;
		for(;l = getdword( blob: ret, pos: s );){
			if( s + 4 + l < strlen( ret ) ){
	...
	if( vers == "unknown" || isnull( vers ) ){
		...
		log_message( data: build_detection_report( app: "IBM solidDB Server", version: vers, install: install, cpe: cpe ), port: port );
	}
	else {
		...
		register_product( cpe: cpe, location: install, port: port );
		log_message( data: build_detection_report( app: "IBM solidDB Server", version: vers, install: install, cpe: cpe, extra: "The remote solidDB has default credentials set. You should change this credentials as soon as possible.", concluded: version_string ), port: port );
	}
}
exit( 0 );
