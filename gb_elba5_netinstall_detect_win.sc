...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "ELBA5" )){
			continue;
		}
		set_kb_item( name: "racon_software/elba/win/detected", value: TRUE );
		split = split( buffer: appName, sep: " " );
		appName = split[0];
		version = "unknown";
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:racon_software:elba5:", expr: "^([0-9.]+) ?([A-Z0-9]+)?", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
