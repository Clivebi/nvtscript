...
samPort = http_get_port( default: 80 );
res = http_get_cache( port: samPort, item: "/sws/index.sws" );
if(ContainsString( res, "<title>SyncThru Web Service</title>" ) && IsMatchRegexp( res, "Copyright.*Samsung Electronics" ) && ContainsString( res, "Login" )){
	version = "unknown";
	install = "/";
	...
	if(IsMatchRegexp( res, "HTTP/1.. 200 OK" ) && IsMatchRegexp( res, "<title>SWS.*Information.*Firmware.Version.</title>" )){
	...
	register_product( cpe: cpe, location: install, port: samPort, service: "www" );
	log_message( data: build_detection_report( app: "Samsung SyncThru Web Service", version: version, install: install, cpe: cpe, concluded: "Main Firmware Version " + version + " with Network Firmware Version " + netVer ), port: samPort );
}
exit( 0 );
