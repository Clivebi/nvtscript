...
for dir in nasl_make_list_unique( "/", "/connx", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( buf, "Login to ConnX" ) && ContainsString( buf, "ConnXButton" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "ConnX", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
