if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of Siemens SIMATIC Prosave." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "SIMATIC Prosave V[0-9.]+" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "Siemens " + appName, ver: version, concluded: concluded, base: "cpe:/a:siemens:simatic_prosave:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
