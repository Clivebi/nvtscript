...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "AVG (Antivirus FREE|Business Security)" )){
			continue;
		}
		...
		if( appName == "AVG Antivirus FREE" ){
			...
			if(appName == "AVG Business Security"){
				set_kb_item( name: "avg/antivirus_business/detected", value: TRUE );
			}
		}
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:avg:anti-virus:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
