...
for dir in nasl_make_list_unique( "/todoyu", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( buf, "todoyu</title>" ) && ContainsString( buf, "todoyu-loginpage" ) && ContainsString( buf, "snowflake productions" )){
		vers = NASLString( "unknown" );
		set_kb_item( name: NASLString( "www/", port, "/todoyu" ), value: NASLString( vers, " under ", install ) );
		set_kb_item( name: "todoyu/detected", value: TRUE );
		info = NASLString( "todoyu Version '" );
