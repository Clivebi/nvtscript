...
for dir in nasl_make_list_unique( "/axis", "/shop", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>Login to Axis administrator panel", string: buf, icase: TRUE ) && ContainsString( buf, "Powered by Axis" )){
		vers = NASLString( "unknown" );
		version = eregmatch( string: buf, pattern: "Powered by Axis v. ([0-9.]+)", icase: TRUE );
		if(!isnull( version[1] )){
			vers = chomp( version[1] );
		}
		set_kb_item( name: NASLString( "www/", port, "/axis_ecommerce" ), value: NASLString( vers, " under ", install ) );
		set_kb_item( name: "axis_ecommerce/detected", value: TRUE );
		info = NASLString( "Axis Commerce Version '" );
