...
if(!registry_key_exists( key: "SOFTWARE\\IBM\\IBM Informix Dynamic Server" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\IBM\\IBM Informix Dynamic Server" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(IsMatchRegexp( name, "IBM Informix Dynamic Server$" )){
		...
		if(version){
			...
			log_message( data: build_detection_report( app: name, version: version, install: insloc, cpe: cpe, concluded: version ) );
			exit( 0 );
		}
	}
}
