...
if(ContainsString( buf, "SolarWinds Platform" ) || ContainsString( buf, "SolarWinds Orion" ) || ContainsString( buf, "Orion Platform" )){
	...
	if(!isnull( vnqm )){
		...
		set_kb_item( name: NASLString( "www/", port, "/orion_vnqm" ), value: vers );
		...
		log_message( data: build_detection_report( app: "SolarWinds VoIP and Network Quality Manager", version: vers, install: dir, cpe: cpe, concluded: vnqm[0] ), port: port );
	}
}
exit( 0 );
