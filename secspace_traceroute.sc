if(description){
	...
	script_version( "2021-03-12T14:25:59+0000" );
	script_tag( name: "last_modification", value: "2021-03-12 14:25:59 +0000 (Fri, 12 Mar 2021)" );
...
func get_filter( ipsrc, port, sport, ipdst ){
	var ipsrc, port, sport, ipdst;
...
func read_packet( packet, ipdst, ipid, sport ){
	var packet, ipdst, ipid, sport;
	var _IPP, ip_hl, _ip_src, ip, ip_dst, id, target_answer, d, p;
	if(!packet){
		return;
	}
...
if(islocalhost()){
	h = get_host_ip();
	hops_count = 1;
	r = NASLString( "Network route from scanner (", h, ") to target (", h, "):\\n\\n", h );
...
for(;TRUE;){
	for(i = 0;i < 2;i++){
		...
		response = send_packet( packet: pkt, pcap_active: TRUE, pcap_filter: pcapfil, pcap_timeout: 1 );
		if(response){
...
hops_count += 2;
hops_report = NASLString( "Network route from scanner (", ipsrc, ") to target (", ipdst, "):\\n\\n" );
if(hops_count == 2 && error > 3){
	ipdst = "?";
	register_hd = FALSE;
}
hops = NASLString( ipsrc, ",", hops, ipdst );
set_kb_item( name: "traceroute/hops", value: hops_count );
set_kb_item( name: "traceroute/route", value: hops );
if(register_hd){
	register_host_detail( name: "traceroute", value: hops, desc: "Traceroute" );
}
hops_report += ereg_replace( string: hops, pattern: ",", replace: NASLString( "\\n" ) );
hops_report += NASLString( "\\n\\nNetwork distance between scanner and target: ", hops_count );
