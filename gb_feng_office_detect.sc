...
rootInstalled = FALSE;
for dir in nasl_make_list_unique( "/", "/feng_community", "/fengoffice", "/feng", "/office", http_cgi_dirs( port: port ) ) {
	if(rootInstalled){
		break;
	}
	install = dir;
	...
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && egrep( pattern: "Powered by .*Feng Office.* - version ", string: res ) && ContainsString( res, "<title>Login</title>" )){
		version = "unknown";
		if(dir == ""){
			rootInstalled = TRUE;
		}
		...
		log_message( data: build_detection_report( app: "Feng Office", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
