...
if(!registry_key_exists( key: "SOFTWARE\\Hewlett Packard Enterprise\\Sizers\\HPE Insight Management Sizing Tool" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\Hewlett Packard Enterprise\\Sizers\\HPE Insight Management Sizing Tool" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HPE Insight Management Sizing Tool" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HPE Insight Management Sizer", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
