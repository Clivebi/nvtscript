...
for key in keylist {
	if(registry_key_exists( key: key )){
		for item in registry_enum_keys( key: key ) {
			...
			if(ContainsString( confmgrName, "Microsoft Systems Management Server 2003" )){
				...
				if(registry_key_exists( key: newKey )){
					...
					if(smsVer){
						...
						log_message( data: build_detection_report( app: confmgrName, version: smsVer, install: smsPath, cpe: cpe, concluded: smsVer ) );
					}
				}
			}
			if(ContainsString( confmgrName, "Microsoft System Center Configuration Manager 2007" ) && !IsMatchRegexp( confmgrName, "R[23]" )){
				...
				if(registry_key_exists( key: newKey )){
					...
					if(confmgrVer){
						...
						log_message( data: build_detection_report( app: confmgrName, version: confmgrVer, install: confmgrPath, cpe: cpe, concluded: confmgrVer ) );
					}
				}
			}
			if(IsMatchRegexp( confmgrName, "Microsoft System Center Configuration Manager 2007 [R3|R2]" )){
				confmgrVer = registry_get_sz( key: key + item, item: "DisplayVersion" );
				if(confmgrVer){
					set_kb_item( name: "MS/SMS_or_ConfigMgr/Installed", value: TRUE );
					if(IsMatchRegexp( confmgrName, "R3" )){
						set_kb_item( name: "MS/ConfigMgr-R3/Version", value: confmgrVer );
					}
					if(IsMatchRegexp( confmgrName, "R2" )){
