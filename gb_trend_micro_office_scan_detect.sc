...
if(!registry_key_exists( key: "SOFTWARE\\TrendMicro\\OfficeScan" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\TrendMicro\\OfficeScan" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( trendName, "Trend Micro OfficeScan" )){
		...
		log_message( data: build_detection_report( app: "Trend Micro OfficeScan", version: trendVer, install: trendPath, cpe: cpe, concluded: trendVer ) );
		exit( 0 );
	}
}
