...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "PowerStudio OPC Server" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "Circutor " + appName, ver: version, concluded: concluded, base: "cpe:/a:circutor:powerstudio_opc_server:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
