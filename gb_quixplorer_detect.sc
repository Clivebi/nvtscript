if(description){
	...
	script_tag( name: "summary", value: "This script finds the installed QuiXplorer Version." );
...
for dir in nasl_make_list_unique( "/quixplorer", "/quixplore", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "QuiXplorer" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "QuiXplorer", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
