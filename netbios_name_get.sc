if(description){
	...
	script_dependencies( "cifs445.sc" );
...
NETBIOS_LEN = 50;
sendata = raw_string( rand() % 255, rand() % 255, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x43, 0x4B, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x00, 0x00, 0x21, 0x00, 0x01 );
sendata_SMBSERVER = raw_string( rand() % 255, rand() % 255, 0x00, 0x10, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x43, 0x4b, 0x46, 0x44, 0x45, 0x4e, 0x45, 0x43, 0x46, 0x44, 0x45, 0x46, 0x46, 0x43, 0x46, 0x47, 0x45, 0x46, 0x46, 0x43, 0x43, 0x41, 0x43, 0x41, 0x43, 0x41, 0x43, 0x41, 0x43, 0x41, 0x43, 0x41, 0x00, 0x00, 0x21, 0x00, 0x01 );
...
if(strlen( result ) > 56){
	...
	if(num_of_names > 0){
		hole_answer = NASLString( hole_answer, "The following ", num_of_names, " NetBIOS names have been gathered :\\n" );
	}
	location++;
	for(name_count = 0;name_count < num_of_names;name_count++){
		...
		for(name_copy = 0;name_copy < 15;name_copy++){
			...
			if( isprint( c: hole_data[location + name_copy + name_count * 18] ) ){
				name = NASLString( name, hole_data[location + name_copy + name_count * 18] );
			}
			else {
				name = NASLString( name, " " );
			}
		}
		...
		if(hole_data[loc] == raw_string( 68 )){
			...
			if( ord( hole_data[subloc] ) == 32 ){
				...
				name_list = make_list( name_list,
					 name + " = This is the computer name." );
			}
			else {
				if(ord( hole_data[subloc] ) == 0){
					candidate = name;
		...
		if(hole_data[loc] == raw_string( 4 )){
			...
			if(hole_data[subloc] == raw_string( 0 )){
				...
				if( !ContainsString( name, "~" ) ){
					name_list = make_list( name_list,
						 name + " = This is the computer name registered for workstation services by a WINS client." );
				}
				else {
					name_list = make_list( name_list,
						 name );
	...
	for(adapter_count = 0;adapter_count < 6;adapter_count++){
		...
		adapter_name += tolower( NASLString( hex( ord( hole_data[loc] ) ), col ) ) - "0x";
	}
	if( adapter_name == "00:00:00:00:00:00" ){
		set_kb_item( name: "SMB/samba", value: TRUE );
		hole_answer += NASLString( "\\nThis SMB server seems to be a SAMBA server (this is not a security risk, this is for your information). This can be told because this server claims to have a null MAC address." );
		os_register_and_report( os: "Linux/Unix", cpe: "cpe:/o:linux:kernel", banner_type: BANNER_TYPE, port: dsport, proto: "udp", banner: "null MAC address of a Samba server", desc: SCRIPT_DESC, runs_key: "unixoide" );
	}
	else {
		hole_answer += NASLString( "\\nThe remote host has the following MAC address on its adapter :\\n" );
		hole_answer += " " + adapter_name;
		register_host_detail( name: "MAC", value: adapter_name, desc: SCRIPT_DESC );
		os_register_and_report( os: "Microsoft Windows", cpe: "cpe:/o:microsoft:windows", banner_type: BANNER_TYPE, port: dsport, proto: "udp", desc: SCRIPT_DESC, runs_key: "windows" );
	}
