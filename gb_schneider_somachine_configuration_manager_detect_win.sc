...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "SoMachine Software" )){
			continue;
		}
		pub = registry_get_sz( key: key + item, item: "Publisher" );
		if(!pub || !IsMatchRegexp( pub, "Schneider Electric" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "Schneider Electric SoMachine Configuration Manager", ver: version, concluded: concluded, base: "cpe:/a:schneider-electric:somachine_configuration_manager:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
