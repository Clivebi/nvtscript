...
if(ContainsString( buf, "SolarWinds Platform" ) || ContainsString( buf, "SolarWinds Orion" ) || ContainsString( buf, "Orion Platform" )){
	...
	if(!isnull( ipam )){
		...
		set_kb_item( name: NASLString( "www/", port, "/orion_ipam" ), value: vers );
		...
		log_message( data: build_detection_report( app: "SolarWinds IP Address Manager", version: vers, install: dir, cpe: cpe, concluded: ipam[0] ), port: port );
	}
}
exit( 0 );
