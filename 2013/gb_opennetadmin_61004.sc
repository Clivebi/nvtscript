...
for dir in nasl_make_list_unique( "/ona", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	url = dir + "/";
	buf = http_get_cache( item: url, port: port );
	if(buf && ContainsString( buf, "<title>OpenNetAdmin ::" )){
		install = dir;
		break;
	}
}
...
if(!ContainsString( result, "Module ADDED" ) || !ContainsString( result, check )){
	exit( 99 );
}
