...
if(description){
	...
	script_dependencies( "sw_mysqldumper_detect.sc" );
...
url = dir + "/sql.php?db=-'%20union%20select%201,2," + "'VT-SQL-Injection-Test'%20from%20tblusers%20where%20'1";
if(http_vuln_check( port: port, url: url, check_header: TRUE, pattern: "vt-sql-injection-test", extra_check: make_list( "Database",
	 "Table View" ) )){
