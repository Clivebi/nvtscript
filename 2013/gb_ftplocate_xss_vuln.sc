...
for dir in nasl_make_list_unique( "/", "/ftplocate", "/ftp", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	req = http_get( item: NASLString( dir, "/flsummary.pl" ), port: port );
	res = http_keepalive_send_recv( port: port, data: req, bodyonly: TRUE );
	if(ContainsString( res, ">FtpLocate" ) && ContainsString( res, "Ftp Search Engine<" )){
