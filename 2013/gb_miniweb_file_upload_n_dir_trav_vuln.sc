...
file = NASLString( "ov-upload-test-", rand_str( length: 5 ), ".txt" );
req = create_upload_req( url: url, file: file, host: host );
http_keepalive_send_recv( port: port, data: req );
if(http_vuln_check( port: port, url: NASLString( "/", file ), check_header: TRUE, pattern: "File-Upload-Vulnerability-Test" )){
	msg = "Scanner has created a file " + file + " to check the vulnerability." + " Please remove this file as soon as possible.";
