...
if(description){
	...
	script_dependencies( "gb_ganglia_detect.sc" );
...
url = dir + "/?r=custom&cs=1&ce=1&s=by+name&c=1&h=&host_regex='><script>" + "alert(document.cookie)</script>&max_graphs=0&tab=m&vn=&hid" + "e-hf=false&sh=1&z=small&hc=0";
if(http_vuln_check( port: port, url: url, check_header: TRUE, pattern: "host_regex value=''><script>alert\\(document.cookie\\)</script>", extra_check: ">Ganglia" )){
