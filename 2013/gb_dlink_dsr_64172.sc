...
if(!ContainsString( banner, "Server: Embedded HTTP Server" ) && !ContainsString( banner, "Unified Services Router" )){
	exit( 0 );
}
useragent = http_get_user_agent();
for dir in make_list( "/scgi-bin/",
	 "/" ) {
	url = dir + "platform.cgi";
	req = http_get( item: url, port: port );
	buf = http_send_recv( port: port, data: req, bodyonly: FALSE );
	if(!ContainsString( buf, "Unified Services Router" )){
		continue;
	}
	...
	buf = http_send_recv( port: port, data: req, bodyonly: FALSE );
	if(( ContainsString( buf, "adminSettings.htm" ) && ContainsString( buf, ">Logout<" ) ) || ( ContainsString( buf, ">User already logged in<" ) )){
