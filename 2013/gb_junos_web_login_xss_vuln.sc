...
if(!ContainsString( banner, "Server: Mbedthis-Appweb/" )){
	exit( 0 );
}
res = http_get_cache( item: "/index.php", port: port );
if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "Juniper Networks, Inc" ) && ContainsString( res, ">Log In" )){
	url = "/index.php?name=Your_Account&error=1\"><script>" + "alert(document.cookie)<%2Fscript>&uname=bGF";
	if(http_vuln_check( port: port, url: url, check_header: TRUE, pattern: "><script>alert\\(document\\.cookie\\)</script>", extra_check: make_list( ">Log In",
		 "Juniper Networks" ) )){
