...
if( !IsMatchRegexp( uname, "SunOS .*" ) ){
	...
	if(IsMatchRegexp( pamlogin, ".*such.file.*directory" )){
		pamlogin = "none";
	}
	if(IsMatchRegexp( pamsshd, ".*such.file.*directory" )){
		pamsshd = "none";
	}
	if(IsMatchRegexp( pamgdm, ".*such.file.*directory" )){
		pamgdm = "none";
	}
	if(IsMatchRegexp( pamxdm, ".*such.file.*directory" )){
		pamxdm = "none";
	}
	if(IsMatchRegexp( pamkde, ".*such.file.*directory" )){
		pamkde = "none";
	}
	if(IsMatchRegexp( pamcomacc, ".*such.file.*directory" )){
		pamcomacc = "none";
	}
	if(IsMatchRegexp( pamcomses, ".*such.file.*directory" )){
		pamcomses = "none";
	}
	if(IsMatchRegexp( pamcompas, ".*such.file.*directory" )){
		pamcompas = "none";
	}
	if(IsMatchRegexp( pamcomauth, ".*such.file.*directory" )){
		pamcomauth = "none";
	}
	if(IsMatchRegexp( limits, ".*such.file.*directory" )){
		limits = "none";
	}
	if(IsMatchRegexp( pampasswd, ".*such.file.*directory" )){
		pampasswd = "none";
	}
	if(IsMatchRegexp( pamcompasswd, ".*such.file.*directory" )){
		pamcompasswd = "none";
	}
}
else {
	if(IsMatchRegexp( uname, "SunOS .*" )){
		...
		if( IsMatchRegexp( pamconf, ".*such.file.*directory" ) ){
...
if(limits != "none" && limits != "empty"){
	Lst = split( buffer: limits, keep: 0 );
...
if( pamlogin != "none" ){
	Lst = split( buffer: pamlogin, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(Lst[i] == ""){
			continue;
		}
		if( IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
			login_pamlastlog = "truefail";
		}
		else {
			if(IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*" )){
				login_pamlastlog = "true";
			}
		}
		if(IsMatchRegexp( Lst[i], "^.*session.*required.*pam_limits.so.*" )){
			login_pamlimits = "true";
		}
		if(IsMatchRegexp( Lst[i], "^@include.*common-session,*" )){
			Lst1 = split( buffer: pamcomses, keep: 0 );
			for(j = 0;j < max_index( Lst1 );j++){
				if(Lst1[j] == ""){
					continue;
				}
				if(login_pamlastlog != "truefail"){
					if( IsMatchRegexp( Lst1[j], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
						login_pamlastlog = "truefail";
					}
					else {
						if(IsMatchRegexp( Lst1[j], "^.*session.*pam_lastlog.so.*" )){
							login_pamlastlog = "true";
						}
					}
				}
				if(!login_pamlimits){
					if(IsMatchRegexp( Lst1[i], "^.*session.*required.*pam_limits.so.*" )){
						login_pamlimits = "true";
					}
				}
			}
		}
		if(IsMatchRegexp( Lst[i], "^.*auth.*required.*pam_tally.so.*deny=.*" )){
			login_pamtally = "true";
			login_pamtally_data = Lst[i];
		}
		if(!login_pamtally){
			if(IsMatchRegexp( Lst[i], "^@include.*common-auth,*" )){
				Lst2 = split( buffer: pamcomauth, keep: 0 );
				for(k = 0;k < max_index( Lst2 );k++){
					if(IsMatchRegexp( Lst2[k], "^.*auth.*required.*pam_tally.so.*deny=.*" )){
						login_pamtally = "true";
						login_pamtally_data = Lst2[k];
					}
				}
			}
		}
	...
	if(!login_pamlastlog){
		login_pamlastlog = "fail";
	}
	if(!login_pamlimits){
		login_pamlimits = "fail";
	}
	if(!login_pamtally){
		login_pamtally_data = "fail";
	}
	if(!login_pamtally){
		login_pamtally = "fail";
	}
...
if( pamsshd != "none" ){
	Lst = split( buffer: pamsshd, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(Lst[i] == ""){
			continue;
		}
		if( IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
			sshd_pamlastlog = "truefail";
		}
		else {
			if(IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*" )){
				sshd_pamlastlog = "true";
			}
		}
		if(IsMatchRegexp( Lst[i], "^.*session.*required.*pam_limits.so.*" )){
			sshd_pamlimits = "true";
	...
	if(!sshd_pamlastlog){
		sshd_pamlastlog = "fail";
	}
	if(!sshd_pamlimits){
		sshd_pamlimits = "fail";
	}
	if(!sshd_pamtally){
		sshd_pamtally_data = "fail";
	}
	if(!sshd_pamtally){
		sshd_pamtally = "fail";
	}
...
if( pamgdm != "none" ){
	Lst = split( buffer: pamgdm, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(Lst[i] == ""){
			continue;
		}
		if( IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
			gdm_pamlastlog = "truefail";
		}
		else {
			if(IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*" )){
				gdm_pamlastlog = "true";
...
if( pamxdm != "none" ){
	Lst = split( buffer: pamxdm, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(Lst[i] == ""){
			continue;
		}
		if( IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
			xdm_pamlastlog = "truefail";
		}
		else {
			if(IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*" )){
				xdm_pamlastlog = "true";
...
if( pamkde != "none" ){
	Lst = split( buffer: pamkde, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(Lst[i] == ""){
			continue;
		}
		if( IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*showfailed.*" ) ) {
			kde_pamlastlog = "truefail";
		}
		else {
			if(IsMatchRegexp( Lst[i], "^.*session.*pam_lastlog.so.*" )){
				kde_pamlastlog = "true";
...
if( pampasswd != "none" ){
	Lst = split( buffer: pampasswd, keep: 0 );
	for(i = 0;i < max_index( Lst );i++){
		if(IsMatchRegexp( Lst[i], "^.*password.*required.*pam_cracklib.so.*" )){
			pam_cracklib += Lst[i] + "\n";
		}
		if(IsMatchRegexp( Lst[i], "^.*password.*required.*pam_unix.so.*" )){
			pam_unix += Lst[i] + "\n";
		}
		if(IsMatchRegexp( Lst[i], "^@include.*common-password.*" )){
			Lst1 = split( buffer: pamcompasswd, keep: 0 );
			for(j = 0;j < max_index( Lst1 );j++){
				if(IsMatchRegexp( Lst1[j], "^.*password.*required.*pam_cracklib.so.*" )){
					pam_cracklib += Lst1[j] + "\n";
				}
				if(IsMatchRegexp( Lst1[j], "^.*password.*required.*pam_unix.so.*" )){
					pam_unix += Lst1[j] + "\n";
				}
			}
		}
