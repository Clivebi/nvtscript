...
if(!windirpath || ContainsString( windirpath, "error" ) || ContainsString( windirpath, "none" )){
...
metabase = ReadAndX( socket: soc, uid: uid, tid: tid, fid: fid, count: size, off: 0 );
if( !metabase ){
	AspEnableParentPaths = "error";
	log_message( port: port, data: "Cannot access/open the IIS Metabase file." );
}
else {
	if( egrep( pattern: "AspEnableParentPaths=.TRUE.", string: metabase ) ){
