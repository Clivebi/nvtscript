if(description){
	...
	script_dependencies( "GSHB/GSHB_WMI_OSInfo.sc", "GSHB/EL15/GSHB_M4_097.sc", "gb_ms_ie_detect.sc", "lsc_options.sc" );
...
if(!ContainsString( tolower( Windows_OSName ), "windows server 2012" )){
	for(i = 1;i <= 14;i++){
...
if( res == "" ){
	...
	for line in res {
		...
		if(IsMatchRegexp( line[5], "S-1-5-21-[0-9,-]+-(500|512|544)$" )){
...
GROUPUSER = split( buffer: GROUPUSER, sep: "\n", keep: FALSE );
for USER in GROUPUSER {
	USER = split( buffer: USER, sep: "|", keep: FALSE );
	if(ContainsString( USER[1], "Win32_UserAccount" )){
		Group = eregmatch( string: USER[0], pattern: "Name=\".+\"" );
		if(!ContainsString( Group, "Administrator" ) && !ContainsString( Group, "Guest" ) && !ContainsString( Group, "Protected User" )){
			Name = eregmatch( string: USER[1], pattern: "Name=\".+\"" );
			Name = split( buffer: Name, sep: "\"", keep: FALSE );
			NotValidUser += Name[1] + "\n";
		}
	}
	if(ContainsString( USER[1], "Win32_SystemAccount" )){
		Group = eregmatch( string: USER[0], pattern: "Name=\".+\"" );
		if(ContainsString( Group, "Protected User" )){
			Name = eregmatch( string: USER[1], pattern: "Name=\".+\"" );
			Name = split( buffer: Name, sep: "\"", keep: FALSE );
...
if( ContainsString( res, "not recognized" ) || disabled_win_cmd_exec ){
	...
	for line in res {
		if(ContainsString( line, "3" )){
			...
			if( ContainsString( BitLocker, "not recognized" ) ){
				continue;
			}
			else {
				BitLockerInstalled = TRUE;
				BitLocker = split( buffer: BitLocker, keep: FALSE );
				for INFO in BitLocker {
					if(ContainsString( tolower( INFO ), "protection status" )){
						if(!ContainsString( tolower( INFO ), "protection on" )){
