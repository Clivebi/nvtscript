...
if(ContainsString( logindefs, "grep: /etc/login.defs:" )){
	logindefs = "nologindefs";
}
if(ContainsString( logindefs, "Permission denied" ) || ContainsString( logindefs, "Keine Berechtigung" )){
	logindefs = "nopermission";
}
if(!logindefs){
	logindefs = "nologindefs";
}
if( !ContainsString( logindefs, "nologindefs" ) && !ContainsString( logindefs, "nopermission" ) ){
	logindefs = toupper( logindefs );
	Lst = split( logindefs );
	for(i = 0;i < max_index( Lst );i++){
		if(ContainsString( Lst[i], "PASS_MAX_DAYS" )){
			PASS_MAX_DAYS = Lst[i];
		}
		if(ContainsString( Lst[i], "PASS_MIN_DAYS" )){
			PASS_MIN_DAYS = Lst[i];
		}
		if(ContainsString( Lst[i], "PASS_WARN_AGE" )){
			PASS_WARN_AGE = Lst[i];
		}
	}
	if( PASS_MAX_DAYS ){
...
if(ContainsString( shadow, "cat: /etc/aliases:" )){
	shadow = "noshadow";
}
if(ContainsString( shadow, "Permission denied" ) || ContainsString( shadow, "Keine Berechtigung" )){
	shadow = "nopermission";
}
if(!shadow){
	shadow = "noshadow";
}
if( !ContainsString( shadow, "noshadow" ) && !ContainsString( shadow, "nopermission" ) ){
	Lst = split( shadow );
	for(i = 0;i < max_index( Lst );i++){
		UserLst = split( buffer: Lst[i], sep: ":", keep: 0 );
		if(UserLst[1] != "*" && UserLst[1] != "!" && UserLst[1] != "*LK*" && UserLst[1] != "NP" && UserLst[4] > 90){
			PWChange += UserLst[0] + ":" + UserLst[4] + "\n";
		}
		if( UserLst[1] == "!" || UserLst[1] == "NP" ) {
			NoPWUser += UserLst[0] + "\n";
		}
		else {
			if( UserLst[1] == "*" || UserLst[1] == "*LK*" ) {
				continue;
			}
			else {
	...
	if(!IsMatchRegexp( uname, "SunOS .*" )){
		...
		for(i = 0;i < max_index( Lst );i++){
			UserLst = split( buffer: Lst[i], sep: " ", keep: 0 );
			if(UserLst[1] != "L" && UserLst[1] != "NP" && UserLst[4] > 90){
				PWChange += UserLst[0] + ":" + UserLst[4] + "\n";
			}
			if( UserLst[1] == "NP" ) {
				NoPWUser += UserLst[0] + "\n";
				...
				if( UserLst[1] == "L" ) {
					...
					userlistdate = split( buffer: UserLst[2], sep: "/", keep: 0 );
	...
	if(IsMatchRegexp( uname, "SunOS .*" )){
		userlist = ssh_cmd( socket: sock, cmd: "passwd -sa" );
		if( IsMatchRegexp( userlist, ".*ermission denied.*" ) ) {
			sunpasswd = "noperm";
		}
		else {
			sunpasswd = "ok";
			Lst = split( userlist );
			for(i = 0;i < max_index( Lst );i++){
				if( IsMatchRegexp( Lst[i], "^.*[ ]{1,}PS[ ]{1,}" ) ){
					PS_Lst = split( buffer: Lst[i], sep: " ", keep: 0 );
					val = ssh_cmd( socket: sock, cmd: "logins -x -l " + PS_Lst[0] + " | grep PS" );
					date_val = split( buffer: val, sep: "PS", keep: 0 );
					date_val = date_val[1];
					...
					if(IsMatchRegexp( Lst[i], "^.*[ ]{1,}NP[ ]{1,}" )){
						NP_Lst = split( buffer: Lst[i], sep: " ", keep: 0 );
						NoPWUser += NP_Lst[0] + "\n";
					}
				}
			}
...
if(ContainsString( shadow, "noshadow" ) || ContainsString( shadow, "nopermission" )){
	set_kb_item( name: "GSHB/etc_shadow", value: shadow );
}
