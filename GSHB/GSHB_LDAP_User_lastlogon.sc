if(description){
	...
	script_dependencies( "GSHB/GSHB_WMI_OSInfo.sc", "toolcheck.sc" );
...
if(!DomFunkMod || DomFunkMod == "none" || int( DomFunkMod ) < 3){
	...
	for(m = 0;m < max_index( split_res );m++){
		if(IsMatchRegexp( split_res[m], "msDS-Behavior-Version: [0-9]{1}" )){
			DomFunkMod = split_res[m] - "msDS-Behavior-Version: ";
		}
	}
	if(DomFunkMod && DomFunkMod != "none"){
		DomFunkMod = ereg_replace( pattern: "\n", replace: "", string: DomFunkMod );
	}
	if(DomFunkMod && DomFunkMod != "none"){
		set_kb_item( name: "GSHB/LDAP/DomFunkMod", value: DomFunkMod );
	}
}
...
for(i = 0;i < max_index( split_res );i++){
	user = split( buffer: split_res[i], sep: ",", keep: 0 );
	user = user[0];
	val = split( split_res[i] );
	for(a = 0;a < max_index( val );a++){
		if(IsMatchRegexp( val[a], "lastLogonTimestamp: [0-9]{1,}" )){
			TS = val[a] - "lastLogonTimestamp: ";
		}
	}
	if(TS && TS != "none"){
		Userlist += user + "," + ereg_replace( pattern: "\n", replace: "", string: TS ) + ";";
	}
	TS = "none";
}
if(!Userlist){
