if(description){
	...
	script_dependencies( "GSHB/GSHB_WMI_OSInfo.sc", "GSHB/GSHB_WMI_Antivir.sc", "GSHB/GSHB_SMB_UAC_Config.sc", "GSHB/GSHB_WMI_EFS.sc" );
...
if(Windows_Version != "6.3" || !ContainsString( tolower( Windows_Name ), "windows 8.1" )){
	for(i = 1;i <= 21;i++){
...
if( IsMatchRegexp( tolower( Win_OSArchitecture ), "64.*bit" ) ){
...
if( AuditPolicy ){
	AuditPolicy = split( buffer: AuditPolicy, keep: FALSE );
	desc = "Folgende Überwachungsrichtlinien sind auf dem Host aktiviert:\n";
	for line in AuditPolicy {
		line = split( buffer: line, sep: "|", keep: FALSE );
...
if( UserPrivilegeRight ){
	UserPrivilegeRight = split( buffer: UserPrivilegeRight, keep: FALSE );
	desc += "Folgende Benutzerrechte sind zugewiesen:\n";
	for line in UserPrivilegeRight {
		line = split( buffer: line, sep: "|", keep: FALSE );
