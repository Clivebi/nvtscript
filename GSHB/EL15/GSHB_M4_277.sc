...
if( WMIOSLOG == "On the Target System runs Samba, it is not an Microsoft System." ){
	...
	if( ContainsString( CPSGENERAL, "error" ) ){
		...
		if( !CPSGENERAL ){
			...
			if( OSTYPE < 2 ){
				result = NASLString( "nicht zutreffend" );
				desc = NASLString( "Das System ist kein Windows Server." );
			}
			else {
				if( NTLMMinServerSec == "537395248" && requiresignorseal == "1" && requirestrongkey == "1" && RequireSecuritySignatureWs == "1" && EnablePlainTextPassword == "0" && RequireSecuritySignatureSvr == "1" && EnableSecuritySignatureSvr == "1" && NoLMHash == "1" && lmcomplevel >= "5" && LDAPClientIntegrity == "1" && NTLMMinClientSec == "537395248" ){
...
if(!silence){
	itg_send_details( itg_id: "GSHB/M4_277" );
}
exit( 0 );
