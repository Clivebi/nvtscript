...
if(description){
	...
	for d in depend {
		script_dependencies( "GSHB/EL15/GSHB_" + d + ".sc" );
	}
...
for m in massnahmen {
	...
	if(!result){
		if( IsMatchRegexp( name, "M[45]\\.... Diese Maßnahme ist entfallen!" ) ) {
			result = "Diese Maßnahme ist entfallen.";
		}
		else {
			if( !ContainsString( depend, m ) ) {
			...
			if( ContainsString( "erfüllt", result ) ) {
				...
				if( ContainsString( "nicht zutreffend", result ) ) {
					...
					if( ContainsString( "nicht erfüllt", result ) ) {
						...
						if( ContainsString( "unvollständig", result ) ) {
							result = "NC";
						}
						else {
							if( ContainsString( "Prüfung dieser Maßnahme ist nicht implementierbar.", result ) ) {
								result = "NA";
							}
							else {
	...
	log += NASLString( "\"" + ip + "\"|\"" + ml + "\"|\"" + result + "\"|\"" + log_desc + "\"" ) + "\n";
}
