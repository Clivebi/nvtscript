		...
		if( OSVER < "5.1" ){
			...
			if( FW == "on" ){
				...
				if( OSVER == "5.2" && OSTYPE != "none" && OSTYPE > 1 ){
					...
					if( IPFilter == "True" ){
						result = NASLString( "erfüllt" );
						desc = NASLString( "Auf dem System ist die Windows Firewall für folgende Netzwerkkarte aktiviert: " + NWCard );
					}
					else {
					...
					if( Firewall_State != "none" && Firewall_State != "inapplicable" ){
						...
						if( Firewall_State == "266256" ){
							result = NASLString( "erfüllt" );
							desc = NASLString( "Auf dem System ist folgende Firewall Software aktiviert: " + Firewall_Name );
						}
						else {
...
if(!silence){
	itg_send_details( itg_id: "GSHB/M4_238" );
}
exit( 0 );
