...
if( !ContainsString( W2SVCPAR, "RDSServer.DataFactory" ) ) {
	set_kb_item( name: "WMI/RDSServer.DataFactory", value: "off" );
}
else {
	set_kb_item( name: "WMI/RDSServer.DataFactory", value: "on" );
}
if( !ContainsString( W2SVCPAR, "AdvancedDataFactory" ) ) {
	set_kb_item( name: "WMI/AdvancedDataFactory", value: "off" );
}
else {
	set_kb_item( name: "WMI/AdvancedDataFactory", value: "on" );
}
if( !ContainsString( W2SVCPAR, "VbBusObj.VbBusObjCls" ) ) {
	set_kb_item( name: "WMI/VbBusObj.VbBusObjCls", value: "off" );
}
else {
	set_kb_item( name: "WMI/VbBusObj.VbBusObjCls", value: "on" );
}
wmi_close( wmi_handle: handle );
exit( 0 );
