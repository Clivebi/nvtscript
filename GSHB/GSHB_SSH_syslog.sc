...
if(IsMatchRegexp( var_log, ".*such.file.*directory.*" )){
	var_log = "none";
}
if(IsMatchRegexp( var_adm, ".*such.file.*directory.*" )){
	var_adm = "none";
}
if(IsMatchRegexp( syslog_r, ".*such.file.*directory.*" )){
	syslog_r = "none";
}
if(IsMatchRegexp( rsyslog_r, ".*such.file.*directory.*" )){
	rsyslog_r = "none";
}
if( IsMatchRegexp( syslog, ".*such.file.*directory.*" ) ) {
	syslog = "off";
}
else {
	syslog = "running";
}
if( IsMatchRegexp( rsyslog, ".*such.file.*directory.*" ) ) {
	rsyslog = "off";
}
else {
	rsyslog = "running";
...
if(var_log != "none"){
	Lst = split( buffer: var_log, sep: " ", keep: 0 );
	var_log = Lst[0] + ":" + Lst[2] + ":" + Lst[3];
}
if(var_adm != "none"){
	Lst = split( buffer: var_adm, sep: " ", keep: 0 );
	var_adm = Lst[0] + ":" + Lst[2] + ":" + Lst[3];
}
if(syslog_r != "none"){
	Lst = split( buffer: syslog_r, sep: " ", keep: 0 );
	syslog_r = Lst[0] + ":" + Lst[2] + ":" + Lst[3];
}
if(rsyslog_r != "none"){
	Lst = split( buffer: rsyslog_r, sep: " ", keep: 0 );
	rsyslog_r = Lst[0] + ":" + Lst[2] + ":" + Lst[3];
}
if(IsMatchRegexp( syslog, ".*Permission denied.*" )){
	syslog = "norights";
}
if(IsMatchRegexp( syslog, ".*such.file.*directory.*" )){
	syslog = "none";
}
if(IsMatchRegexp( rsyslog, ".*Permission denied.*" )){
	rsyslog = "norights";
}
if(IsMatchRegexp( rsyslog, ".*such.file.*directory.*" )){
	rsyslog = "none";
