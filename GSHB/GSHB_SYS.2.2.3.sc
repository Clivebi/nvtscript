if(description){
	...
	script_dependencies( "GSHB/GSHB_WMI_OSInfo.sc", "GSHB/GSHB_WMI_Antivir.sc", "GSHB/GSHB_SMB_UAC_Config.sc", "GSHB/GSHB_WMI_EFS.sc", "lsc_options.sc" );
...
if(Windows_Version != "10" && !ContainsString( tolower( Windows_Name ), "windows 10" )){
	for(i = 1;i <= 25;i++){
...
SecurityCenter2 = split( buffer: SecurityCenter2, keep: FALSE );
if( max_index( SecurityCenter2 ) <= 1 ){
	...
	for line in SecurityCenter2 {
		...
		if( ProtectionStatus == "00" || ProtectionStatus == "01" ){
			...
			if( ProtectionStatus == "10" || ProtectionStatus == "11" ){
...
if( !SeDenyInteractiveLogonRight ){
	...
	SeDenyInteractiveLogonRight = split( buffer: SeDenyInteractiveLogonRight, keep: FALSE );
	desc += "Folgenden Benutzern wird die lokale Anmeldung verweigert (GPO \"Lokale Anmeldung Verweigern\"):\n";
	for line in SeDenyInteractiveLogonRight {
		line = split( buffer: line, sep: "|", keep: FALSE );
...
if( !SeInteractiveLogonRight ){
	...
	SeInteractiveLogonRight = split( buffer: SeInteractiveLogonRight, keep: FALSE );
	desc += "Folgenden Benutzern wird die lokale Anmeldung zugelassen (GPO \"Lokale Anmeldung Zulassen\"):\n";
	for line in SeInteractiveLogonRight {
		line = split( buffer: line, sep: "|", keep: FALSE );
		if(tolower( line[0] ) == "accountlist"){
			continue;
		}
...
if(!disabled_win_cmd_exec){
	...
	if(res){
		res = split( buffer: res, keep: FALSE );
		for line in res {
...
result = "erfüllt";
if( ContainsString( tolower( Windows_Name ), "enterprise" ) ){
	...
	if( RequirePlatformSecFeatures == "3" ){
		...
		if(RequirePlatformSecFeatures == "1"){
...
if( MaxTicketExpiryUnits == "0" ){
	...
	if( MaxTicketExpiryUnits == "1" ){
		Unit = " Stunden.";
	}
	else {
		if(MaxTicketExpiryUnits == "2"){
			Unit = " Tage.";
		}
	}
}
...
if( !RDP_User ){
	...
	if( ContainsString( RDP_User, "is not recognized as an internal or external command" ) ){
		desc = "Die Mitglieder der Gruppe \"Remote Desktop Users\" konnten nicht ausgelesen werden.\n";
		result = "error";
	}
	else {
		desc = "Folgende Benutzer sind Mitglieder der Gruppe \"Remote Desktop Users\":\n";
		RDP_User = split( buffer: RDP_User, keep: FALSE );
		for line in RDP_User {
...
if( !Administrators ){
	...
	if( ContainsString( Administrators, "is not recognized as an internal or external command" ) ){
		desc += "Die Mitglieder der Gruppe \"Administrators\" konnten nicht ausgelesen werden.\n";
		result = "error";
	}
	else {
		desc += "Folgende Benutzer sind Mitglieder der Gruppe \"Administrators\" und haben somit einen Remote-Desktopzugriff:\n";
		Administrators = split( buffer: Administrators, keep: FALSE );
