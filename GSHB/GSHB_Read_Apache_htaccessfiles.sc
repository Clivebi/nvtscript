if(description){
	...
	script_dependencies( "smb_reg_service_pack.sc", "GSHB/GSHB_WMI_Apache.sc" );
...
if(ContainsString( htaccessList, "None" )){
...
if( htaccessList ){
	htaccessList = split( buffer: htaccessList, sep: "|", keep: FALSE );
	for(h = 0;h < max_index( htaccessList );h++){
		if(!ContainsString( "Name", htaccessList[h] ) || ""){
			path = htaccessList[h];
