...
if(TCPMaxCon != "0"){
	TCPMaxCon = hex2dec( xvalue: TCPMaxCon );
}
...
if(Backlog != "0"){
	Backlog = hex2dec( xvalue: Backlog );
}
...
if(DynDelta != "0"){
	DynDelta = hex2dec( xvalue: DynDelta );
}
...
if(TcpMaxPortsExhausted != "0"){
	TcpMaxPortsExhausted = hex2dec( xvalue: TcpMaxPortsExhausted );
}
val = "None";
if(!TCPMaxCon){
	TCPMaxCon = val;
...
if(!TcpMaxPortsExhausted){
	TcpMaxPortsExhausted = val;
}
...
if(IISSynAttackval != "None"){
	set_kb_item( name: "WMI/IISSynAttack", value: "tested" );
}
wmi_close( wmi_handle: handle );
wmi_close( wmi_handle: handlereg );
exit( 0 );
