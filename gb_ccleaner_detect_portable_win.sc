if(description){
	...
	script_dependencies( "gb_ccleaner_detect_win.sc", "gb_wmi_access.sc", "lsc_options.sc" );
	script_mandatory_keys( "win/lsc/search_portable_apps", "WMI/access_successful" );
	script_exclude_keys( "win/lsc/disable_wmi_search" );
	script_tag( name: "summary", value: "SMB login and WMI file search based detection of CCLeaner Portable." );
...
for filePath in keys( fileList ) {
	...
	if(vers && version = eregmatch( string: vers, pattern: "^([0-9]+\\.[0-9]+\\.[0-9]+)" )){
		...
		if( ContainsString( location, "ccleaner64" ) ){
