...
func getVersion( data, port, proto ){
	...
	if(!data || !ContainsString( tolower( data ), "nominum vantio" )){
		return;
	}
	version = "unknown";
	...
	log_message( data: build_detection_report( app: "Nominum Vantio", version: version, install: port + "/" + proto, cpe: cpe, concluded: data ), port: port, proto: proto );
}
