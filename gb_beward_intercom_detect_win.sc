if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of BEWARD Intercom." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "BEWARD Intercom" )){
			continue;
		}
		version = "unknown";
		concluded = appName;
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:beward:intercom:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
