if(description){
	...
	script_dependencies( "gb_open_udp_ports.sc", "global_settings.sc" );
...
req = NASLString( "M-SEARCH * HTTP/1.1\\r\\n", "HOST: 239.255.255.250:", port, "\\r\\n", "MAN: \"ssdp:discover\"", "\\r\\n", "MX: 10\\r\\n", "ST: ssdp:all\\r\\n\\r\\n" );
len = strlen( req ) + 8;
...
ret = FALSE;
for(;!ret && attempt--;){
	ret = send_packet( packet: udp, pcap_active: TRUE, pcap_filter: filter );
	len = strlen( ret );
	if(len > 28){
		x = ord( ret[0] );
		if(( x & 0xF0 ) == 0x40){
...
if(result && ContainsString( result, "HTTP/" )){
