if(description){
	...
	script_tag( name: "severity_date", value: "2020-08-04 18:03:00 +0000 (Tue, 04 Aug 2020)" );
	...
	script_xref( name: "Advisory-ID", value: "EulerOS-SA-2019-1476" );
	...
	script_tag( name: "summary", value: "The remote host is missing an update for the Huawei EulerOS 'kernel' package(s) announced via the EulerOS-SA-2019-1476 advisory." );
...
An issue was discovered in the Linux kernel through 4.17.2. vbg_misc_device_ioctl() in drivers/virt/vboxguest/vboxguest_linux.c reads the same user data twice with copy_from_user. The header part ... [Please see the references for more information on the vulnerabilities]" );
