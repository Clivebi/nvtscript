...
if(!banner || ( !ContainsString( banner, "Warning: Telnet is not a secure protocol, and it is recommended to use Stelnet." ) && ( !ContainsString( banner, "Login authentication" ) || !ContainsString( banner, "Username:" ) ) )){
	exit( 0 );
}
for cred in creds {
	...
	if(!banner || !ContainsString( banner, "Username:" )){
	...
	sleep( 3 );
	res = recv( socket: soc, length: 128 );
	if(!res || ContainsString( res, "Error: Authentication fail" )){
		telnet_close_socket( socket: soc, data: res );
