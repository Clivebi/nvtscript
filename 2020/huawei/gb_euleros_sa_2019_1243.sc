if(description){
	...
	script_tag( name: "summary", value: "The remote host is missing an update for the Huawei EulerOS 'rsyslog' package(s) announced via the EulerOS-SA-2019-1243 advisory." );
...
if(release == "EULEROSVIRT-2.5.3"){
	if(!isnull( res = isrpmvuln( pkg: "rsyslog", rpm: "rsyslog~8.24.0~12.h18.x86_64", rls: "EULEROSVIRT-2.5.3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "rsyslog-mmjsonparse", rpm: "rsyslog-mmjsonparse~8.24.0~12.h18.x86_64", rls: "EULEROSVIRT-2.5.3" ) )){
		report += res;
	}
	if( report != "" ){
