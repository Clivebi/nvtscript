...
if(description){
	...
	script_tag( name: "severity_date", value: "2020-11-13 20:15:00 +0000 (Fri, 13 Nov 2020)" );
...
for url in urls {
	...
	if(ContainsString( res, "[PWN]" )){
		dbname = eregmatch( pattern: "\\[PWN\\]([^[]+)\\[PWN\\]", string: res );
		if(isnull( dbname[1] )){
			continue;
		}
		report = "It was possible to obtain the database name through an SQL injection at " + http_report_vuln_url( port: port, url: url, url_only: TRUE ) + "\n\nResult:\n\n" + dbname[1];
