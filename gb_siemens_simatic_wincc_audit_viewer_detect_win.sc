...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "SIMATIC WinCC/Audit Viewer [0-9A-Z]+" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "Siemens AG " + appName, ver: version, concluded: concluded, base: "cpe:/a:siemens:simatic_wincc_audit_viewer:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
