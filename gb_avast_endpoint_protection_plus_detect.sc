...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( avastName, "avast! Endpoint Protection Plus" )){
		...
		if(avastVer){
			...
			log_message( data: build_detection_report( app: "Avast Endpoint Protection Plus", version: avastVer, install: avastPath, cpe: cpe, concluded: avastVer ) );
			exit( 0 );
		}
	}
}
