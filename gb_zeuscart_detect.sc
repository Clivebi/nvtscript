...
for dir in nasl_make_list_unique( "/Zeuscart", "/zeuscart", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "target=\"_blank\">ZeusCart</a>", string: res, icase: TRUE )){
		...
		log_message( data: build_detection_report( app: "ZeusCart", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
	}
}
exit( 0 );
