...
paths = ssh_find_file( file_name: "/sqlite3?$", sock: sock, useregex: TRUE );
for bin in paths {
	bin = chomp( bin );
	if(!bin){
		continue;
	...
	if(ver[1]){
		...
		register_and_report_cpe( app: "SQLite", ver: version, concluded: ver[0], base: CPE, expr: "([0-9.]+)", insloc: bin, regPort: 0, regService: "ssh-login" );
	}
}
