...
if(!registry_key_exists( key: "SOFTWARE\\Hewlett Packard Enterprise\\Sizers\\HPE Synergy Planning Tool" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\Hewlett Packard Enterprise\\Sizers\\HPE Synergy Planning Tool" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HPE Synergy Planning Tool" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HPE Synergy Planning Tool", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
