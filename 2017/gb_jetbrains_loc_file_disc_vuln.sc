...
if(description){
  ...
  arbitrary files on the target system." );
	...
	script_dependencies( "gb_jetbrains_ide_detection.sc", "os_detection.sc" );
...
func guessProjectName( port ){
	var port;
	var dictionary, name, url, req, res;
...
func leakWithPyCharmHelpers( homePath, port, check_files ){
	var homePath, port, check_files;
	var projectName, projectPath, url, data, req, res, dotSegs, pattern, file, report;
	projectName = "helpers";
	...
	for pattern in keys( check_files ) {
...
func leakWithProject( name, port, check_files ){
	var name, port, check_files;
	var dotSegs, i, pattern, file, url, report;
	dotSegs = "";
	for(i = 1;i < 5;i++){
		dotSegs += "..%2f";
		for pattern in keys( check_files ) {
