if(description){
	...
	script_dependencies( "gb_cambium_networks_services_server_detect.sc" );
...
files = make_array( "/httpd.conf", "ServerRoot", "/windows/apache2/conf/server.key", "BEGIN PRIVATE KEY", "/windows/apache2/conf/server.pem", "BEGIN PRIVATE KEY", "/windows/apache2/conf/httpd.conf", "ServerRoot", "/stack/php/php.ini", "About php.ini", "/windows/php/php.ini", "About php.ini", "/stack/postgresql/data/ph_hba.conf", "PostgreSQL Client Authentication Configuration File", "/stack/postgresql/data/postgresql.conf", "PostgreSQL configuration file", "/logs", "Index of /logs", "/scripts/cnss_test_users.sql", "INSERT INTO", "/scripts/cnss_seed_users.sql", "INSERT INTO" );
file_list = "";
for file in keys( files ) {
	res = http_get_cache( port: port, item: dir + file );
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, files[file] )){
		VULN = TRUE;
		file_list += file + "\r\n";
	}
}
