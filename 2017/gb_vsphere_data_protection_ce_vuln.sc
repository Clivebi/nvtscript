...
if( IsMatchRegexp( Ver, "^5\\.5\\." ) ){
	VULN = TRUE;
	fix = "6.0.5";
}
else {
	if( IsMatchRegexp( Ver, "^5\\.8\\." ) ){
		VULN = TRUE;
		fix = "6.0.5";
	}
	else {
		if( ( IsMatchRegexp( Ver, "^6\\.1\\." ) ) && ( version_is_less( version: Ver, test_version: "6.1.4" ) ) ){
			VULN = TRUE;
			fix = "6.1.4";
		}
		else {
			if(( IsMatchRegexp( Ver, "^6\\.0\\." ) ) && ( version_is_less( version: Ver, test_version: "6.0.5" ) )){
				VULN = TRUE;
				fix = "6.0.5";
			}
		}
