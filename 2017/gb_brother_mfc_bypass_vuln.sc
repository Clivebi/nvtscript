...
if(!IsMatchRegexp( recv, "^HTTP/1\\.[01] 200" ) || ContainsString( recv, "Please configure the password" )){
	exit( 0 );
}
...
recv = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
if(( IsMatchRegexp( recv, "^HTTP/1\\.[01] 301" ) || IsMatchRegexp( recv, "^HTTP/1\\.[01] 200" ) ) && ( ContainsString( recv, "<title>Brother" ) )){
...
recv = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
if(IsMatchRegexp( recv, "^HTTP/1\\.[01] 200" ) && ContainsString( recv, "<title>Brother" )){
