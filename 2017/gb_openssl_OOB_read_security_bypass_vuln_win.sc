...
vers = infos["version"];
path = infos["location"];
if( IsMatchRegexp( vers, "^1\\.1\\.0" ) && version_is_less_equal( version: vers, test_version: "1.1.0f" ) ){
	fix = "1.1.0g-dev";
}
else {
	if( IsMatchRegexp( vers, "^1\\.0\\.2" ) && version_is_less_equal( version: vers, test_version: "1.0.2l" ) ){
		fix = "1.0.2m-dev";
	}
	else {
		if(IsMatchRegexp( vers, "^1\\.0\\.1" ) || IsMatchRegexp( vers, "^1\\.0\\.0" ) || IsMatchRegexp( vers, "^0\\.9\\.8" )){
