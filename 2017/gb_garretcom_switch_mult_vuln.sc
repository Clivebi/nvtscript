...
if(description){
	...
	script_dependencies( "gb_garrettcom_switch_detect.sc" );
...
if(!model || !IsMatchRegexp( model, "(6k|10k)" )){
	exit( 0 );
}
...
if(http_vuln_check( port: port, url: url, pattern: "<sysversion val=", check_header: TRUE, extra_check: "<serialnumber val=" )){
	report = "It was possible to access the device information page without proper authentication at " + http_report_vuln_url( port: port, url: url, url_only: TRUE );
