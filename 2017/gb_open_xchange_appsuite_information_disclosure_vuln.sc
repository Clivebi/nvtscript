...
if(oxRev){
	oxVer = oxVer + "." + oxRev;
	if( IsMatchRegexp( oxVer, "^(7\\.8\\.1)" ) && version_is_less( version: oxVer, test_version: "7.8.1.8" ) ){
		fix = "7.8.1-rev8";
		VULN = TRUE;
	}
	else {
		if( IsMatchRegexp( oxVer, "^(7\\.6\\.2)" ) && version_is_less( version: oxVer, test_version: "7.6.2.14" ) ){
			fix = "7.6.2-rev14";
			VULN = TRUE;
		}
		else {
			if( IsMatchRegexp( oxVer, "^(7\\.6\\.3)" ) && version_is_less( version: oxVer, test_version: "7.6.3.3" ) ){
				fix = "7.6.3-rev3";
				VULN = TRUE;
			}
			else {
				if(IsMatchRegexp( oxVer, "^(7\\.8\\.0)" ) && version_is_less( version: oxVer, test_version: "7.8.0.7" )){
					fix = "7.8.0-rev7";
					VULN = TRUE;
				}
			}
