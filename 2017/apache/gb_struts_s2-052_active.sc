...
if(description){
	...
	script_dependencies( "gb_apache_struts_consolidation.sc", "global_settings.sc", "os_detection.sc" );
...
len = strlen( data );
url = dir + "/orders/3";
req = http_post_put_req( port: port, url: url, data: data, add_headers: make_array( "Content-Type", "application/xml" ) );
