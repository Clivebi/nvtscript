...
if(description){
	...
	script_dependencies( "gb_apache_http_server_consolidation.sc", "no404.sc", "webmirror.sc", "DDI_Directory_Scanner.sc" );
  ...
  As a workaround the usage of .htaccess should be disabled competely via the
...
for dir in nasl_make_list_unique( "/", http_cgi_dirs( port: port ) ) {
	...
	for(i = 0;i <= 100;i++){
		...
		if(IsMatchRegexp( res, "^HTTP/1\\.[01] 405" )){
			break;
		}
