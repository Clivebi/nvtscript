...
req = http_post_put_req( port: port, url: url, data: soap, add_headers: make_array( "SoapAction", "\"\"", "Content-Type", "text/xml; charset=UTF-8" ) );
res = http_keepalive_send_recv( port: port, data: req );
hostname = eregmatch( pattern: "<ns3:hostname xmlns:ns3=\"http://xml.apache.org/axis/\">([^<]+)", string: res );
...
for pattern in keys( files ) {
	...
	req = http_post_put_req( port: port, url: url, data: soap, add_headers: make_array( "SoapAction", "\"\"", "Content-Type", "text/xml; charset=UTF-8" ) );
	res = http_keepalive_send_recv( port: port, data: req );
	if(ContainsString( res, "boundary=" ) && ContainsString( res, "<getFileInternalReturn href=\"cid:" )){
