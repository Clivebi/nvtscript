...
if(description){
  ...
  This list may not be accurate and/or complete!" );
	...
	script_dependencies( "gb_linksys_devices_consolidation.sc" );
...
postData = "submit_type=&submit_button=UnsecuredEnable&gui_action=Apply" + "&wait_time=19&next_url=www.example.com&change_action=";
url = "/UnsecuredEnable.cgi";
req = http_post_put_req( port: port, url: url, data: postData );
res = http_keepalive_send_recv( port: port, data: req );
if(res && IsMatchRegexp( res, "^HTTP/1\\.[01] 302" ) && IsMatchRegexp( res, "Location.*http://www\\.example\\.com" )){
