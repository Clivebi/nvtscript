...
extensions = script_get_preference( name: "Backup File Extensions", id: 1 );
if( strlen( extensions ) > 0 ){
	extensions = str_replace( string: extensions, find: " ", replace: "" );
	extensions = split( buffer: extensions, sep: ",", keep: FALSE );
	extensions = nasl_make_list_unique( extensions );
}
else {
	extensions = make_list( ".backup",
		 ".bak",
		 ".copy",
		 ".bkp",
		 ".old",
...
default_php_files = make_list( "/index.php",
	 "/wp-config.php",
	 "/configuration.php",
	 "/config.php",
	 "/config.inc.php",
...
dirs = nasl_make_list_unique( "/", http_cgi_dirs( port: port, host: host ) );
for dir in dirs {
	if(dir == "/"){
		dir = "";
	}
	for default_php_file in default_php_files {
		file_list = make_list( file_list,
			 dir + default_php_file );
	}
}
...
file_list = nasl_make_list_unique( file_list );
for file in file_list {
	...
	check_info_split = split( buffer: check_info, sep: "#-----#", keep: FALSE );
	...
	for ext in extensions {
		...
		path = split( buffer: file, sep: "/", keep: TRUE );
