...
for dir in nasl_make_list_unique( "/serendipity", "/", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) && ( egrep( pattern: "Powered.by.*(Serendipity|>s9y</a>)", string: buf, icase: TRUE ) || ContainsString( buf, "<meta name=\"generator\" content=\"Serendipity" ) )){
		version = "unknown";
		...
		if( isnull( vers[1] ) ){
			...
			if(ContainsString( buf, "Powered by Serendipity" )){
		...
		log_message( data: build_detection_report( app: "Serendipity", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
	}
}
exit( 0 );
