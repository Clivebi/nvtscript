...
snagPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows\\CurrentVersion" + "\\App Paths\\SnagIt32.exe", item: "Path" );
if(!snagPath){
	exit( 0 );
}
share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: snagPath );
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: snagPath + "\\SnagIt32.exe" );
