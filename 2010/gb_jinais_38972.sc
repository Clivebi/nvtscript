if(description){
	...
	script_dependencies( "find_service.sc", "ircd.sc" );
...
send( socket: soc, data: NASLString( "NICK ", NICK, "\\r\\n" ) );
buf = recv( socket: soc, length: 256 );
if(isnull( buf )){
	close( soc );
	exit( 0 );
}
send( socket: soc, data: NASLString( "USER ", NICK, "\\r\\n" ) );
buf = recv( socket: soc, length: 1024 );
if(!ContainsString( buf, NICK )){
	close( soc );
	exit( 0 );
}
send( socket: soc, data: NASLString( "TOPIC #", NICK, "\\r\\n" ) );
buf = recv( socket: soc, length: 256 );
close( soc );
