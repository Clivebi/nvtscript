...
func Get_dllversion( path, dllfile ){
	share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: path );
	file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: path + dllfile );
...
if(sysPath){
	...
	if(sysVer1 && sysVer2){
		if( hotfix_check_sp( winVista: 2 ) > 0 ){
			...
			if(ContainsString( SP, "Service Pack 1" )){
				if(version_is_less( version: sysVer1, test_version: "6.0.6001.18498" ) || version_is_less( version: sysVer2, test_version: "6.0.6001.18470" )){
					security_message( port: 0, data: "The target host was found to be vulnerable" );
					exit( 0 );
				}
			}
	...
	if(sysVer3 && sysVer1){
		if(hotfix_check_sp( win7: 1 ) > 0){
			if(version_is_less( version: sysVer1, test_version: "6.1.7600.16624" ) || version_is_less( version: sysVer3, test_version: "7.0.7600.16587" )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
				exit( 0 );
			}
		}
