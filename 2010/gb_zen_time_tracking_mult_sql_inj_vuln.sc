...
for path in nasl_make_list_unique( "/", "/ZenTimeTracking", "/zentimetracking", http_cgi_dirs( port: port ) ) {
	if(path == "/"){
		path = "";
	}
	res = http_get_cache( item: path + "/index.php", port: port );
	if(ContainsString( res, "Zen Time Tracking" )){
		useragent = http_get_user_agent();
		filename = NASLString( path + "/managerlogin.php" );
		...
		if(ContainsString( res2, "Create Group" ) && ContainsString( res2, "Assign Group" ) && ContainsString( res2, "Log Off" )){
