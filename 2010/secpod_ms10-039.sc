...
if(( hotfix_missing( name: "980923" ) == 0 ) && ( hotfix_missing( name: "979441" ) == 0 ) && ( hotfix_missing( name: "979445" ) == 0 )){
	exit( 0 );
}
...
for i in list {
	...
	if(exePath){
		...
		if(exeVer){
			if(version_in_range( version: exeVer, test_version: "11.0", test_version2: "11.0.8232.0" ) || version_in_range( version: exeVer, test_version: "12.0", test_version2: "12.0.6529.4999" )){
				security_message( port: 0, data: "The target host was found to be vulnerable" );
				exit( 0 );
			}
		}
