...
if(!banner || !ContainsString( banner, "Core FTP" )){
	exit( 0 );
}
...
if(login_details){
	...
	if(ftpPort2){
		...
		if(soc2){
			...
			if(!ContainsString( result, "250 CWD command successful" )){
				exit( 0 );
			}
			send( socket: soc1, data: NASLString( "LIST\\r\\n" ) );
	...
	if(result && ContainsString( result, "CONFIG.SYS" )){
