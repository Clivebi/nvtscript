...
if(!banner || !ContainsString( banner, "Server: Sun-" )){
	exit( 0 );
}
host = http_host_name( port: jswsPort );
for files in make_list( "login.jsp",
	 "index.jsp",
	 "default.jsp",
	 "admin.jsp" ) {
	url = NASLString( "/", files, "?ref=http://", host, "/%0D%0AContent-type:+text/html;%0D%0A%0D%0ATEST%3Cscript%3Ealert(111)%3C/script%3E" );
	req = http_get( item: url, port: jswsPort );
	resp = http_send_recv( port: jswsPort, data: req );
	if(egrep( string: resp, pattern: "^HTTP/1\\.[01] 200" ) && ( ContainsString( resp, "TEST<script>alert(111)</script>" ) )){
