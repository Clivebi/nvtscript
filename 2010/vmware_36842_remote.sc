if(description){
	...
	script_dependencies( "gb_vmware_esx_web_detect.sc", "gb_vmware_esx_snmp_detect.sc", "os_detection.sc", "global_settings.sc" );
	script_require_ports( "Services/www", 8222 );
	script_mandatory_keys( "VMware/ESX/installed", "Host/runs_unixoide" );
...
if( ContainsString( res, "VMware ESX" ) ){
	path = "/sdk/%2E%2E/%2E%2E/%2E%2E/%2E%2E/%2E%2E/%2E%2E/";
}
else {
	if( ContainsString( res, "<title>VMware Server" ) ){
		path = "/sdk/../../../../../../";
	}
	else {
		exit( 0 );
...
if(ContainsString( buf, "Location: https://" )){
	exit( 0 );
}
...
for pattern in keys( files ) {
	...
	req += NASLString( "TE: deflate,gzip;q=0.3\\r\\nConnection: TE, close\\r\\n" );
