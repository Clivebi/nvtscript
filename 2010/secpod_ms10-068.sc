...
if(( hotfix_missing( name: "981550" ) == 1 ) && registry_key_exists( key: "SYSTEM\\CurrentControlSet\\Services\\NTDS\\Performance" )){
...
if(( hotfix_missing( name: "982000)" ) == 1 ) && registry_key_exists( key: "SYSTEM\\CurrentControlSet\\Services\\ADAM\\Linkage" )){
	dllPath = registry_get_sz( key: "SOFTWARE\\Microsoft\\COM3\\Setup", item: "Install Path" );
	if(dllPath != NULL){
		share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: dllPath );
...
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: sysPath + "\\system32\\Ntdsai.dll" );
