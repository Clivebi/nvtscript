...
ex = NASLString( "<body onload=alert('vt-xss-test')>" );
url = dir + "/util/icon_browser.php?subdir=" + urlencode( str: ex ) + "&app=horde";
if(http_vuln_check( port: port, url: url, pattern: "<body onload=alert\\('vt-xss-test'\\)>. not found", extra_check: "Subdirectory", check_header: TRUE )){
