if(description){
	...
	script_dependencies( "secpod_nakid_cms_detect.sc" );
...
if(!isnull( ncVer[2] )){
	url = NASLString( ncVer[2], "/modules/catalog/upload_photo.php?core[system_path]=VT_RFI.php" );
	sndReq = http_get( item: url, port: ncPort );
	rcvRes = http_send_recv( port: ncPort, data: sndReq );
	if(ContainsString( rcvRes, "VT_RFI.php" ) && ContainsString( rcvRes, "failed to open stream" )){
