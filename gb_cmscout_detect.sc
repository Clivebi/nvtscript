...
for dir in nasl_make_list_unique( "/", "/cmscout", "/cmscout", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ( ContainsString( res, ":: CMScout ::" ) || ContainsString( res, ">Welcome to CMScout" ) )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "CMScout", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
