...
port = http_get_port( default: 88 );
url = "/lg/ENU.xml";
res = http_get_cache( port: port, item: url );
if(ContainsString( res, "\"IDS_DEV_NAME\">Camera Model</string>" ) && ContainsString( res, "\"IDS_HEARD_LIVE\">Live Video</string>" )){
	version = "unknown";
	set_kb_item( name: "foscam/ip_camera/detected", value: TRUE );
	cpe = "cpe:/h:foscam:ip_camera:";
	conclUrl = http_report_vuln_url( port: port, url: url, url_only: TRUE );
	register_and_report_cpe( app: "Foscam IP Camera", ver: version, base: cpe, expr: "^([0-9.]+)", insloc: "/", regPort: port, regService: "www", conclUrl: conclUrl );
}
exit( 0 );
