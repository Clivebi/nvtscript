...
baseVers = make_list( "1.0",
	 "2.0",
	 "3.0" );
for basever in baseVers {
	scom_newkey = scomKey + basever + "\\Setup";
	scomName = registry_get_sz( key: scom_newkey, item: "Product" );
	if(!scomName){
		continue;
	}
	if(ContainsString( scomName, "System Center Operations Manager" )){
		...
		if(scomVer){
			...
			cpe = build_cpe( value: scomVer, exp: "^([0-9.]+)", base: "cpe:/a:microsoft:system_center_operations_manager:" );
			if(!cpe){
				cpe = "cpe:/a:microsoft:system_center_operations_manager";
			}
			register_product( cpe: cpe, location: scomPath );
			log_message( data: build_detection_report( app: scomName, version: scomVer, install: scomPath, cpe: cpe, concluded: scomDetails ) );
			exit( 0 );
		}
	}
}
