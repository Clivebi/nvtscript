...
for dir in nasl_make_list_unique( "/spip", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( buf, "Composed-By: SPIP" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "SPIP", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
	}
}
exit( 0 );
