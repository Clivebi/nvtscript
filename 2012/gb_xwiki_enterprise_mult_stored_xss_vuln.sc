...
tokenValue = eregmatch( pattern: "name=\"form_token\" value=\"([a-zA-Z0-9]+)\"", string: rcvRes );
if(!tokenValue || !tokenValue[1]){
	exit( 0 );
}
postdata = "form_token=" + tokenValue[1] + "&parent=xwiki%3AMain.UserDirectory&" + "register_first_name=" + xss + "&" + "register_last_name=&" + "xwikiname=ThisUserNameDefinitelyNotExists&" + "register_password=password&" + "register2_password=password&" + "register_email=&" + "template=XWiki.XWikiUserTemplate&" + "xredirect=";
...
if(res){
	...
	if(http_vuln_check( port: xwikiPort, url: url, check_header: TRUE, pattern: "<img src='1.jpg'onerror=javascript:alert\\(0\\)>" )){
