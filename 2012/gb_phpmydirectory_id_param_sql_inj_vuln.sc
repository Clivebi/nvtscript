...
for dir in nasl_make_list_unique( "/", "/phpMyDirectory", "/phpmydirectory", "/pmd", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	url = dir + "/index.php";
	...
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, ">phpMyDirectory.com<" )){
		url = dir + "/page.php?id='";
