...
for dir in nasl_make_list_unique( "/moniwiki", "/MoniWiki", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: NASLString( dir, "/wiki.php" ), port: port );
	if(rcvRes && ContainsString( rcvRes, "powered by MoniWiki" )){
		postdata = "action=userform&login_id=<script>alert(document.cookie)" + "</script>&password=<script>alert(document.cookie)</script>";
		monReq = NASLString( "POST ", dir, "/wiki.php/FrontPage HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
		monRes = http_keepalive_send_recv( port: port, data: monReq );
		if(IsMatchRegexp( monRes, "^HTTP/1\\.[01] 200" ) && ContainsString( monRes, "<script>alert(document.cookie)</script>" )){
