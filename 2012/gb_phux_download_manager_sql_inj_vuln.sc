...
for dir in nasl_make_list_unique( "/", "/download_manager", "/phux", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(!isnull( rcvRes ) && ContainsString( rcvRes, ">phux.org's<" ) && ContainsString( rcvRes, "Public Download Center<" )){
		url = dir + "/download.php?file='";
		if(http_vuln_check( port: port, url: url, pattern: "mysql_num_rows\\(\\): " + "supplied argument is not a valid MySQL" )){
