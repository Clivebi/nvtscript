...
for dir in nasl_make_list_unique( "/", "/manageengine", http_cgi_dirs( port: port ) ) {
	...
	if(ContainsString( res, "manageengine" ) && ContainsString( res, "<title>Applications Manager Login Screen</title>" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "ZOHO ManageEngine Applications Manager", version: version, install: install, cpe: cpe, concluded: concluded ), port: port );
		exit( 0 );
	}
}
exit( 0 );
