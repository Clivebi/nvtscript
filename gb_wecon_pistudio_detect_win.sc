...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "PIStudio" )){
			continue;
		}
		location = "unknown";
		version = "unknown";
		...
		register_and_report_cpe( app: "WECON " + appName, ver: version, concluded: concluded, base: "cpe:/a:we-con:pi_studio:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
