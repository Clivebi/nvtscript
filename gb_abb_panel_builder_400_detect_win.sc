if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of ABB Automation Panel Builder
  400." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Panel Builder 400" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "ABB Automation " + appName, ver: version, concluded: concluded, base: "cpe:/a:abb:panel_builder_400:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
