if(description){
	...
	script_dependencies( "gb_gwsecurity_ip_camera_detect.sc", "gb_default_credentials_options.sc" );
...
creds = make_array( "888888", "admin", "admin", "admin" );
url = "/ISAPI/Security/userCheck";
for cred in keys( creds ) {
	username = creds[cred];
	password = cred;
	auth_cookie = "Basic " + base64( str: username + ":" + password );
	req = http_get_req( port: port, url: url, add_headers: make_array( "Authorization", auth_cookie ) );
	res = http_keepalive_send_recv( port: port, data: req );
	if(ContainsString( res, "<statusString>OK</statusString>" )){
		VULN = TRUE;
		if(!password){
