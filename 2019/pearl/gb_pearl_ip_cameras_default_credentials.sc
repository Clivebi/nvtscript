if(description){
	...
	script_dependencies( "gb_pearl_ip_cameras_detect.sc", "gb_default_credentials_options.sc" );
...
if( res && IsMatchRegexp( res, "^HTTP/1\\.[01] 401" ) ){
	...
	for username in keys( creds ) {
		...
		if(res && ( ContainsString( res, "General.Network.PPPoE.Enabled&group=" ) && ContainsString( res, "var onloadFun" ) ) || ContainsString( res, "General.Network.PPPoE.Enable" )){
			VULN = TRUE;
			report += "\n" + username + ":" + password;
		}
	}
