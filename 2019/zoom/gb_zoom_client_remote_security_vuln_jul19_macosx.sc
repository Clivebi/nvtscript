...
if(description){
	...
	script_name( "Zoom Client Remote Security Vulnerability (ZSB-19002) - Mac OS X" );
	script_tag( name: "summary", value: "Zoom Client is prone to a remote security vulnerability." );
	script_tag( name: "vuldetect", value: "Checks if a vulnerable version is present on the target host." );
	script_tag( name: "insight", value: "The flaw exists due to a local web server which insecurely
  receives commands without validation and lets any website to interact with it." );
	script_tag( name: "impact", value: "Successful exploitation will allow remote attackers to bypass
  security restrictions and perform unauthorized actions which may aid in further attacks." );
	script_tag( name: "affected", value: "Zoom Client version 4.4.4 and prior on Mac OS X." );
