...
path = infos["location"];
if( IsMatchRegexp( coreVers, "^1\\.0" ) && version_is_less( version: coreVers, test_version: "1.0.14" ) ){
	fix = "1.0.14";
}
else {
	if( IsMatchRegexp( coreVers, "^1\\.1" ) && version_is_less( version: coreVers, test_version: "1.1.11" ) ){
		fix = "1.1.11";
	}
	else {
		if( IsMatchRegexp( coreVers, "^2\\.1" ) && version_is_less( version: coreVers, test_version: "2.1.8" ) ){
			fix = "2.1.8";
		}
		else {
			if(IsMatchRegexp( coreVers, "^2\\.2" ) && version_is_less( version: coreVers, test_version: "2.2.2" )){
				fix = "2.2.2";
			}
		}
	}
