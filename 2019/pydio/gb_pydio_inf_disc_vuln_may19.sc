...
vuln_url = "";
if(IsMatchRegexp( buf, "\"ajxpVersion\":" ) && IsMatchRegexp( buf, "HTTP/[0-9]([.][0-9]+)? 200" )){
	proto = "GET";
	vuln_url = url;
}
if(!proto){
	url = location + "/index.php";
	req = http_post_put_req( port: port, url: url, data: "get_action=display_doc&doc_file=CREDITS&secure_token=", add_headers: make_array( "Content-Type", "application/x-www-form-urlencoded" ) );
	buf = http_keepalive_send_recv( port: port, data: req );
	if(IsMatchRegexp( buf, ">PHP Libraries</div>" ) && IsMatchRegexp( buf, "HTTP/[0-9]([.][0-9]+)? 200" )){
		proto = "POST";
		vuln_url = url;
	}
}
if(proto){
	report = "It was possible to access sensitive information with a crafted HTTP " + proto + " request to: " + http_report_vuln_url( port: port, url: vuln_url, url_only: TRUE );
