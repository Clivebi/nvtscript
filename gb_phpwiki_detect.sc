...
for dir in nasl_make_list_unique( "/", "/phpwiki", "/wiki", http_cgi_dirs( port: port ) ) {
	...
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "content=\"PhpWiki" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "PhpWiki", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
