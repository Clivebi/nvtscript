...
if(!ContainsString( tolower( buf ), "<model>telepresence server" )){
	exit( 0 );
}
...
if(!isnull( version[1] )){
	vers = version[1];
	cpe += ":" + vers;
	set_kb_item( name: "cisco_telepresence_server/version", value: vers );
}
...
if(!isnull( _model[2] )){
	model = _model[2];
	if(ContainsString( model, "Virtual Machine" )){
		model = "VM";
	}
...
log_message( data: build_detection_report( app: "Cisco Telepresence Server (" + model + ")", version: vers, install: "/", cpe: cpe, concluded: version[0] ), port: port );
exit( 0 );
