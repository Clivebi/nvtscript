if(description){
	...
	script_dependencies( "vrp_display_ospf_peer_brief.sc" );
...
interfaces_dict = make_array( "Ana", "Analogmodem", "Asy", "Async", "Dia", "dialer", "Eth", "Ethernet", "GE", "GigabitEthernet", "H", "HSI", "IMA", "IMA-Group", "Log", "Logic-Channel", "Loop", "loopback", "MTun", "MTunnel", "S", "serial", "Tun", "tunnel", "VE", "virtual Ethernet", "VT", "Virtual-Template" );
...
if( get_kb_item( "Policy/vrp/installed/ERROR" ) ){
	...
	if( get_kb_item( "Policy/vrp/ssh/ERROR" ) ){
		...
		if( get_kb_item( "Policy/vrp/ospf/peer/brief/empty" ) ){
			...
			for interface in interfaces {
				...
				ret = ssh_cmd( socket: sock, cmd: cmd1, return_errors: FALSE, pty: TRUE, nosh: TRUE, timeout: 20, retry: 10, force_reconnect: TRUE, clear_buffer: TRUE );
				if(!IsMatchRegexp( ret, "ospf authentication-mode" ) || IsMatchRegexp( ret, "ospf authentication-mode md5" )){
