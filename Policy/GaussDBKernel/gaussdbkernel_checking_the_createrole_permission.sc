if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150448" );
	...
	script_name( "GaussDB Kernel: Checking the CREATEROLE Permission" );
	...
	script_tag( name: "summary", value: "Only the role with the CREATEROLE permission can create, modify, or delete
other roles. Such a role can also grant permissions to or revoke permissions of
other roles. To avoid arbitrary permission granting, delete roles that do not require
the CREATEROLE permission." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT rolname FROM pg_roles WHERE rolcreaterole=true AND rolsuper=false;";
title = "Checking the CREATEROLE Permission";
