if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150446" );
	...
	script_name( "GaussDB Kernel: Deleting the Default Username" );
	...
	script_tag( name: "summary", value: "You are advised not to use a well-known username, for example, postgres." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT rolname FROM pg_roles WHERE rolname='postgres';";
title = "Deleting the Default Username";
solution = "Delete user postgres or replace it with another one.";
default = "N/A";
test_type = "SQL_Query";
