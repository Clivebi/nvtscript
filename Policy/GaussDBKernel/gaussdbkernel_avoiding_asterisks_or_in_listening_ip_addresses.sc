if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150418" );
	...
	script_name( "GaussDB Kernel: Avoiding Asterisks (*) or 0.0.0.0 in Listening IP Addresses" );
	...
	script_tag( name: "summary", value: "Listening IP addresses must not contain asterisks (*) or 0.0.0.0 because an asterisk
or 0.0.0.0 indicates that all available IP addresses will be listened on." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'listen_addresses' AND (setting = '*' OR setting = '0.0.0.0');";
title = "Avoiding Asterisks (*) or 0.0.0.0 in Listening IP Addresses";
solution = "Set listen_addresses to localhost in the postgresql.conf file and then restart the
Database.";
default = "none";
test_type = "SQL_Query";
