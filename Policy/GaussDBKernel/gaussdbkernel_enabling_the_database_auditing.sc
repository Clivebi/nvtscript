if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150468" );
	...
	script_name( "GaussDB Kernel: Enabling the Database Auditing" );
	...
	script_tag( name: "summary", value: "After a security event occurs, audit logs are used to trace the event, locate faults,
and clarify responsibilities. You are advised to set audit_enabled to on." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_enabled';";
title = "Enabling the Database Auditing";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_enabled = on'";
default = "on";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_enabled\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
