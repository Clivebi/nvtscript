if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150479" );
	...
	script_name( "GaussDB Kernel: Configuring the Minimum Audit Log File Retention Period" );
	...
	script_tag( name: "summary", value: "The parameter audit_file_remain_time specifies the minimum period for storing
audit logs." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_file_remain_time';";
title = "Configuring the Minimum Audit Log File Retention Period";
solution = "Set the value of audit_file_remain_time to 90.
gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_file_remain_time = 90'";
default = "90";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_file_remain_time\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
