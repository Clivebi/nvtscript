if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150483" );
	...
	script_name( "GaussDB Kernel: Configuring Log File Permissions" );
	...
	script_tag( name: "summary", value: "After logging_collector is set to on, you can set log_file_mode to specify server
log file permissions. Access to log files must be restricted, because the log files
may contain sensitive data." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='log_file_mode' AND setting != '0600';";
title = "Configuring Log File Permissions";
solution = "Set log_file_mode to 0600 in the postgresql.conf file and restart the database.";
default = "none";
test_type = "SQL_Query";
