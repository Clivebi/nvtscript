if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150473" );
	...
	script_name( "GaussDB Kernel: Enabling the Auditing of Addition Deletion and Modification of Database Objects" );
	...
	script_tag( name: "summary", value: "The parameter audit_system_object specifies whether to audit the CREATE, DROP,
and ALTER operations on database objects, including DATABASE, USER, SCHEMA,
and TABLE. After this parameter is set to on, operations on these database objects
are traced." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_system_object';";
title = "Enabling the Auditing of Addition Deletion and Modification of Database Objects";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_system_object = 1229'";
default = "12295";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_system_object\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
