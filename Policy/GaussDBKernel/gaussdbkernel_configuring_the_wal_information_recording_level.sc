if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150494" );
	...
	script_name( "GaussDB Kernel: Configuring the WAL Information Recording Level" );
	...
	script_tag( name: "summary", value: "The parameter wal_level specifies the level of information to be written into a
...
replication. Set this parameter to archive at least." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='wal_level';";
title = "Configuring the WAL Information Recording Level";
solution = "Set wal_level to archive in the postgresql.conf file and restart the database.";
default = "archive or hot_standby";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "wal_level\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
				comment = "Can not determine setting.";
			}
			else {
				if( IsMatchRegexp( setting[1], "(archive|hot_standby)" ) ){
					compliant = "yes";
				}
				else {
					compliant = "no";
