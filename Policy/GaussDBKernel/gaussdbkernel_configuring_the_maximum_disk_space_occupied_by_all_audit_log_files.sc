if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150478" );
	...
	script_name( "GaussDB Kernel: Configuring the Maximum Disk Space Occupied by All Audit Log Files" );
	...
	script_tag( name: "summary", value: "The parameter audit_space_limit specifies the maximum disk space occupied by
all audit log files. When the total size of audit log files exceeds the maximum
...
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_space_limit';";
title = "Configuring the Maximum Disk Space Occupied by All Audit Log Files";
solution = "Set the value of audit_space_limit to 1048576. The unit is KB, that is, 1 GB.
gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_space_limit = 1048576'";
default = "1048576";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_space_limit\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
