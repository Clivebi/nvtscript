if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150484" );
	...
	script_name( "GaussDB Kernel: Disabling Log Overwriting" );
	...
	script_tag( name: "summary", value: "The parameter log_truncate_on_rotation specifies whether GaussDB Kernel
deletes or overwrites (rather than append) the existing log file with the same
Name." );
	exit( 0 );
}
...
solution = "Set log_truncate_on_rotation to off in the postgresql.conf file and restart the
Database.";
default = "none";
test_type = "SQL_Query";
