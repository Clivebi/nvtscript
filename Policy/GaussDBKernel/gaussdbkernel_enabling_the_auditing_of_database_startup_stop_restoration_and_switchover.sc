if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150470" );
	...
	script_name( "GaussDB Kernel: Enabling the Auditing of Database Startup Stop Restoration and Switchover" );
	...
	script_tag( name: "summary", value: "The parameter audit_database_process specifies whether to audit database
startup, stop, switchover, and restoration. After this parameter is set to on,
database running status is traced." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_database_process';";
title = "Enabling the Auditing of Database Startup Stop Restoration and Switchover";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_database_process = 1'";
default = "1";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_database_process\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
