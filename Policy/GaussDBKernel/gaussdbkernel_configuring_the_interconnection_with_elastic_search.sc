...
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( !elastic_search_ip_addr = policy_gsql_cmd( socket: sock, query: "SELECT name,setting FROM pg_settings WHERE name = 'elastic_search_ip_addr';", db_type: "gaussdbkernel" ) ){
			compliant = "incomplete";
			value = "error";
			comment = "No elastic_search_ip_addr found";
		}
		else {
			if( IsMatchRegexp( value, "failed to connect" ) || IsMatchRegexp( elastic_search_ip_addr, "failed to connect" ) ){
				compliant = "incomplete";
				value = "error";
