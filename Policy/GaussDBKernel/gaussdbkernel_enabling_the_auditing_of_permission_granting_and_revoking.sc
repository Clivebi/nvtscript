if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150472" );
	...
	script_name( "GaussDB Kernel: Enabling the Auditing of Permission Granting and Revoking" );
	...
	script_tag( name: "summary", value: "The parameter audit_grant_revoke specifies whether to audit the permissions
granted to and revoked from database users. After this parameter is set to on,
database account permissions are traced." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_grant_revoke';";
title = "Enabling the Auditing of Permission Granting and Revoking";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_grant_revoke = 1'";
default = "1";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_grant_revoke\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
