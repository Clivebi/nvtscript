if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150433" );
	...
	script_name( "GaussDB Kernel: Disabling the Permission to Copy Files on the Server" );
	...
	script_tag( name: "summary", value: "The enable_copy_server_files parameter allows users with the sysadmin
...
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'enable_copy_server_files';";
title = "Disabling the Permission to Copy Files on the Server";
solution = "None";
default = "off";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "enable_copy_server_files\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
