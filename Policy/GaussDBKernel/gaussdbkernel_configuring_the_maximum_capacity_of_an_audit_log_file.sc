if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150477" );
	...
	script_name( "GaussDB Kernel: Configuring the Maximum Capacity of an Audit Log File" );
	...
	script_tag( name: "summary", value: "The parameter audit_rotation_size specifies the maximum capacity of an audit
log file. When the size of an audit log file reaches the specified value, another
audit log file is automatically created." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_rotation_size';";
title = "Configuring the Maximum Capacity of an Audit Log File";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_rotation_size = 10240'";
default = "10240";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_rotation_size\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
