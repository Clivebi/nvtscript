if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150469" );
	...
	script_name( "GaussDB Kernel: Enabling the Auditing of Database Login and Logout" );
	...
	script_tag( name: "summary", value: "The parameter audit_login_logout specifies whether to audit user login and
logout. After this parameter is set to on, users logged in to the database and the
time when they logged out are traced." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_login_logout';";
title = "Enabling the Auditing of Database Login and Logout";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_login_logout = 7'";
default = "7";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_login_logout\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
