if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150457" );
	...
	script_name( "GaussDB Kernel: Configuring the Automatic Account Unlocking Time" );
	...
	script_tag( name: "summary", value: "Configure the automatic account unlocking time for locked abnormal accounts." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='password_lock_time';";
title = "Configuring the Automatic Account Unlocking Time";
solution = "gs_guc reload -Z coordinator -D /gaussdb/data/coordinator -c 'password_lock_time=1'";
default = "1";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "password_lock_time\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
