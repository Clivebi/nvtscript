if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150450" );
	...
	script_name( "GaussDB Kernel: Checking the OPRADMIN Permission" );
	...
	script_tag( name: "summary", value: "A role with the OPRADMIN permission can use Roach to perform backup and
restoration. To avoid arbitrary database file backup, delete roles that do not
require the OPRADMIN permission." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT rolname FROM pg_roles WHERE roloperatoradmin=true AND rolsuper=false;";
title = "Checking the OPRADMIN Permission";
