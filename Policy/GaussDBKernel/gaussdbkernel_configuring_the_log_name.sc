if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150482" );
	...
	script_name( "GaussDB Kernel: Configuring the Log Name" );
	...
	script_tag( name: "summary", value: "The parameter log_filename specifies the server run log file name." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='log_filename' AND setting NOT LIKE 'postgresql-%.log';";
title = "Configuring the Log Name";
solution = "Set log_filename to postgresql-%Y-%m-%d_%H%M%S.log in the
postgresql.conf file and restart the database.";
default = "none";
test_type = "SQL_Query";
