if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150451" );
	...
	script_name( "GaussDB Kernel: Checking the POLADMIN Permission" );
	...
	script_tag( name: "summary", value: "A role with the POLADMIN permission can create resource labels, anonymization
policies, and unified audit policies. To avoid arbitrary security policy creation,
delete roles that do not require the POLADMIN permission." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT rolname FROM pg_roles WHERE rolpolicyadmin=true AND rolsuper=false;";
title = "Checking the POLADMIN Permission";
