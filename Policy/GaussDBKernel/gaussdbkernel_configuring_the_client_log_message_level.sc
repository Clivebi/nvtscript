if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150487" );
	...
	script_name( "GaussDB Kernel: Configuring the Client Log Message Level" );
...
the client.
The valid values include DEBUG5, DEBUG4, DEBUG3, DEBUG2, DEBUG1, LOG,
NOTICE, WARNING, ERROR, FATAL, and PANIC. It must be NOTICE at least. After
a level is selected, messages at this level and its lower levels will be sent to the
client. That is, a lower level indicates fewer messages sent. If the configured level
is higher than ERROR, for example, FATAL or PANIC, the system changes the level
to ERROR by default." );
	exit( 0 );
}
...
solution = "Set client_min_messages to NOTICE in the postgresql.conf file and restart the
Database.";
default = "NOTICE or LOG";
test_type = "SQL_Query";
