if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150474" );
	...
	script_name( "GaussDB Kernel: Enabling the Auditing of the Query of Database Objects" );
	...
	script_tag( name: "summary", value: "The parameter audit_dml_state_select specifies whether to audit the SELECT
operation on database objects. After this parameter is set to on, the SELECT
operation on database objects is traced." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'audit_dml_state_select';";
title = "Enabling the Auditing of the Query of Database Objects";
solution = "gs_guc reload -Z coordinator -Z datanode -N all -I all -c 'audit_dml_state_select = 1'";
default = "1";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd, db_type: "gaussdbkernel" ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "audit_dml_state_select\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
