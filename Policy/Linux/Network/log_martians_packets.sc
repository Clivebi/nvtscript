...
if( get_kb_item( "Policy/linux/sysctl/conf/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ssh/ERROR" ) ){
	...
	if( get_kb_item( "Policy/linux//etc/sysctl.conf/content/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ERROR" ) ){
		...
		for content in content_list {
			if(IsMatchRegexp( content, "net\\.ipv4\\.conf\\.all\\.log_martians\\s*=\\s*1" ) || IsMatchRegexp( content, "net\\.ipv4\\.conf\\.default\\.log_martians\\s*=\\s*1" )){
				value = "Enabled";
			}
		}
		...
		if(net_ipv4_conf_all_log_martians != "0" || net_ipv4_conf_default_log_martians != "0"){
			value = "Enabled";
		}
		if(!value){
			value = "Disabled";
