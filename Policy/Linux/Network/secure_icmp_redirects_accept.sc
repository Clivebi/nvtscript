if(description){
	...
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 3.2.3 Ensure secure ICMP redirects are not accepted (Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 3.2.3 Ensure secure ICMP redirects are not accepted (Scored)" );
...
if( get_kb_item( "Policy/linux/sysctl/conf/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ssh/ERROR" ) ){
	...
	if( get_kb_item( "Policy/linux//etc/sysctl.conf/content/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ERROR" ) ){
		...
		for content in content_list {
			if(IsMatchRegexp( content, "net\\.ipv4\\.conf\\.all\\.secure_redirects\\s*=\\s*1" ) || IsMatchRegexp( content, "net\\.ipv4\\.conf\\.default\\.secure_redirects\\s*=\\s*1" )){
				value = "Enabled";
			}
		}
