if(description){
	...
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 3.2.6 Ensure bogus ICMP responses are ignored (Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 3.2.6 Ensure bogus ICMP responses are ignored (Scored)" );
...
solution = "Set the parameters in /etc/sysctl.conf or a /etc/sysctl.d/* file and run 'sysctl -w net.ipv4.icmp_ignore_bogus_error_responses = [0,1]'";
test_type = "SSH_Cmd";
...
if( get_kb_item( "Policy/linux/sysctl/conf/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ssh/ERROR" ) ){
	...
	if( get_kb_item( "Policy/linux//etc/sysctl.conf/content/ERROR" ) || get_kb_item( "Policy/linux/sysctl/ERROR" ) ){
		...
		for content in content_list {
			if(IsMatchRegexp( content, "net\\.ipv4\\.icmp_ignore_bogus_error_responses\\s*=\\s*1" )){
				value = "Enabled";
			}
		}
		net_ipv4_icmp_ignore_bogus_error_responses = get_kb_item( "Policy/linux/sysctl/net.ipv4.icmp_ignore_bogus_error_responses" );
		if(net_ipv4_icmp_ignore_bogus_error_responses != "0"){
			value = "Enabled";
		}
		if(!value){
			value = "Disabled";
