if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150561" );
	script_version( "2021-01-15T14:22:32+0000" );
	script_tag( name: "last_modification", value: "2021-01-15 14:22:32 +0000 (Fri, 15 Jan 2021)" );
	script_tag( name: "creation_date", value: "2021-01-15 13:47:26 +0000 (Fri, 15 Jan 2021)" );
	script_tag( name: "cvss_base", value: "0.0" );
	script_tag( name: "cvss_base_vector", value: "AV:L/AC:H/Au:S/C:N/I:N/A:N" );
	script_tag( name: "qod", value: "97" );
	script_name( "Linux: SSH AuthenticationMethods" );
...
AuthenticationMethods specifies the authentication methods that must be successfully completed for
a user to be granted access" );
	exit( 0 );
}
require("ssh_func.inc.sc");
require("policy_functions.inc.sc");
cmd = "grep '^AuthenticationMethods' /etc/ssh/sshd_config";
title = "SSH AuthenticationMethods";
...
if( get_kb_item( "Policy/linux/sshd_config/ERROR" ) ){
	...
	if( !value = get_kb_item( "Policy/linux/sshd_config/authenticationmethods" ) ){
		value = "Error";
		compliant = "incomplete";
		comment = "Could not get supported AuthenticationMethods from /etc/ssh/sshd_config";
	}
	else {
