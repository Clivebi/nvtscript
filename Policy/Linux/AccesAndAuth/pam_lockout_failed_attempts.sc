if(description){
	...
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 5.3.2 Ensure lockout for failed password attempts is configured (Not Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 5.4.2 Ensure lockout for failed password attempts is configured (Scored)" );
	script_xref( name: "Policy", value: "CIS Controls Version 7: 16.7 Establish Process for Revoking Access" );
...
if( get_kb_item( "Policy/linux/etc/pam.d/ERROR" ) ){
	...
	for file in files {
		...
		line = egrep( string: content, pattern: "auth\\s+required\\s+(pam_faillock.so|pam_tally2.so)" );
		if(!line){
			continue;
		}
		match = eregmatch( string: line, pattern: "deny=([0-9]*)" );
		if(match){
			if( value == "" ) {
				value = match[1];
			}
