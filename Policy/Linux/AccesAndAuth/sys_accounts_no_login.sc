if(description){
	...
	script_dependencies( "kb_set_uid_min_max.sc", "read_passwd_file.sc" );
...
if( get_kb_item( "Policy/linux//etc/login.defs/ERROR" ) || get_kb_item( "Policy/linux//etc/passwd/ERROR" ) ){
	...
	for line in split( buffer: content, keep: FALSE ) {
		...
		if(IsMatchRegexp( fields[0], "^(root|sync|shutdown|halt)$" ) || int( fields[2] ) > int( max )){
			continue;
		}
		if(!IsMatchRegexp( fields[6], "^(/sbin/nologin|/bin/false)$" )){
			value += ", " + fields[0];
		}
	}
	if( value ){
