if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.109811" );
	...
	script_name( "Linux: Access /etc/shadow" );
	...
	script_add_preference( name: "Permissions", type: "entry", value: "640", id: 3 );
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 6.1.3 Ensure permissions on /etc/shadow are configured (Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 6.1.3 Ensure permissions on /etc/shadow are configured (Scored)" );
	script_xref( name: "Policy", value: "CIS Controls Version 7: 16.4 Encrypt or Hash all Authentication Credentials" );
	script_tag( name: "summary", value: "The file '/etc/shadow' stores password for an user account in
encrypted format and other secure user account information. If an attacker gains access to the
'/etc/shadow' file, the user passwords can easily be cracked using password cracking programs.
This script checks if the given access rights are set on '/etc/shadow'." );
	exit( 0 );
}
...
if( get_kb_item( "policy/linux/access_permissions/error" ) ){
	...
	comment = "Could not read /etc/shadow";
}
else {
	uid = get_kb_item( "Policy/linux//etc/shadow/user" );
	gid = get_kb_item( "Policy/linux//etc/shadow/group" );
	perm = get_kb_item( "Policy/linux//etc/shadow/perm" );
	if( !uid || !gid || !perm ){
		value = "Error";
		compliant = "incomplete";
