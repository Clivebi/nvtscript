if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.109821" );
	script_version( "2021-05-18T12:49:08+0000" );
	script_tag( name: "last_modification", value: "2021-05-18 12:49:08 +0000 (Tue, 18 May 2021)" );
	script_tag( name: "creation_date", value: "2019-03-18 11:05:45 +0100 (Mon, 18 Mar 2019)" );
	script_tag( name: "cvss_base", value: "0.0" );
	script_tag( name: "cvss_base_vector", value: "AV:L/AC:H/Au:S/C:N/I:N/A:N" );
	script_tag( name: "qod", value: "97" );
	script_name( "Linux: SUID files" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2019 Greenbone Networks GmbH" );
	script_family( "Policy" );
	script_dependencies( "gather-package-list.sc", "compliance_tests.sc" );
	script_mandatory_keys( "Compliance/Launch" );
	script_exclude_keys( "ssh/no_linux_shell" );
	script_add_preference( name: "Files with SUID", type: "entry", value: "", id: 1 );
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 6.1.13 Audit SUID executables (Not Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 6.1.13 Audit SUID executables (Not Scored)" );
	script_xref( name: "Policy", value: "CIS Controls Version 7: 5.1 Establish Secure Configurations" );
	script_xref( name: "URL", value: "https://linoxide.com/how-tos/stickbit-suid-guid/" );
	script_tag( name: "summary", value: "When the SUID (set user ID) bit is set on an executable, it
executes with the UID of the owner. This may be intended for some executables. Add files with SUID
bit which should be allowed to have this bit set in the preference.

This script checks if any other local files than the given have the SUID bit set." );
	exit( 0 );
}
require("ssh_func.inc.sc");
require("policy_functions.inc.sc");
cmd = "df --local -P | awk '{if (NR!=1) print $6}' | xargs -I '{}' find '{}' -xdev -type f -perm -4000";
title = "SUID executables";
solution = "Ensure that no rogue SUID programs have been introduced into the system. Review the
files returned by the action in the Audit section and confirm the integrity of these binaries.";
test_type = "Manual Check";
default = "None";
compliant = "incomplete";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	value = "Error";
	comment = "No SSH connection";
}
else {
	if( !value = ssh_cmd( cmd: cmd, socket: sock, nosh: TRUE ) ){
		value = "None";
		comment = "Command did not return anything";
	}
	else {
		comment = "Please check the value manually";
	}
}
policy_reporting( result: value, default: default, compliant: compliant, fixtext: solution, type: test_type, test: cmd, info: comment );
policy_set_kbs( type: test_type, cmd: cmd, default: default, solution: solution, title: title, value: value, compliant: compliant );
exit( 0 );

