...
if( get_kb_item( "policy/linux/file_content/error" ) ){
	...
	if( !passwd = get_kb_item( "Policy/linux//etc/passwd/content" ) ){
		value = "Error";
		compliant = "incomplete";
		comment = "Could not get content of '/etc/passwd'";
	}
	else {
		if( !group = get_kb_item( "Policy/linux//etc/group/content" ) ){
			value = "Error";
			compliant = "incomplete";
			...
			for line in split( buffer: group, keep: FALSE ) {
				fields = split( buffer: line, sep: ":", keep: FALSE );
				if(!fields){
					continue;
				}
				group_gids = make_list( group_gids,
					 fields[2] );
			}
			for line in split( buffer: passwd, keep: FALSE ) {
				fields = split( buffer: line, sep: ":", keep: FALSE );
				if(!fields){
					continue;
				}
