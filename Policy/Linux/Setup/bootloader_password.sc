if(description){
	...
	script_dependencies( "read_boot_grub_config_files.sc" );
...
test_type = "SSH_Cmd";
default = script_get_preference( name: "Password protection", id: 1 );
	...
	if( get_kb_item( "Policy/linux//etc/sysconfig/init/content/ERROR" ) ){
		...
		for file in grub_config_files {
			...
			for line in split( grep ) {
				if(IsMatchRegexp( line, "^\\s*password --md5" ) || IsMatchRegexp( line, "^\\s*password_pbkdf2" ) || IsMatchRegexp( line, "^\\s*GRUB2_PASSWORD=" )){
					value = "Enabled";
				}
			}
		}
