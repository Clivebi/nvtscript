if(description){
	...
	script_dependencies( "read_etc_modprobe_files.sc", "read_lsmod_kernel_modules.sc" );
...
if( get_kb_item( "Policy/linux//etc/modprobe.d/ERROR" ) || get_kb_item( "Policy/linux/lsmod/ERROR" ) ){
	...
	if( get_kb_item( "Policy/linux//etc/modprobe.d/NO_BASH" ) ){
		value = "Error";
		compliant = "incomplete";
		...
		for file in get_kb_list( "Policy/linux//etc/modprobe.d" ) {
