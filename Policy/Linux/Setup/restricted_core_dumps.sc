if(description){
	...
	script_dependencies( "read_etc_security_limits.sc" );
	...
	script_xref( name: "Policy", value: "CIS Distribution Independent Linux v2.0.0: 1.5.1 Ensure core dumps are restricted (Scored)" );
	script_xref( name: "Policy", value: "CIS CentOS Linux 8 Benchmark v1.0.0: 1.6.1 Ensure core dumps are restricted (Scored)" );
...
if( get_kb_item( "Policy/linux/security/limits/conf/ERROR" ) ){
	...
	if( get_kb_item( "Policy/linux//etc/security/limits.conf/content/ERROR" ) ){
		value = "Error";
		compliant = "incomplete";
		...
		if(IsMatchRegexp( content, "\\*\\s+hard\\s+core\\s+0" )){
			value = "Enabled";
		}
		for file in get_kb_list( "Policy/linux//etc/security/limits.d//files/*" ) {
