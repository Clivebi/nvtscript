if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150693" );
	...
	script_name( "Huawei Data Communication: Configuring SNMP ACLs" );
	...
	script_tag( name: "summary", value: "Configure SNMP ACLs to defend against attacks." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "display current-configuration";
title = "Configuring SNMP ACLs";
solution = "Configure the SNMP ACL.";
test_type = "SSH_Cmd";
default = "snmp-agent acl command exists";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = ssh_cmd( socket: sock, cmd: cmd, return_errors: FALSE, pty: TRUE, nosh: TRUE, timeout: 20, retry: 10, force_reconnect: TRUE, clear_buffer: TRUE ) ){
		...
		if( IsMatchRegexp( value, "-----More----" ) ){
			...
			if( IsMatchRegexp( value, "snmp-agent\\s+acl" ) ){
				compliant = "yes";
			}
			else {
				compliant = "no";
