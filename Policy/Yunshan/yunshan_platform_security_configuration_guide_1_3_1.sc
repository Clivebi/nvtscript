if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150681" );
	...
	script_name( "Huawei Data Communication: The client ssl-verify peer is not configured on the HTTP client" );
	...
	script_tag( name: "summary", value: "Checks if setting 'client ssl-verify peer' exists." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "disp current-configuration configuration http";
title = "The client ssl-verify peer is not configured on the HTTP client";
solution = "Add this configuration.";
test_type = "SSH_Cmd";
default = "Check whether client ssl-verify peer exists.
If not, there is a risk.";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = ssh_cmd( socket: sock, cmd: cmd, return_errors: FALSE, pty: TRUE, nosh: TRUE, timeout: 20, retry: 10, force_reconnect: TRUE, clear_buffer: TRUE ) ){
		...
		if( IsMatchRegexp( value, "-----More----" ) ){
			...
			if( IsMatchRegexp( value, "client\\s+ssl-verify\\s+peer" ) || IsMatchRegexp( value, "^#\\s+(http\\s+#\\s+)?return" ) ){
				compliant = "yes";
			}
			else {
				compliant = "no";
