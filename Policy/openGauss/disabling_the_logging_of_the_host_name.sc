if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150400" );
	...
	script_name( "openGauss: Disabling the Logging of the Host Name" );
	...
	script_tag( name: "summary", value: "By default, the connection log message displays only the IP address of the
connected host. If log_hostname is set to on, the host name is also recorded. It
may take some time to parse the host name and affect database performance." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='log_hostname';";
title = "Disabling the Logging of the Host Name";
solution = "Set log_hostname to off in the postgresql.conf file and then restart the database.";
default = "off";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "log_hostname\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
				comment = "Can not determine setting.";
			}
			else {
				if( IsMatchRegexp( setting[1], "off" ) ){
					compliant = "yes";
				}
				else {
					compliant = "no";
