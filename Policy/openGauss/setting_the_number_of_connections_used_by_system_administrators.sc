if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150338" );
	...
	script_name( "openGauss: Setting the Number of Connections Used by System Administrators" );
...
reserved for the openGauss system administrator. The value of this parameter
...
solution = "Set sysadmin_reserved_connections to 3 in the postgresql.conf file and then restart the database.";
default = "1-3";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "sysadmin_reserved_connections\\s+\\|\\s+([0-9]+)" );
			if( !setting[1] ){
				compliant = "incomplete";
