if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150336" );
	...
	script_name( "openGauss: Configuring the Port" );
	...
	script_tag( name: "summary", value: "To prevent unauthorized connections to the database and illegal operations on
data, you need to change the default port number in time." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name = 'port' AND setting = '8000';";
title = "Configuring the Port";
solution = "Set port to any value except 8000 in the postgresql.conf file and then restart the database.";
default = "N/A";
test_type = "SQL_Query";
