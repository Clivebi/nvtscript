if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150337" );
	...
	script_name( "openGauss: Setting the Maximum Number of Concurrent Connections to the Database" );
...
Increasing the value of this parameter may cause openGauss to request System V
to share more memory or semaphore. If this occurs, the shared memory or
semaphore will exceed the default value allowed by the OS. When increasing the
value of this parameter, you need to adjust the value of max_pool_size to ensure
that the value of max_pool_size is greater than that of max_connections." );
	exit( 0 );
}
...
solution = "Set max_connections to 800 in the postgresql.conf file and then restart the database.";
default = "1-800";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "max_connections\\s+\\|\\s+([0-9]+)" );
			if( !setting[1] ){
				compliant = "incomplete";
