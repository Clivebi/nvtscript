if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150397" );
	...
	script_name( "openGauss: Enabling the Logging of User Login" );
	...
	script_tag( name: "summary", value: "The parameter log_connections specifies whether to record each connection
attempt and the successful client connection authentication in server logs." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='log_connections';";
title = "Enabling the Logging of User Login";
solution = "Set log_connections to on in the postgresql.conf file and then restart the database.";
default = "on";
test_type = "SQL_Query";
if( !get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection() ){
	...
	if( !value = policy_gsql_cmd( socket: sock, query: cmd ) ){
		...
		if( IsMatchRegexp( value, "failed to connect" ) ){
			...
			setting = eregmatch( string: value, pattern: "log_connections\\s+\\|\\s+(.+)" );
			if( !setting[1] ){
				compliant = "incomplete";
				comment = "Can not determine setting.";
			}
			else {
				if( IsMatchRegexp( setting[1], "on" ) ){
					compliant = "yes";
				}
				else {
					compliant = "no";
