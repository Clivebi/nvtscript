if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150356" );
	...
	script_name( "openGauss: Deleting the Default User Name" );
	...
	script_tag( name: "summary", value: "You are advised not to use a well-known user name, for example, postgres." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT rolname FROM pg_roles WHERE rolname='postgres';";
title = "Deleting the Default User Name";
