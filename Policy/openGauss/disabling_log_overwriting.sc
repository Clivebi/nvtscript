if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150391" );
	...
	script_name( "openGauss: Disabling Log Overwriting" );
	...
	script_tag( name: "summary", value: "The parameter log_truncate_on_rotation specifies whether openGauss deletes or
overwrites (rather than append) the existing log file with the same name." );
	exit( 0 );
}
require("policy_functions.inc.sc");
require("ssh_func.inc.sc");
cmd = "SELECT name,setting FROM pg_settings WHERE name='log_truncate_on_rotation' AND setting != 'off';";
title = "Disabling Log Overwriting";
solution = "Set log_truncate_on_rotation to off in the postgresql.conf file and restart the database.";
default = "N/A";
test_type = "SQL_Query";
