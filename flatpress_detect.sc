...
for dir in nasl_make_list_unique( "/flatpress", "/blog", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "powered by <a [^>]>FlatPress", string: buf, icase: TRUE ) || egrep( pattern: "meta name=\"generator\" content=\"FlatPress", string: buf, icase: TRUE )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "FlatPress", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
