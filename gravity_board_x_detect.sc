...
for dir in nasl_make_list_unique( "/gravity", "/forum", "/board", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: ".*Gravity Board X.*", string: buf, icase: TRUE ) && egrep( pattern: "Powered by", string: buf, icase: TRUE )){
		...
		cpe = build_cpe( value: vers, exp: "^([0-9.]+\\.[0-9])\\.?([a-z0-9]+)?", base: "cpe:/a:gravityboardx:gravity_board_x:" );
		if(!cpe){
			cpe = "cpe:/a:gravityboardx:gravity_board_x";
		}
		register_product( cpe: cpe, location: install, port: port, service: "www" );
		log_message( data: build_detection_report( app: "Gravity Board X", version: vers, install: install, concluded: version[0], cpe: cpe ), port: port );
		exit( 0 );
	}
}
exit( 0 );
