...
for location in nasl_make_list_unique( "/", http_cgi_dirs( port: port ) ) {
	...
	if(IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) && IsMatchRegexp( buf, "\"name\": *\"Home Assistant\"" )){
		set_kb_item( name: "home_assistant/detected", value: TRUE );
		set_kb_item( name: "home_assisant/port", value: port );
		register_and_report_cpe( app: "Home Assistant", ver: "unknown", base: CPE, expr: "([0-9.]+)", insloc: location, regPort: port, conclUrl: path );
		exit( 0 );
	}
}
exit( 0 );
