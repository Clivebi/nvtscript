if(description){
	...
	script_name( "Siemens Automation License Manager Detection (Windows SMB Login)" );
	...
	script_tag( name: "summary", value: "SMB login-based detection of Siemens Automation License Manager." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Siemens Automation License Manager V[0-9.]+" )){
			continue;
		}
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:siemens:automation_license_manager:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
