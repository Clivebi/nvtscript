...
for dir in nasl_make_list_unique( "/phpforum", "/forum", "/board", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "Powered by.*TinyPHPForum", string: buf )){
		...
		set_kb_item( name: NASLString( "www/", port, "/TinyPHPForum" ), value: tmp_version );
		...
		info = NASLString( "TinyPHPForum Version '" );
