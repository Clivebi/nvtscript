if(description){
	...
	script_tag( name: "last_modification", value: "2021-08-10 10:19:24 +0000 (Tue, 10 Aug 2021)" );
...
func set_mandatory_key( key, regex, banner, extra_key ){
	var key, regex, banner, extra_key;
	if(!key){
	...
	if(!banner){
		set_kb_item( name: "vt_debug_empty/" + get_script_oid(), value: get_script_oid() + "#-#banner#-#set_mandatory_key" );
		return;
	}
...
set_mandatory_key( key: "D-LinkDSL", regex: "^Server\\s*:\\s*(Boa|micro_httpd|Linux,|RomPager|uhttpd)", banner: banner );
set_mandatory_key( key: "D-LinkDWR", regex: "^Server\\s*:\\s*(GoAhead-Webs|server|Alpha_webserv|WebServer)", banner: banner );
