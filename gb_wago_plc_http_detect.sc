...
if(ContainsString( res, "title> WAGO Ethernet Web-Based Management" )){
	...
	if(!isnull( mac[1] )){
		macaddr = tolower( substr( mac[1], 0, 1 ) ) + ":" + tolower( substr( mac[1], 2, 3 ) ) + ":" + tolower( substr( mac[1], 4, 5 ) ) + ":" + tolower( substr( mac[1], 6, 7 ) ) + ":" + tolower( substr( mac[1], 8, 9 ) ) + ":" + tolower( substr( mac[1], 10, 11 ) );
...
if(ContainsString( res, "<title>WAGO Ethernet Web-based Management" )){
	...
	headers = make_array( "Content-Type", "application/x-www-form-urlencoded", "X-Requested-With", "XMLHttpRequest" );
	data = "{\"csrfToken\":false,\"renewSession\":true,\"aDeviceParams\":{\"0\":{\"name\":\"get_typelabel_value\"," + "\"parameter\":[\"SYSDESC\"],\"sudo\":true,\"multiline\":false,\"timeout\":12000,\"dataId\":0},\"1\":" + "{\"name\":\"get_typelabel_value\",\"parameter\":[\"ORDER\"],\"sudo\":true,\"multiline\":false,\"timeout\":12000," + "\"dataId\":0},\"2\":{\"name\":\"get_actual_eth_config\",\"parameter\":[\"X1\",\"mac-address\"],\"sudo\":true," + "\"multiline\":false,\"timeout\":12000,\"dataId\":0},\"3\":{\"name\":\"get_coupler_details\",\"parameter\":" + "[\"firmware-revision\"],\"sudo\":true,\"multiline\":false,\"timeout\":12000,\"dataId\":0}}}";
