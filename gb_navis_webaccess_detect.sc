...
if(ContainsString( res, "Confidential Information of Navis" ) && ContainsString( res, "accRequestEnter.do" )){
	version = "unknown";
	built_date = "unknown";
	req = http_get( port: port, item: "/express/about.jsp" );
	res = http_keepalive_send_recv( port: port, data: req );
	vers = eregmatch( pattern: "Version.*<blockquote>.*EXPRESS ([0-9._-]+) built ([A-Za-z0-9 ]+).*</blockquote", string: res );
	if(!isnull( vers[1] )){
		version = vers[1];
	}
	if(!isnull( vers[2] )){
	...
	log_message( data: build_detection_report( app: "Navis WebAccess", version: version, install: "/express", cpe: cpe, concluded: vers[0], extra: "Build date: " + built_date ), port: port );
	exit( 0 );
}
exit( 0 );
