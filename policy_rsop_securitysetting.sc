if(description){
	...
	script_dependencies( "smb_login.sc", "compliance_tests.sc" );
...
rsop_securitysetting_classes = make_list( "RSOP_SecuritySettingBoolean",
	 "RSOP_SecuritySettingString",
	 "RSOP_SecuritySettingNumeric" );
func wmic_set_kb( rsop_list, rsop_class ){
	for line in split( buffer: rsop_list, keep: FALSE ) {
		values = split( buffer: line, sep: "|", keep: FALSE );
		keyname = values[0];
		setting = values[2];
		if(IsMatchRegexp( keyname, "keyname" )){
			continue;
		}
		set_kb_item( name: "policy/rsop_securitysetting/" + tolower( rsop_class ) + "/" + tolower( keyname ), value: setting );
	}
}
func powershell_set_kb( rsop_list, rsop_class ){
	for line in split( buffer: rsop_list, keep: FALSE ) {
...
if( handle = wmi_connect( host: infos["host"], username: infos["username_wmi_smb"], password: infos["password"], ns: "root\\rsop\\computer" ) ){
	for rsop_securitysetting_class in rsop_securitysetting_classes {
		...
		if( !ret || ContainsString( ret, "NTSTATUS" ) ){
