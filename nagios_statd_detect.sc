...
for(;data = recv_line( socket: soc, length: 100 );){
	ret += data;
}
close( soc );
if(ContainsString( ret, "nagios-statd" )){
	...
	if(strlen( result )){
		info = NASLString( "Here are a few Information from the nagios-statd daemon received by the scanner:\\n\\n" );
		info += result;
	}
