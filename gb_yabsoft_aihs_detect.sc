...
for dir in nasl_make_list_unique( "/", "/aihs", "/gallery", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "yabsoft" ) && ContainsString( rcvRes, "AIH" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "YABSoft Advanced Image Hosting Script (AIHS)", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
