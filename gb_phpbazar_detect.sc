...
for dir in nasl_make_list_unique( "/", "/phpBazar", "/PHPBazar", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "Welcome to phpBazar!" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "phpBazar", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
