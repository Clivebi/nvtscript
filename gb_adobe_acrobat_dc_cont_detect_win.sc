...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(ContainsString( adobeName, "Adobe Acrobat DC" ) && ContainsString( adobePath, "Acrobat DC" )){
			...
			if(adobeVer){
				...
				log_message( data: build_detection_report( app: "Adobe Acrobat DC (Continuous Track)", version: adobeVer, install: adobePath, cpe: cpe, concluded: adobeVer ) );
				exit( 0 );
			}
		}
	}
