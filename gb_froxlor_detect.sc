...
for dir in nasl_make_list_unique( "/froxlor", http_cgi_dirs( port: port ) ) {
	rep_dir = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( res, "Froxlor Server Management Panel" ) && ContainsString( res, ">the Froxlor Team</a>" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "Froxlor", version: vers, install: rep_dir, cpe: cpe ), port: port );
	}
}
exit( 0 );
