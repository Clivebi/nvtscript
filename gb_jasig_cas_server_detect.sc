...
for dir in nasl_make_list_unique( "/", "/cas", "/cas-server-webapp", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/login", port: port );
	if(IsMatchRegexp( res, "Powered by.*>Jasig Central Authentication Service" ) && ContainsString( res, "login" ) && ContainsString( res, "id=\"cas" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Jasig Central Authentication Service Server", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
