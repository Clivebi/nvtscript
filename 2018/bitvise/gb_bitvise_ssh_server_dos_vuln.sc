...
if(description){
  ...
  later. Please see the references for more information." );
	...
	script_dependencies( "gb_bitvise_ssh_server_detect.sc" );
...
if(!infos = get_app_version_and_location( cpe: CPE, port: sshport, exit_no_version: TRUE )){
	exit( 0 );
}
vers = infos["version"];
path = infos["location"];
if( IsMatchRegexp( vers, "^6\\." ) && version_is_less( version: vers, test_version: "6.51" ) ){
	fix = "6.51";
}
else {
	if(IsMatchRegexp( vers, "^7\\." ) && version_is_less( version: vers, test_version: "7.41" )){
		fix = "7.41";
	}
}
if(fix){
