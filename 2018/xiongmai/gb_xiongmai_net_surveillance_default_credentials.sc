if(description){
	...
	script_dependencies( "gb_xiongmai_net_surveillance_detect.sc", "gb_default_credentials_options.sc" );
...
creds = make_array( "admin", "", "default", "tluafed", "root", "xc3511" );
url = "/Login.htm";
for username in keys( creds ) {
	...
	req = http_post_put_req( port: port, url: url, data: data, add_headers: make_array( "Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8", "Cookie", auth_cookie ) );
	res = http_keepalive_send_recv( port: port, data: req );
	if(ContainsString( res, "var g_SoftWareVersion=" ) && !ContainsString( res, "failedinfo=\"Log in failed!\"" )){
		VULN = TRUE;
		if(!password){
