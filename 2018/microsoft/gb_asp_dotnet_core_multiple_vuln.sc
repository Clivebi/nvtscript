...
path = infos["location"];
if(IsMatchRegexp( coreVers, "^2\\.0" )){
	sdkVer = get_kb_item( ".NET/Core/SDK/Ver" );
	affected = make_list( "2.0.0",
		 "2.0.2",
		 "2.0.3",
		 "2.1.2",
		 "2.1.3" );
	for affecVer in affected {
		if(sdkVer == affecVer){
			report = report_fixed_ver( installed_version: "ASP .NET Core With Microsoft .NET Core SDK " + sdkVer, fixed_version: "ASP .NET Core With Microsoft .NET Core SDK 2.1.4", install_path: path );
