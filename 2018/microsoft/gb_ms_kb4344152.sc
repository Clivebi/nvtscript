...
for item in registry_enum_keys( key: key2 ) {
	...
	if(path){
		...
		if(!dllVer || !IsMatchRegexp( dllVer, "^3\\." )){
			continue;
		}
		if(version_in_range( version: dllVer, test_version: "3.0.4506.7082", test_version2: "3.0.4506.8799" )){
			report = report_fixed_ver( file_checked: path + "system.identitymodel.dll", file_version: dllVer, vulnerable_range: "3.0.4506.7082 - 3.0.4506.8799" );
