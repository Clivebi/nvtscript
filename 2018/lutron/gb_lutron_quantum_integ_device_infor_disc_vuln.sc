...
if(description){
  ...
  are to upgrade to a newer release, disable respective features, remove the product or replace the product by another one." );
	...
	script_dependencies( "gb_lutron_devices_detect.sc" );
...
url = "/DbXmlInfo.xml";
if(http_vuln_check( port: lutPort, url: url, pattern: "<Latitude>[ 0-9.-]+</Latitude>", extra_check: make_list( "<Copyright>Copyright.* Lutron Electronics",
