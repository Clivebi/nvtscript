...
creds = make_array( "lutron", "integration", "nwk", "nwk2" );
for login in keys( creds ) {
	...
	if(ContainsString( tolower( recv ), "login:" )){
		send( socket: soc, data: login + "\r\n" );
		recv = recv( socket: soc, length: 256 );
		if(recv && ContainsString( tolower( recv ), "password:" )){
			send( socket: soc, data: creds[login] + "\r\n" );
			recv = recv( socket: soc, length: 2048 );
			VULN = FALSE;
			if(ContainsString( recv, "QNET>" )){
				VULN = TRUE;
			}
			send( socket: soc, data: "?HELP,#ETHERNET\r\n" );
			recv = recv( socket: soc, length: 2048 );
			if(ContainsString( recv, "Configures the IP" ) || VULN){
