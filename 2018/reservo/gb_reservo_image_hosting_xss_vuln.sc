if(description){
	...
	script_dependencies( "gb_reservo_detect.sc" );
...
if(IsMatchRegexp( res, "loadBrowsePageRecentImages\\(.+\\);%22%20style%3D<script>alert\\(" ) + timestamp + "\\)</script>" || IsMatchRegexp( res, "loadBrowsePageAlbums\\(.+\\);%22%20style%3D<script>alert\\(" ) + timestamp + "\\)</script>"){
