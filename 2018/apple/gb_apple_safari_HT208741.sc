...
if(description){
  ...
  11.1 (13605.1.33.1.4) on macOS High Sierra 10.13.4 or later. Please see the references for more information." );
	...
	script_dependencies( "macosx_safari_detect.sc", "gather-package-list.sc" );
	script_mandatory_keys( "AppleSafari/MacOSX/Version", "ssh/login/osx_version",  "ssh/login/osx_version=^10\\.1[1-3]\\." );
	exit( 0 );
}
...
if(safVer == "11.1"){
	...
	if( IsMatchRegexp( osVer, "^10\\.11" ) && version_is_less( version: ver, test_version: "11605.1.33.1.4" ) ){
		fix = "11.1 (11605.1.33.1.4)";
	}
	else {
		if( IsMatchRegexp( osVer, "^10\\.12" ) && version_is_less( version: ver, test_version: "12605.1.33.1.4" ) ){
			fix = "11.1 (12605.1.33.1.4)";
		}
		else {
			if(IsMatchRegexp( osVer, "^10\\.13" ) && version_is_less( version: ver, test_version: "13605.1.33.1.4" )){
