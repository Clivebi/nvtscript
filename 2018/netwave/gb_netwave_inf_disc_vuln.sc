if(description){
	...
	script_dependencies( "gb_netwave_ip_cam_detect.sc" );
...
req = http_get_req( port: port, url: attack_url, accept_header: "*/*", host_header_use_ip: TRUE, user_agent: "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.98 Safari/537.36" );
buf = http_keepalive_send_recv( port: port, data: req );
if(IsMatchRegexp( buf, "SSID=" ) && IsMatchRegexp( buf, "WPAPSK=" )){
...
req = http_get_req( port: port, url: attack_url, accept_header: "*/*", host_header_use_ip: TRUE, user_agent: "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.98 Safari/537.36" );
buf = http_keepalive_send_recv( port: port, data: req );
if(IsMatchRegexp( buf, "var alias=" ) && IsMatchRegexp( buf, "var ddns_host=" )){
	if(report != ""){
		report += "\r\n\r\n";
	}
	report += http_report_vuln_url( port: port, url: attack_url );
