...
for dir in nasl_make_list_unique( "/prochatrooms", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/help/index.php", port: port );
	if(egrep( pattern: "^HTTP/1\\.[01] 200", string: res ) && egrep( pattern: "<title>Pro Chat Rooms", string: res )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Pro Chat Room", version: version, install: install, cpe: cpe, concluded: pcrVer[0] ), port: port );
	}
}
exit( 0 );
