...
for dir in nasl_make_list_unique( "/phpfinance", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>PHPFinance", string: buf, icase: TRUE ) && ContainsString( buf, "phpfinance.sourceforge.net" )){
		vers = NASLString( "unknown" );
		version = eregmatch( string: buf, pattern: "<b>Version : ([0-9.]+)</b>", icase: TRUE );
		if(!isnull( version[1] )){
			vers = chomp( version[1] );
		}
		set_kb_item( name: NASLString( "www/", port, "/phpfinance" ), value: NASLString( vers, " under ", install ) );
		set_kb_item( name: "phpfinance/detected", value: TRUE );
		info = NASLString( "PHPFinance Version '" );
