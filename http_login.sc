if(description){
	...
	script_dependencies( "httpver.sc", "logins.sc" );
...
if(http_login_page){
	...
	referer = NASLString( referer, get_host_name() );
	if(( ( trp == 1 ) && ( port != 80 ) ) || ( ( trp > 1 ) && ( port != 443 ) )){
		referer = NASLString( referer, ":", port );
	}
	if(ereg( pattern: "^[^/]", string: http_login_page )){
		referer = NASLString( referer, "/" );
	}
	referer = NASLString( referer, http_login_page, "\\r\\n" );
}
req = http_post( port: port, item: http_login_form, data: http_login_fields );
req = ereg_replace( string: req, pattern: "Content-Length\\s*:\\s*", replace: NASLString( "Content-Type: application/x-www-form-urlencoded\\r\\n", referer, cookie1, "Content-Length: " ) );
soc = http_open_socket( port );
if(!soc){
	exit( 0 );
}
...
h = split( r );
for r in h {
	if(egrep( pattern: "HTTP/[019.]+ +[45][0-9][0-9]", string: r )){
		exit( 0 );
	}
	if( IsMatchRegexp( r, "^Set-Cookie" ) ){
		...
		if(cookie1){
			set_kb_item( name: NASLString( "/tmp/http/auth/", port ), value: cookie1 );
		}
	}
}
if(cookies_string){
	set_kb_item( name: NASLString( "/tmp/http/auth/", port ), value: cookies_string );
}
exit( 0 );
