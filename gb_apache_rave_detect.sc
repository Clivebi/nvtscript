...
for dir in nasl_make_list_unique( "/", "/rave", "/portal", "/social", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	url = NASLString( dir, "/login" );
	buf = http_get_cache( item: url, port: port );
	if(buf == NULL){
		continue;
	}
	if(ContainsString( buf, ">RAVE<" ) && ContainsString( buf, ">Apache Rave" )){
		vers = NASLString( "unknown" );
		version = eregmatch( string: buf, pattern: ">Apache Rave ([0-9.]+)", icase: TRUE );
		if(!isnull( version[1] )){
			vers = chomp( version[1] );
		}
		set_kb_item( name: NASLString( "www/", port, "/ApacheRave" ), value: NASLString( vers, " under ", install ) );
		...
		log_message( data: build_detection_report( app: "Apache Rave", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
	}
}
exit( 0 );
