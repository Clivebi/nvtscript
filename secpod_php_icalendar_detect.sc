...
for dir in nasl_make_list_unique( "/phpicalendar", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/print.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "PHP iCalendar" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "PHP iCalendar", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
