...
for dir in nasl_make_list_unique( "/", "/mrbs", "/mrbs1261", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/web/help.php", port: port );
	if(!IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) || ( !ContainsString( rcvRes, "About MRBS" ) && !ContainsString( rcvRes, "Meeting Room Booking System" ) )){
	...
	log_message( data: build_detection_report( app: "Meeting Room Booking System", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
}
exit( 0 );
