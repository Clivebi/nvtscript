...
if(IsMatchRegexp( res, "^HTTP/1\\.[01] 302" )){
	res = http_get_cache( port: port, item: "/login" );
}
if(( IsMatchRegexp( res, "Copyright \\(c\\)[^-]+- rConfig" ) && ContainsString( res, "rConfigLogo" ) ) || ContainsString( res, "content=\"rConfig\"" )){
	version = "unknown";
	...
	log_message( data: build_detection_report( app: "rConfig", version: version, install: "/", cpe: cpe, concluded: vers[0] ), port: port );
	exit( 0 );
}
exit( 0 );
