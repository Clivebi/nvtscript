...
if(ContainsString( buf, "SolarWinds Platform" ) || ContainsString( buf, "SolarWinds Orion" ) || ContainsString( buf, "Orion Platform" )){
	...
	if(!isnull( ncm )){
		...
		set_kb_item( name: NASLString( "www/", port, "/orion_ncm" ), value: vers );
		...
		log_message( data: build_detection_report( app: "SolarWinds Network Configuration Manager", version: vers, install: dir, cpe: cpe, concluded: ncm[0] ), port: port );
	}
}
exit( 0 );
