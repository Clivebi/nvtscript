...
send( socket: soc, data: NASLString( "STARTTLS\\r\\n" ) );
r = smtp_recv_line( socket: soc );
if(!r){
	smtp_close( socket: soc, check_data: r );
	exit( 0 );
}
if( IsMatchRegexp( r, "^220[ -]" ) || ContainsString( r, "Ready to start TLS" ) || ContainsString( r, "TLS go ahead" ) || ContainsString( r, " Start TLS" ) ){
	...
	if(commandlist && NASLTypeof( commandlist ) == "array"){
	...
	if(soc){
		...
		if(ehlo){
			commands = split( buffer: ehlo, keep: FALSE );
			first = 0;
			...
			if(auth_string = egrep( string: ehlo, pattern: "^250[ -]AUTH .+" )){
				...
				auth_report = "";
				auths = split( buffer: auth_string, sep: " ", keep: FALSE );
				for auth in auths {
