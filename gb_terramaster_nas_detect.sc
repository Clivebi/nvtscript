...
if(( !ContainsString( res, "TerraMaster" ) || !ContainsString( res, "Tos_Check_Box" ) ) && !ContainsString( res, "<title>TOS</title>" )){
	res = http_get_cache( port: port, item: "/" );
	if(!ContainsString( res, "<title>TerraMaster" ) || !ContainsString( res, "name=\"minuser\"" ) || !ContainsString( res, "name=\"dataError\"" )){
		exit( 0 );
	}
}
version = "unknown";
...
log_message( data: build_detection_report( app: "TerraMaster NAS", version: version, install: "/", cpe: cpe, concluded: vers[0], concludedUrl: concUrl ), port: port );
exit( 0 );
