...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "ABB DriveManager for SIMATIC" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:abb:drivemanager_for_simatic:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
