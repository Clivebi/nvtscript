if(description){
	...
	script_dependencies( "gb_amqp_detect.sc" );
...
if(ereg( pattern: "productSRabbitMQ.", string: res )){
	...
	log_message( data: build_detection_report( app: "RabbitMQ", version: version, install: port + "/amqp", cpe: cpe, concluded: res ), port: port );
}
exit( 0 );
