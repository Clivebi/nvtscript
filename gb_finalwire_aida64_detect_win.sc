...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "AIDA64 (Extreme|Engineer|Business|Network Audit) [a-zA_Z0-9.]+" )){
			continue;
		}
		pub = registry_get_sz( key: key + item, item: "Publisher" );
		if(!pub || !IsMatchRegexp( pub, "FinalWire Ltd." )){
			continue;
		}
		...
		register_and_report_cpe( app: pub + " " + appName, ver: version, concluded: concluded, base: "cpe:/a:finalwire:aida64:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
