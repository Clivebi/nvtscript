...
for dir in nasl_make_list_unique( "/", "/netpet", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "Netpet Content Management System" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Netpet CMS", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
