...
for dir in nasl_make_list_unique( "/", "/piwik", "/matomo", "/analytics", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	buf = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) && ( eregmatch( pattern: "<title>.*Matomo.*</title>", string: buf, icase: TRUE ) || ( ContainsString( buf, "https://matomo.org" ) && ContainsString( buf, "piwik.piwik_url" ) ) )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Matomo", version: version, install: install, cpe: cpe, extra: extra, concludedUrl: conclUrl, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
