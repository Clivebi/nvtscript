if(description){
	...
	script_version( "2020-10-06T09:07:14+0000" );
	script_tag( name: "last_modification", value: "2020-10-06 09:07:14 +0000 (Tue, 06 Oct 2020)" );
...
var do_report;
launch_gshb_10 = script_get_preference( name: "Launch IT-Grundschutz (10. EL)", id: 1 );
...
launch_gshb_11 = script_get_preference( name: "Launch IT-Grundschutz (11. EL)", id: 2 );
...
launch_gshb_12 = script_get_preference( name: "Launch IT-Grundschutz (12. EL)", id: 3 );
...
launch_gshb_13 = script_get_preference( name: "Launch IT-Grundschutz (13. EL)", id: 4 );
...
launch_gshb_15 = script_get_preference( name: "Launch IT-Grundschutz (15. EL)", id: 5 );
...
launch_gshb = script_get_preference( name: "Launch latest IT-Grundschutz version", id: 6 );
...
verbose_gshb = script_get_preference( name: "Verbose IT-Grundschutz results", id: 8 );
...
security_level = script_get_preference( name: "Level of Security (IT-Grundschutz)", id: 7 );
report += "\n" + "Level of Security (IT-Grundschutz):     " + security_level;
set_kb_item( name: "GSHB/level", value: security_level );
launch_pci_dss = script_get_preference( name: "Launch PCI-DSS (Version 2.0)", id: 9 );
...
launch_latest_pci_dss = script_get_preference( name: "Launch latest PCI-DSS version", id: 10 );
...
verbose_pci_dss = script_get_preference( name: "Verbose PCI-DSS results", id: 11 );
report += "\n" + "Verbose PCI-DSS results:                " + verbose_pci_dss;
if(verbose_pci_dss == "no"){
	set_kb_item( name: "PCI-DSS/silence", value: "Wahr" );
}
launch_ce = script_get_preference( name: "Launch Cyber Essentials", id: 12 );
...
launch_gdpr = script_get_preference( name: "Launch EU GDPR", id: 13 );
...
launch_compliance_result = script_get_preference( name: "Launch Compliance Test", id: 15 );
...
verbose_policy_controls = script_get_preference( name: "Verbose Policy Controls", id: 14 );
...
var DomFunk = "none";
CN = script_get_preference( name: "Testuser Common Name", id: 17 );
OU = script_get_preference( name: "Testuser Organization Unit", id: 18 );
DomFunkMod = script_get_preference( name: "Windows Domaenenfunktionsmodus", id: 19 );
...
if( DomFunkMod == "Unbekannt" ) {
	...
	if( DomFunkMod == "Windows 2000 gemischt und Windows 2000 pur" ) {
		DomFunk = "0";
	}
	else {
		if( DomFunkMod == "Windows Server 2003 Interim" ) {
			DomFunk = "1";
		}
		else {
			if( DomFunkMod == "Windows Server 2003" ) {
				DomFunk = "2";
			}
			else {
				if( DomFunkMod == "Windows Server 2008" ) {
					DomFunk = "3";
				}
				else {
					if( DomFunkMod == "Windows Server 2008 R2" ) {
						DomFunk = "4";
					}
					else {
						if(!DomFunk){
