...
func oracle_tnslsnr_security( port ){
	...
	if(sock){
		...
		if( ContainsString( reply, "SECURITY=OFF" ) ){
			security_message( port: port );
		}
		else {
			if(ContainsString( reply, "SECURITY=ON" )){
				report = NASLString( "This host is running a passworded Oracle tnslsnr.\\n" );
