...
if(description){
	...
	script_name( "Microsoft Internet Information Services (IIS) Possible Compromise" );
	script_category( ACT_ATTACK );
	script_copyright( "Copyright (C) 2003 Digital Defense Inc." );
	script_family( "Malware" );
	script_dependencies( "secpod_ms_iis_detect.sc", "no404.sc", "DDI_Directory_Scanner.sc", "webmirror.sc" );
	script_require_ports( "Services/www", 80 );
	script_mandatory_keys( "IIS/installed" );
	script_tag( name: "solution", value: "Investigate the discovered files." );
	script_tag( name: "summary", value: "One or more files were found on this host that indicate a
  possible compromise of the Microsoft Internet Information Services (IIS)." );
...
func check( url, arg, pat ){
	var str, r;
	str = http_get( item: NASLString( url, arg ), port: port );
...
func dllcheck( req ){
	...
	if(ContainsString( r, "module could not be found" )){
		add_cache( url: req );
	}
	if(ContainsString( r, "procedure could not be found" )){
		return TRUE;
	}
	return FALSE;
}
func iisecheck( req ){
	...
	if( egrep( pattern: "We Got It", string: r ) ){
		...
		if(ereg( pattern: "HTTP/1\\.[01] 40[34] ", string: r ) || egrep( pattern: "module could not be found", string: r2 )){
			add_cache( url: req );
			http_close_socket( soc );
			return FALSE;
		}
...
func initialize_dirs(  ){
	var _dir_idx, d;
	_dir_idx = 0;
	for d in nasl_make_list_unique( "/scripts", "/msadc", http_cgi_dirs( port: port ) ) {
		add_dir_list( dir: d );
		_dir_idx = _dir_idx + 1;
	}
	return ( _dir_idx );
}
func check_dir_list( dir ){
	var CDL;
...
func add_report( name, url, reports ){
	if(!reported( url: url )){
		if( reported_tests[name] ){
			}
		else {
			report_header = report_header + NASLString( reports[name], "\\n\\n" );
			reported_tests[name] = 1;
		}
		report_footer = report_footer + NASLString( name, " - ", url, "\\n" );
		report = NASLString( report_header, "\\n\\nDetails:\\n", report_footer );
	}
}
...
pats["cmd.asp"] = "\\.CMD";
pats["upload.asp"] = NASLString( "type=", dblquot, "file", dblquot );
...
for(x = 0;tests[x];x++){
	...
	for(d = 0;dirs[d];d++){
		stop = 0;
		for(f = 0;stop == 0;f++){
			filename = files[NASLString( cur_test, "_", f )];
			if( filename ){
				if( dirs[d] == "/" ){
					url = NASLString( "/", filename );
				}
				else {
					url = NASLString( dirs[d], "/", filename );
				}
				if(!check_cache( url: url )){
					if(cur_meth == "GET"){
						if(check( url: url, arg: cur_args, pat: cur_pat )){
							add_report( name: cur_test, url: url, reports: reports );
							add_cache( url: url );
						}
					}
