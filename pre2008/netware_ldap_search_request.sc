...
senddata = raw_string( 0x30, 0x25, 0x02, 0x01, 0x02, 0x63, 0x20, 0x04, 0x00, 0x0a, 0x01, 0x02, 0x0a, 0x01, 0x00, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x01, 0x01, 0x00, 0x87, 0x0b, 0x6f, 0x62, 0x6a, 0x65, 0x63, 0x74, 0x63, 0x6c, 0x61, 0x73, 0x73, 0x30, 0x00 );
...
version = NASLString( buf );
hbuf = hexstr( buf );
if(ContainsString( buf, "Novell" )){
	...
	if(hostname){
		report += NASLString( "Hostname: ", hostname, "\\n" );
		flag = TRUE;
	}
}
if(ContainsString( buf, "LDAP Server" )){
	...
	if(version){
		report += NASLString( "LDAP Server Version: ", version, "\\n" );
		flag = TRUE;
	}
}
if(flag){
