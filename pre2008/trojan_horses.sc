if(description){
	...
	script_dependencies( "unknown_services.sc", "find_service_nmap.sc", "os_detection.sc", "gb_tls_version_get.sc" );
...
req = NASLString( "^TCP ", port, " " );
str = egrep( string: trojanlist, pattern: req );
if(!str){
	exit( 0 );
}
name = ereg_replace( string: str, pattern: req, replace: "" );
name = ereg_replace( string: name, pattern: " *, *", replace: NASLString( "\\n- " ) );
m = NASLString( "An unknown service runs on this port. It is sometimes opened by this/these Trojan horse(s):\\n\\n- ", name );
