...
hx_banner[0] = NASLString( "Hacker Defender v0.51-0.82b" );
...
hx_banner[1] = NASLString( "Hacker Defender v0.82-0.83" );
...
hx_banner[2] = NASLString( "Hacker Defender v0.84-1.0.0" );
				...
				if(data && strlen( data ) == 1 && ord( data[0] ) == 0xe0){
					for(t = 0;t < 20;t++){
						...
						if(data && strlen( data ) == 1 && ord( data[0] ) == 0xe2){
							report = NASLString( "The remote host is running the ", hx_banner[j], " backdoor." );
