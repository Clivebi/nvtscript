...
if(via){
	...
	for(;via;){
		...
		trace = NASLString( trace, " ", line, "\\n" );
	}
}
close( soc );
if( trace ){
	log_message( port: port, data: NASLString( "The GET method revealed those proxies on the way to this web server :\\n", trace ) );
}
else {
	if(egrep( pattern: "^X-Cache:", string: heads )){
...
count = 0;
for(i = 1;i <= n;i++){
	tmp = NASLString( viaL[i], " - ", srvL[i] );
	if(!ContainsString( trace, tmp )){
		count++;
		trace += tmp + "\n";
	}
}
if(n > 0){
	log_message( port: port, data: NASLString( "The TRACE method revealed ", count, " proxy(s) between us and the web server :\\n", trace ) );
}
exit( 0 );
