if(description){
	...
	script_version( "2021-02-19T12:59:27+0000" );
	script_tag( name: "last_modification", value: "2021-02-19 12:59:27 +0000 (Fri, 19 Feb 2021)" );
...
for cgi in cgis {
	everythingrray = split( buffer: cgi, sep: " ", keep: FALSE );
	if( IsMatchRegexp( everythingrray[0], ".*/$" ) ){
		isdir = TRUE;
	}
	else {
		isdir = FALSE;
	...
	maxFailedReqs = 5;
	if(!isdir){
		vrequest = NASLString( everythingrray[0], "?" );
		bogus_vrequest = NASLString( everythingrray[0], "?", rand() );
		pseudocount = 0;
		for rrayval in everythingrray {
			if( pseudocount >= 2 ){
				if( ContainsString( rrayval, "]" ) ){
					pseudocount--;
					tmpf = ereg_replace( pattern: "\\[|\\]", string: rrayval, replace: "" );
					data[pseudocount] = tmpf;
					vrequest = NASLString( vrequest, "=", tmpf );
				}
				else {
					param[pseudocount] = rrayval;
					if( pseudocount == 2 ){
						vrequest = NASLString( vrequest, rrayval );
					}
					else {
						vrequest = NASLString( vrequest, "&", rrayval );
					}
	...
	for(z = 2;param[z];z++){
		...
		res_saved = strstr( res, NASLString( "\\r\\n\\r\\n" ) );
		...
		for(i = 0;posreply[i];i++){
			if(ContainsString( res, posreply[i] )){
				exit( 0 );
			}
		}
		for(poo = 0;poison[poo];poo++){
			...
			blind = NASLString( param[0], "?" );
			for(i = 2;param[i];i++){
				if( i == z ){
					if(blinder[poo]){
						doblind++;
						qa = NASLString( blind, param[i], "=", data[i], "'" );
						blind = NASLString( blind, param[i], "=", data[i], blinder[poo] );
					}
					if( data[i] ){
						url = NASLString( url, param[i], "=", poison[poo] );
					}
					else {
						url = NASLString( url, param[i], "=", poison[poo] );
					}
					...
					if(blinder[poo]){
						qa = NASLString( qa, param[i], "=", data[i] );
						blind = NASLString( blind, param[i], "=", data[i] );
					}
					if( data[i] ){
						url = NASLString( url, param[i], "=", data[i] );
					}
					else {
						url = NASLString( url, param[i], "=" );
					}
				}
				if(param[i + 1]){
					url = NASLString( url, "&" );
					blind = NASLString( blind, "&" );
					qa = NASLString( qa, "&" );
				}
			}
			req = http_get( item: url, port: port );
			...
			if(ContainsString( inbuff, "Content-Length: 0" )){
				continue;
			}
			for(mu = 0;posreply[mu];mu++){
				if(ContainsString( inbuff, posreply[mu] )){
					unsafe_urls = NASLString( unsafe_urls, http_report_vuln_url( port: port, url: url, url_only: TRUE ), ":", posreply[mu], "\\n" );
					mywarningcount++;
				}
			}
			if(doblind > 0){
				...
				buff_body = strstr( inbuff, NASLString( "\\r\\n\\r\\n" ) );
				if(buff_body == res_saved){
					req_qa = http_get( item: qa, port: port );
					inbuff = http_keepalive_send_recv( port: port, data: req_qa );
					qa_body = strstr( inbuff, NASLString( "\\r\\n\\r\\n" ) );
					if(ContainsString( inbuff, "Content-Length: 0" )){
						continue;
					}
					if(qa_body != res_saved){
						blind_urls = NASLString( blind_urls, http_report_vuln_url( port: port, url: blind, url_only: TRUE ), "\\n" );
						blindwarningcount++;
					}
				}
			}
			if(safe_checks() == 0){
				tmppost = split( buffer: url, sep: "?", keep: FALSE );
				...
				if(doblind > 0){
					tmppost = split( buffer: blind, sep: "?", keep: FALSE );
					mypostdata = tmppost[1];
					postreq = http_post( item: param[0], port: port, data: mypostdata );
					inbuff = http_keepalive_send_recv( port: port, data: postreq );
					if(isnull( inbuff )){
					...
					buff_body = strstr( inbuff, NASLString( "\\r\\n\\r\\n" ) );
					if(buff_body == res_saved){
						qapost = split( buffer: blind, sep: "?", keep: FALSE );
						...
						qa_body = strstr( qabuff, NASLString( "\\r\\n\\r\\n" ) );
						if(qa_body != res_saved){
							blind_urls = NASLString( blind_urls, http_report_vuln_url( port: port, url: blind, url_only: TRUE ), "\\n" );
							blindwarningcount++;
						}
...
if(mywarningcount > 0){
	VULN = TRUE;
	report += NASLString( "The following URLs seem to be vulnerable to various SQL injection techniques : <url>:<matching pattern>\\n\\n", unsafe_urls, "\\n\\n" );
}
if(blindwarningcount > 0){
	VULN = TRUE;
	report += NASLString( "The following URLs seem to be vulnerable to BLIND SQL injection techniques : \\n\\n", blind_urls, "\\n\\n" );
}
if(VULN){
