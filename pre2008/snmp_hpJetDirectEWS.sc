if(description){
	...
	script_family( "SNMP" );
	script_dependencies( "snmp_detect.sc", "gb_hp_printer_detect.sc" );
...
func get( community, object ){
	var len, tot_len, packet, object_len, pack_len, community, object;
	...
	packet = packet + community + raw_string( 0xA0, pack_len, 0x02, 0x04, 0x5e, 0xa4, 0x3f, 0x0c, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x30, object_len ) + object + raw_string( 0x05, 0x00 );
	return ( packet );
}
func vulnerable( httpport ){
	var httpport, url, reply, sndReq, rcvRes;
	...
	if(IsMatchRegexp( rcvRes, "HTTP/1\\.. 200" ) || IsMatchRegexp( rcvRes, "HTTP/1\\.. 401" )){
		return ( reply );
	}
	if(IsMatchRegexp( rcvRes, "HTTP/1\\.. 404" )){
		url = "/";
...
if(!( vulnerable( httpport: httpport ) )){
	exit( 0 );
}
if(!get_udp_port_state( snmpport )){
	exit( 0 );
}
...
MIB = raw_string( 0x30, 0x11, 0x06, 0x0D, 0x2B, 0x06, 0x01, 0x04, 0x01, 0x0B, 0x02, 0x03, 0x09, 0x01, 0x01, 0x0D, 0x00 );
...
if(!( passwordless )){
	password = NASLString( "The password is " );
	for(i = start;i < len;i++){
		if( r[i] == equal_sign ){
			i = len;
		}
