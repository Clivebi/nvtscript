if(description){
	...
	script_copyright( "Copyright (C) 2004 Andrey I. Zakharov and John Lampe" );
	script_dependencies( "os_detection.sc", "global_settings.sc" );
...
for port in make_list( 22,
	...
	reply = NULL;
	sport++;
	filter = NASLString( "src port ", port, " and src host ", get_host_ip(), " and dst port ", sport );
	ip = forge_ip_packet( ip_v: 4, ip_hl: 5, ip_tos: 0, ip_off: 0, ip_len: 20, ip_p: IPPROTO_TCP, ip_id: ipid, ip_ttl: 0x40, ip_src: this_host() );
	tcp = forge_tcp_packet( ip: ip, th_sport: sport, th_dport: port, th_flags: 0x02, th_seq: init_seq, th_ack: myack, th_x2: 0, th_off: 5, th_win: 2048, th_urp: 0 );
	for(i = 0;i < 3;i++){
		reply = send_packet( packet: tcp, pcap_active: TRUE, pcap_filter: filter, pcap_timeout: 1 );
		if(reply){
			break;
		}
	}
	if(!reply){
		...
		filter = NASLString( "src port ", port, " and src host ", get_host_ip(), " and dst port ", sport );
		ip = forge_ip_packet( ip_v: 4, ip_hl: 5, ip_tos: 0, ip_off: 0, ip_len: 20, ip_p: IPPROTO_TCP, ip_id: ipid, ip_ttl: 0x40, ip_src: this_host() );
		tcp = forge_tcp_packet( ip: ip, th_sport: sport, th_dport: port, th_flags: 0x42, th_seq: init_seq, th_ack: myack, th_x2: 0, th_off: 5, th_win: 2048, th_urp: 0 );
		for(i = 0;i < 3;i++){
			reply = send_packet( pcap_active: TRUE, pcap_filter: filter, pcap_timeout: 1,packet: tcp );
			if(reply){
				break;
			}
		}
