...
send( socket: soc, data: NASLString( "EHLO X\\r\\n" ) );
ok = smtp_recv_line( socket: soc );
if(!ok || !ContainsString( ok, "XEXCH50" )){
	smtp_close( socket: soc, check_data: ok );
	exit( 0 );
}
send( socket: soc, data: NASLString( "MAIL FROM: Administrator\\r\\n" ) );
ok = smtp_recv_line( socket: soc );
if(!ok){
	smtp_close( socket: soc, check_data: ok );
	exit( 0 );
}
send( socket: soc, data: NASLString( "RCPT TO: Administrator\\r\\n" ) );
ok = smtp_recv_line( socket: soc );
if(!ok){
	smtp_close( socket: soc, check_data: ok );
	exit( 0 );
}
send( socket: soc, data: NASLString( "XEXCH50 2 2\\r\\n" ) );
ok = smtp_recv_line( socket: soc );
smtp_close( socket: soc, check_data: ok );
if(!ok){
	exit( 0 );
