...
if(!isnull( matches )){
	dir = matches[2];
	req = http_get( item: NASLString( dir, "/post.php?reply=<script>document.write('VT to detect post.php flaw');</script>" ), port: port );
	res = http_keepalive_send_recv( port: port, data: req );
	if(isnull( res )){
		exit( 0 );
	}
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ereg( pattern: "VT plugin to detect post.php flaw", string: res )){
