if(description){
	...
	script_version( "2020-10-05T06:32:09+0000" );
	script_tag( name: "last_modification", value: "2020-10-05 06:32:09 +0000 (Mon, 05 Oct 2020)" );
	...
	script_dependencies( "gb_hcl_domino_imap_detect.sc", "gb_hcl_domino_pop3_detect.sc", "gb_hcl_domino_smtp_detect.sc", "gb_hcl_domino_http_detect.sc" );
...
if(http_ports = get_kb_list( "hcl/domino/http/port" )){
	for port in http_ports {
		...
		if(concluded){
			extra += "  The following URLs where used for the product / version detection (URL : exposed version):" + concluded;
		}
	}
}
report = build_detection_report( app: "HCL Domino", version: detected_version, install: location, cpe: cpe1 );
if(extra){
	report += "\n\nDetection methods:\n";
	report += "\n" + extra;
}
