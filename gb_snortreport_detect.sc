...
for dir in nasl_make_list_unique( "/snort", "/snortreport", "/snortreport-1.3.1", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>SNORT Report", string: buf, icase: TRUE ) && ContainsString( buf, "Symmetrix" )){
		vers = NASLString( "unknown" );
		version = eregmatch( string: buf, pattern: "Snort Report Version ([0-9.]+)", icase: TRUE );
		if(!isnull( version[1] )){
			vers = chomp( version[1] );
		}
		set_kb_item( name: NASLString( "www/", port, "/snortreport" ), value: NASLString( vers, " under ", install ) );
		set_kb_item( name: "snortreport/detected", value: TRUE );
		info = NASLString( "Snort Report Version '" );
