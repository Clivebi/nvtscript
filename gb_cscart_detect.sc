...
for dir in nasl_make_list_unique( "/", "/cart", "/cs", "/store", "/cscart", "/cs-cart", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( port: port, item: dir + "/" );
	if(ContainsString( res, "CS-Cart - Shopping Cart Software" ) && ContainsString( res, "index.php?dispatch=" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "CS-Cart", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
	}
}
exit( 0 );
