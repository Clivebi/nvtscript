...
req = http_post_put_req( port: port, url: "/login.php", data: login_data, accept_header: "text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8", add_headers: make_array( "Cookie", cookie + "; loginName=admin", "Content-Type", "application/x-www-form-urlencoded" ) );
buf = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
if(ContainsString( buf, "cmd=loginfail" )){
	exit( 99 );
}
if(http_vuln_check( port: port, url: "/save_config.php", pattern: "<title>Save Configuration</title>", check_header: TRUE, cookie: cookie + "; loginName=admin" )){
