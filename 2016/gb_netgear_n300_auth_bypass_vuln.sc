...
if(!ContainsString( banner, "Basic realm=\"NETGEAR" )){
	exit( 0 );
}
buf = http_get_cache( item: "/", port: netport );
if(!ContainsString( buf, "HTTP/1.1 401 Unauthorized" )){
	exit( 0 );
}
for(i = 0;i <= 5;i++){
	...
	if(buf1){
		...
		if(ContainsString( buf2, "NETGEAR" ) && ContainsString( buf2, "firstpage_var" ) && ContainsString( buf2, "enable_action" )){
