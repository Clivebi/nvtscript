...
if(!notePath || ContainsString( notePath, "Could not find the install location" )){
	exit( 0 );
}
noteVer = fetch_file_version( sysPath: notePath, file_name: "onmain.dll" );
if(noteVer){
	if(IsMatchRegexp( noteVer, "^(12|14|15|16).*" )){
		if( IsMatchRegexp( noteVer, "^12" ) ){
			Vulnerable_range = "12 - 12.0.6753.4999";
		}
		else {
			if( IsMatchRegexp( noteVer, "^14" ) ){
				Vulnerable_range = "14 - 14.0.7172.4999";
			}
			else {
				if( IsMatchRegexp( noteVer, "^15" ) ){
					Vulnerable_range = "15 - 15.0.4849.0999";
				}
				else {
					if(IsMatchRegexp( noteVer, "^16" )){
	...
	if(version_in_range( version: noteVer, test_version: "12.0", test_version2: "12.0.6753.4999" ) || version_in_range( version: noteVer, test_version: "14.0", test_version2: "14.0.7172.4999" ) || version_in_range( version: noteVer, test_version: "15.0", test_version2: "15.0.4849.0999" ) || version_in_range( version: noteVer, test_version: "16.0", test_version2: "16.0.4417.0999" )){
