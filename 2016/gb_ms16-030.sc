				...
				if( IsMatchRegexp( dllVer, "^(6\\.2\\.9200\\.2)" ) ){
					...
					if( IsMatchRegexp( dllVer, "^(6\\.1\\.7601\\.1)" ) ){
						...
						if( IsMatchRegexp( dllVer, "^(6\\.1\\.7601\\.2)" ) ){
							...
							if( IsMatchRegexp( dllVer, "^(10\\.0\\.10240\\.1)" ) ){
								Vulnerable_range = "Less than 10.0.10240.16724";
							}
							else {
								if(IsMatchRegexp( dllVer, "^(10\\.0\\.10586)" )){
...
if( hotfix_check_sp( winVista: 3, win2008: 3 ) > 0 ){
	if(version_is_less( version: dllVer, test_version: "6.0.6002.19596" ) || version_in_range( version: dllVer, test_version: "6.0.6002.23000", test_version2: "23906" )){
		VULN = TRUE;
	}
}
else {
	if( hotfix_check_sp( win8_1: 1, win8_1x64: 1, win2012R2: 1 ) > 0 ){
		...
		if( hotfix_check_sp( win2012: 1 ) > 0 ){
			if(version_is_less( version: dllVer, test_version: "6.2.9200.17646" ) || version_in_range( version: dllVer, test_version: "6.2.9200.21000", test_version2: "6.2.9200.21764" )){
				VULN = TRUE;
			}
		}
		else {
			if(hotfix_check_sp( win10: 1, win10x64: 1 ) > 0){
				if( version_is_less( version: dllVer, test_version: "10.0.10240.16724" ) ){
					...
					if(version_in_range( version: dllVer, test_version: "10.0.10586.0", test_version2: "10.0.10586.161" )){
						VULN = TRUE;
					}
				}
			}
