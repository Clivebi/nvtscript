...
if( IsMatchRegexp( sysVer, "^(6\\.0\\.6002\\.1)" ) ){
	...
	if( IsMatchRegexp( sysVer, "^(6\\.0\\.6002\\.2)" ) ){
		...
		if( IsMatchRegexp( sysVer, "^(6\\.1\\.7601\\.2)" ) ){
			...
			if( IsMatchRegexp( sysVer, "^(6\\.1\\.7601\\.1)" ) ){
				Vulnerable_range = "Less than 6.1.7601.19113";
			}
			else {
				if( IsMatchRegexp( sysVer, "^(6\\.2\\.9200\\.1)" ) ){
...
if( hotfix_check_sp( winVista: 3, win2008: 3 ) > 0 ){
	if(version_is_less( version: sysVer, test_version: "6.0.6002.19576" ) || version_in_range( version: sysVer, test_version: "6.0.6002.23000", test_version2: "6.0.6002.23885" )){
		VULN = TRUE;
	}
}
else {
	if( hotfix_check_sp( win7: 2, win7x64: 2, win2008r2: 2 ) > 0 ){
		if(version_is_less( version: sysVer, test_version: "6.1.7601.19113" ) || version_in_range( version: sysVer, test_version: "6.1.7601.23000", test_version2: "6.1.7601.23316" )){
			VULN = TRUE;
		}
	}
	else {
		if( hotfix_check_sp( win2012: 1 ) > 0 ){
			if(version_is_less( version: sysVer, test_version: "6.2.9200.17619" ) || version_in_range( version: sysVer, test_version: "6.2.9200.21000", test_version2: "6.2.9200.21737" )){
				VULN = TRUE;
			}
		}
		else {
...
if(hotfix_check_sp( win10: 1, win10x64: 1 ) > 0){
	if(version_is_less( version: sysVer, test_version: "10.0.10240.16683" ) || version_in_range( version: sysVer, test_version: "10.0.10586.0", test_version2: "10.0.10586.102" )){
		VULN = TRUE;
	}
}
if(VULN){
