...
if( IsMatchRegexp( sysVer, "^(6\\.0\\.6002\\.1)" ) ){
	...
	if( IsMatchRegexp( sysVer, "^(6\\.0\\.6002\\.2)" ) ){
		...
		if( IsMatchRegexp( sysVer, "^(6\\.1\\.7601)" ) ){
			Vulnerable_range = "Less than 6.1.7601.23488";
		}
		else {
			if( IsMatchRegexp( sysVer, "^(6\\.2\\.9200)" ) ){
...
if( hotfix_check_sp( winVista: 3, winVistax64: 3, win2008x64: 3, win2008: 3 ) > 0 ){
	if(version_is_less( version: sysVer, test_version: "6.0.6002.19666" ) || version_in_range( version: sysVer, test_version: "6.0.6002.23000", test_version2: "6.0.6002.23980" )){
		VULN = TRUE;
	}
}
else {
...
if(hotfix_check_sp( win10: 1, win10x64: 1 ) > 0){
	if(version_is_less( version: sysVer, test_version: "10.0.10240.17022" ) || version_in_range( version: sysVer, test_version: "10.0.10586.0", test_version2: "10.0.10586.493" )){
		VULN = TRUE;
	}
}
if(VULN){
