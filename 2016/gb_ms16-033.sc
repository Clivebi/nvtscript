...
if(hotfix_check_sp( winVista: 3, win7: 2, win7x64: 2, win2008: 3, win2008r2: 2, win8: 1, win2012: 1, win2012R2: 1, win8_1: 1, win8_1x64: 1, win10: 1, win10x64: 1 ) <= 0){
	exit( 0 );
}
...
if( IsMatchRegexp( usb_store_ver, "^(6\\.0\\.6002\\.1)" ) ){
	Vulnerable_range = "Less than 6.0.6002.19595";
}
else {
	if( IsMatchRegexp( usb_store_ver, "^(6\\.0\\.6002\\.2)" ) ){
		Vulnerable_range = "6.0.6002.23000 - 6.0.6002.23905";
	}
	else {
		if( IsMatchRegexp( usb_store_ver, "^(6\\.1\\.7601\\.2)" ) ){
			Vulnerable_range = "6.1.7601.23000 - 6.1.7601.23343";
		}
		else {
			if( IsMatchRegexp( usb_store_ver, "^(6\\.1\\.7601\\.1)" ) ){
				Vulnerable_range = "Less than 6.1.7601.19144";
			}
			else {
				if( IsMatchRegexp( usb_store_ver, "^(6\\.2\\.9200\\.1)" ) ){
					Vulnerable_range = "Less than 6.2.9200.17642";
				}
				else {
					if( IsMatchRegexp( usb_store_ver, "^(6\\.2\\.9200\\.2)" ) ){
						Vulnerable_range = "6.2.9200.21000 - 6.2.9200.21760";
					}
					else {
						if( IsMatchRegexp( usb_store_ver, "^(6\\.3\\.9600\\.1)" ) ){
							Vulnerable_range = "Less than 6.3.9600.18224";
						}
						else {
							if( IsMatchRegexp( usb_store_ver, "^(10\\.0\\.10240)" ) ){
								Vulnerable_range = "Less than 10.0.10240.16724";
							}
							else {
								if(IsMatchRegexp( usb_store_ver, "^(10\\.0\\.10586)" )){
...
if(usb_store_ver){
	if( hotfix_check_sp( winVista: 3, win2008: 3 ) > 0 ){
		if(version_is_less( version: usb_store_ver, test_version: "6.0.6002.19595" ) || version_in_range( version: usb_store_ver, test_version: "6.0.6002.23000", test_version2: "6.0.6002.23905" )){
			VULN = TRUE;
		}
	}
	else {
		if( hotfix_check_sp( win7: 2, win7x64: 2, win2008r2: 2 ) > 0 ){
			if(version_is_less( version: usb_store_ver, test_version: "6.1.7601.19144" ) || version_in_range( version: usb_store_ver, test_version: "6.1.7601.23000", test_version2: "6.1.7601.23343" )){
				VULN = TRUE;
			}
		}
		else {
			if( hotfix_check_sp( win2012: 1 ) > 0 ){
				if(version_is_less( version: usb_store_ver, test_version: "6.2.9200.17642" ) || version_in_range( version: usb_store_ver, test_version: "6.2.9200.21000", test_version2: "6.2.9200.21760" )){
					VULN = TRUE;
				}
			}
			else {
	...
	if(hotfix_check_sp( win10: 1, win10x64: 1 ) > 0){
		if(version_is_less( version: usb_store_ver, test_version: "10.0.10240.16724" ) || version_in_range( version: usb_store_ver, test_version: "10.0.10586.0", test_version2: "10.0.10586.161" )){
			VULN = TRUE;
		}
	}
}
