...
if(description){
	...
	script_name( "Adobe Flash Player Security Update (apsa16-02) - Mac OS X" );
  ...
  also some unknown impact." );
	script_tag( name: "affected", value: "Adobe Flash Player versions before 18.0.0.352 and 20.x before 21.0.0.242." );
	script_tag( name: "solution", value: "Update to version 18.0.0.352, 21.0.0.242 or later." );
...
vers = infos["version"];
path = infos["location"];
if( version_in_range( version: vers, test_version: "20", test_version2: "21.0.0.241" ) ){
	fix = "21.0.0.242";
	VULN = TRUE;
}
else {
	if(version_is_less( version: vers, test_version: "18.0.0.352" )){
		fix = "18.0.0.352";
		VULN = TRUE;
	}
}
