if(description){
	...
	script_name( "libsoup HTTP Server Detection (HTTP)" );
...
if(concl = egrep( pattern: "Server:.*libsoup", string: banner )){
	concl = chomp( concl );
	version = "unknown";
	...
	log_message( data: build_detection_report( app: "libsoup HTTP Server", version: version, install: "/", cpe: cpe, concluded: concl ), port: port );
	exit( 0 );
}
exit( 0 );
