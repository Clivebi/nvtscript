...
for keypart in nasl_make_list_unique( registry_enum_keys( key: base_key_one ), registry_enum_keys( key: base_key_two ) ) {
	...
	if(!IsMatchRegexp( name, "^CX[- ]Programmer" )){
		continue;
	}
	...
	register_and_report_cpe( app: "Omron CX-Programmer", ver: version, concluded: version, base: CPE, expr: "([0-9.]+)", insloc: insloc, regService: "smb-login" );
	exit( 0 );
}
exit( 0 );
