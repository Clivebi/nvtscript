...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "^QuickOPC " )){
			continue;
		}
		pub = registry_get_sz( key: key + item, item: "Publisher" );
		if(!pub || !IsMatchRegexp( pub, "OPC Labs" )){
			continue;
		}
		app = pub + " " + appName;
		concluded = appName;
		...
		register_and_report_cpe( app: app, ver: version, concluded: concluded, base: "cpe:/a:opc_labs:quickopc:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
