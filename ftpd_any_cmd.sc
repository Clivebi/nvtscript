if(description){
	...
	script_dependencies( "find_service_3digits.sc", "logins.sc" );
...
ftpcmd["REIN"] = 1;
ftpcmd["LIST"] = 1;
func test( port, login, pass ){
	var port, login, pass;
	var soc, r, cmd, answer;
	...
	r = ftp_recv_line( socket: soc, retry: 2 );
	if(!IsMatchRegexp( r, "^230" )){
		send( socket: soc, data: "PASS " + pass + "\r\n" );
		r = ftp_recv_line( socket: soc, retry: 2 );
		if(!IsMatchRegexp( r, "2[0-9][0-9] " )){
	...
	if(strlen( r ) == 0 || IsMatchRegexp( r, "^5[0-9][0-9]" )){
		return 0;
	}
