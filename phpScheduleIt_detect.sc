...
for dir in nasl_make_list_unique( "/conference", "/schedule", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	url = NASLString( dir, "/roschedule.php" );
	...
	if(egrep( pattern: "phpScheduleIt v[0-9.]+", string: buf, icase: TRUE )){
		...
		set_kb_item( name: NASLString( "www/", port, "/phpScheduleIt" ), value: tmp_version );
		...
		info = NASLString( "phpScheduleIt Version '" );
