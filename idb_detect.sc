...
for dir in nasl_make_list_unique( "/idb", "/forum", "/board", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "Powered by <a [^>]+>iDB", string: buf, icase: TRUE )){
		...
		log_message( data: build_detection_report( app: "iDB", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
