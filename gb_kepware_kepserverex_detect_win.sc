...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "KEPServerEX" )){
			continue;
		}
		...
		register_and_report_cpe( app: "Kepware " + appName, ver: version, concluded: concluded, base: "cpe:/a:kepware:kepserverex:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
