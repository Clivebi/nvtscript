...
for dir in nasl_make_list_unique( "/", "/uebimiau", "/webmail", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "Uebimiau Webmail" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Uebimiau Webmail", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
