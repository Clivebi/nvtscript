...
for dir in nasl_make_list_unique( "/phpcoin", "/phpCoin165", "/", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && IsMatchRegexp( rcvRes, "php[Cc][Oo][Ii][Nn] [Ll]icense" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "phpCOIN", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
