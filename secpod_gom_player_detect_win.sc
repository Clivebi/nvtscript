...
if(gomVer){
	...
	cpe = build_cpe( value: gomVer, exp: "^([0-9.]+)", base: "cpe:/a:gomlab:gom_media_player:" );
	if(!cpe){
		cpe = "cpe:/a:gomlab:gom_media_player";
	}
	register_product( cpe: cpe, location: path );
	log_message( data: build_detection_report( app: "GOM Media Player", version: gomVer, install: path, cpe: cpe, concluded: gomVer ) );
}
