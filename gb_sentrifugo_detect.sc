...
for dir in nasl_make_list_unique( "/", "/sentrifugo", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( port: port, item: dir + "/index.php" );
	if(ContainsString( res, "index/loginpopupsave" ) && ContainsString( res, "loginpopdiv" ) && ContainsString( res, "hrms.js" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Sentrifugo", version: version, install: install, cpe: cpe, concluded: vers[0], concludedUrl: concUrl ), port: port );
		exit( 0 );
	}
}
exit( 0 );
