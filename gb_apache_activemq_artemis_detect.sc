...
if(egrep( pattern: "<title>ActiveMQ Artemis Web .* API</title>", string: buf, icase: TRUE ) || ContainsString( buf, "WWW-Authenticate: basic realm=\"ActiveMQ\"" )){
	install = "/";
	appVer = "unknown";
	...
	log_message( data: build_detection_report( app: "Apache ActiveMQ Artemis", install: install, version: appVer, concluded: version[0], concludedUrl: conclUrl, cpe: cpe ), port: port );
}
exit( 0 );
