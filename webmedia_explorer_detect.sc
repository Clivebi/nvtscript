...
for dir in nasl_make_list_unique( "/fileexplorer", "/explorer", "/wme", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "Powered by <a [^>]+>webmedia explorer", string: buf, icase: TRUE ) && egrep( pattern: "Set-Cookie: dir_stack_cookie", string: buf )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Webmedia Explorer", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
