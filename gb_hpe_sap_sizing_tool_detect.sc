...
if(!registry_key_exists( key: "SOFTWARE\\Hewlett Packard Enterprise\\Sizers\\SAP Sizing Tool" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\Hewlett Packard Enterprise\\Sizers\\SAP Sizing Tool" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HPE SAP Sizing Tool" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HPE SAP Sizing Tool", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
