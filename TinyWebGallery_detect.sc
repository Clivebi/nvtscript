...
for dir in nasl_make_list_unique( "/tinywebgallery", "/gallery", "/twg", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "TWG Administration", string: buf ) && egrep( pattern: "TWG Admin [0-9.]+", string: buf )){
		...
		set_kb_item( name: NASLString( "www/", port, "/TinyWebGallery" ), value: tmp_version );
		...
		info = NASLString( "TinyWebGallery Version '" );
