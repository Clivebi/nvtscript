...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !ContainsString( appName, "Cscape EnvisionRV" )){
			continue;
		}
		...
		register_and_report_cpe( app: "Horner APG " + appName, ver: version, concluded: concluded, base: "cpe:/a:hornerautomation:cscape_envisionrv:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
