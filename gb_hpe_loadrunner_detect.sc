...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HP LoadRunner" ) || ContainsString( hpName, "HPE LoadRunner" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HPE LoadRunner", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
