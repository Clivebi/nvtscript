...
rootInstalled = FALSE;
for dir in nasl_make_list_unique( "/", "/admin", "/admin/Public", "/Public", "/a2billing", "/a2billing/Public", "/a2billing/admin/Public", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "<title>..:: A2Billing Portal ::..</title>" )){
		if(install == "/"){
			rootInstalled = TRUE;
		}
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "A2billing", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
