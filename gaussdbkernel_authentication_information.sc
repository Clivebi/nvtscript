...
if(use_su && ContainsString( use_su, "yes" )){
	set_kb_item( name: "Policy/gaussdbkernel/use_su", value: "yes" );
	if( !isnull( su_user ) ){
		if( !su_user ){
			replace_kb_item( name: "Policy/gaussdbkernel/use_su", value: "no" );
		}
		else {
			set_kb_item( name: "Policy/gaussdbkernel/su_user", value: su_user );
		}
	}
	else {
		replace_kb_item( name: "Policy/gaussdbkernel/use_su", value: "no" );
