...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "Profi cash" )){
			continue;
		}
		version = "unknown";
		concluded = appName;
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:fiducia_gad_it_ag:profi_cash:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
