...
filter = NASLString( "icmp and src host ", get_host_ip(), " and dst host ", host, " and icmp[0:1] = 14" );
for(i = 0;i < 5;i++){
	res = send_packet( packet: icmp, pcap_active: TRUE, pcap_filter: filter, pcap_timeout: 1 );
	if(res){
