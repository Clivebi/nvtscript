...
if(!registry_key_exists( key: "SOFTWARE\\Hewlett Packard Enterprise\\Sizers\\ConvergedSystems Virtualization Sizing Tool" ) && !registry_key_exists( key: "SOFTWARE\\Wow6432Node\\Hewlett Packard Enterprise\\Sizers\\ConvergedSystems Virtualization Sizing Tool" )){
	exit( 0 );
}
...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HPE Sizer for ConvergedSystems Virtualization" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HPE Sizer for ConvergedSystems Virtualization", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
