...
for dir in nasl_make_list_unique( "/crawltrack", "/crawler", "/", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( rcvRes, ">CrawlTrack informations<" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "CrawlTrack", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
