if(description){
	...
	script_dependencies( "find_service1.sc", "slident.sc", "secpod_open_tcp_ports.sc" );
...
list = service_get_ports( default_port_list: make_list( 113 ), proto: "auth" );
for iport in list {
		...
		if(get_port_state( port ) && !get_kb_item( "ident/tcp" + port )){
			...
			if(soc){
				...
				if( res && ContainsString( res, "USERID" ) ){
					_res = split( buffer: res, sep: ":", keep: FALSE );
					if( max_index( _res ) > 2 ){
						os = chomp( _res[2] );
						os = ereg_replace( string: os, pattern: "^(\\s+)", replace: "" );
						id = chomp( _res[3] );
						id = ereg_replace( string: id, pattern: "^(\\s+)", replace: "" );
						if(ContainsString( _res[1], "USERID" ) && strlen( id ) && strlen( id ) < 30){
							identd_n++;
