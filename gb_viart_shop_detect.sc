...
for dir in nasl_make_list_unique( "/shop", "/viart", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<Program_Name>ViArt Shop", string: buf, icase: TRUE )){
		...
		set_kb_item( name: NASLString( "www/", port, "/viart_shop" ), value: NASLString( vers, " under ", install ) );
		...
		log_message( data: build_detection_report( app: "ViArt Shop", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
