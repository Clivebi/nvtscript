if(description){
	...
	script_dependencies( "smtpserver_detect.sc", "smtp_settings.sc", "global_settings.sc" );
...
if(!is_public_addr()){
	if(debug){
		display( "DEBUG: Target-IP \"" + get_host_ip() + "\" is not a public address. Exiting..." );
	}
	exit( 0 );
}
...
FROM = NASLString( vtstrings["lowercase"], "@", src_name );
TO = NASLString( vtstrings["lowercase"], "@", domain );
target = get_host_name();
port = smtp_get_port( default: 25 );
if(get_kb_item( "smtp/" + port + "/qmail/detected" )){
	if(debug){
		display( "DEBUG: QMail detected. Exiting..." );
	}
	exit( 0 );
}
if(smtp_get_is_marked_wrapped( port: port )){
	if(debug){
		display( "DEBUG: SMTP service is marked as \"wrapped\". Exiting..." );
	}
	exit( 0 );
}
...
if(!soc){
	if( debug && __smtp_open_helo_ehlo_sent && __smtp_open_helo_ehlo_recv ) {
		display( "DEBUG: SMTP service is not accepting our HELO:\n\n" + __smtp_open_helo_ehlo_sent + "\n\nAnswer received:\n\n" + __smtp_open_helo_ehlo_recv + "\n\n Exiting..." );
	}
	else {
		if( debug && __smtp_open_helo_ehlo_sent ) {
			display( "DEBUG: SMTP service is not accepting our HELO:\n\n" + __smtp_open_helo_ehlo_sent + "\n\nNo answer received. Exiting..." );
		}
		else {
			if(debug){
				display( "DEBUG: Failed to open a connection to the SMTP service. Exiting..." );
			}
		}
	}
	exit( 0 );
...
mfres = l;
if(debug){
	display( "DEBUG: SMTP response received:\n\n" + str_replace( string: mfres, find: "\r\n", replace: "<CR><LF>" ) );
}
...
dc = NASLString( "Subject: ", vtstrings["default"], "-Relay-Test\\r\\n" );
dc += NASLString( "To: ", vtstrings["default"], "-Relay-Test <", TO, ">\\r\\n" );
dc += NASLString( "From: ", vtstrings["default"], "-Relay-Test <", FROM, ">\\r\\n\\r\\n" );
dc += NASLString( "This is a ", vtstrings["default"], "-Relay-Test to test the mail server at:\\r\\n\\r\\n", target, "\\r\\n\\r\\nif it is configured as an open mail relay.\\r\\n\\r\\n" );
dc += NASLString( "If you have received this message please forward it to the administrator of this mail server and ask to protect it against mail relaying." );
dc += NASLString( "\\r\\n.\\r\\n" );
send( socket: soc, data: dc );
l = smtp_recv_line( socket: soc );
smtp_close( socket: soc, check_data: l );
if( l && IsMatchRegexp( l, "^250[ -].+" ) ){
	if(debug){
		display( "DEBUG: SMTP response received:\n\n" + str_replace( string: l, find: "\r\n", replace: "<CR><LF>" ) );
	}
	report = "SMTP banner:\n\n";
