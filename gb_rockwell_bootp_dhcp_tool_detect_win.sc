...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "BootP-DHCP Tool" )){
			continue;
		}
		concluded = appName;
		location = "unknown";
		...
		register_and_report_cpe( app: "Rockwell Automation " + appName, ver: version, concluded: concluded, base: "cpe:/a:rockwellautomation:bootp_dhcp_tool:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
