...
for dir in nasl_make_list_unique( "/", "/discoverer", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( res, ">OracleBI Discoverer" ) && ContainsString( res, ">Oracle Technology" )){
		set_kb_item( name: "OracleBI Discoverer/installed", value: TRUE );
		ver = eregmatch( string: res, pattern: "Version ([0-9.]+)" );
		if(ver[1]){
			set_kb_item( name: NASLString( "www/", port, "/OracleBIDiscoverer" ), value: NASLString( ver[1], " under ", install ) );
			...
			log_message( data: build_detection_report( app: "OracleBI Discoverer", version: ver[1], install: install, cpe: cpe, concluded: ver[1] ), port: port );
		}
	}
}
