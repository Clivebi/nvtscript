if(description){
	...
	script_dependencies( "find_service.sc", "cifs445.sc" );
...
func smb_neg_prot_smb_1_check( target, port ){
	var target, port, soc, r, neg_prot;
	...
	neg_prot = raw_string( 0x00, 0x00, 0x00, 0xbe, 0xff, 0x53, 0x4d, 0x42, 0x72, 0x00, 0x00, 0x00, 0x00, 0x08, 0x01, 0xc8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x33, 0x0c, 0x00, 0x00, 0x00, 0x00 );
...
func smb_neg_prot_smb2_check( target, port ){
	var target, port, soc, r, neg_prot;
	soc = open_sock_tcp( port );
	if(!soc){
		return FALSE;
	...
	neg_prot = raw_string( 0x00, 0x00, 0x00, 0xd4, 0xff, 0x53, 0x4d, 0x42, 0x72, 0x00, 0x00, 0x00, 0x00, 0x08, 0x01, 0xc8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x33, 0x0c, 0x00, 0x00, 0x00, 0x00 );
...
func smb_neg_prot_smb_2_3_check( target, port ){
	var target, port, soc, r, neg_prot;
	soc = open_sock_tcp( port );
	if(!soc){
		return FALSE;
	...
	neg_prot = raw_string(  );
	send( socket: soc, data: neg_prot );
	r = smb_recv( socket: soc );
	close( soc );
	if(strlen( r ) < 74){
...
if( smb1_support && smb2_support && smb3_support ){
	...
	if( smb2_support && smb3_support && !smb1_support ){
		...
		if( smb1_support && smb2_support && !smb3_support ){
			...
			if( smb1_support && smb3_support && !smb2_support ){
				...
				if( smb1_support && !smb2_support && !smb3_support ){
					...
					if( smb2_support && !smb1_support && !smb3_support ){
						report = "Only SMBv2 is enabled on remote target";
					}
					else {
						if( smb3_support && !smb1_support && !smb2_support ){
