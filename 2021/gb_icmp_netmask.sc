if(description){
	...
	script_tag( name: "last_modification", value: "2021-08-05 07:12:10 +0000 (Thu, 05 Aug 2021)" );
	...
	script_dependencies( "os_fingerprint.sc", "global_settings.sc" );
...
icmp_id = rand() % 65536;
ip = forge_ip_packet( ip_hl: 5, ip_v: 4, ip_off: 0, ip_id: 9, ip_tos: 0, ip_p: IPPROTO_ICMP, ip_len: 20, ip_src: own_ip, ip_ttl: 255 );
icmp = forge_icmp_packet( ip: ip, icmp_type: icmp_addrmask_request, icmp_code: 0, icmp_seq: 1, icmp_id: icmp_id, data: crap( length: 4, data: raw_string( 0 ) ) );
filter = NASLString( "icmp and src host ", target_ip, " and dst host ", own_ip, " and icmp[0:1] = ", icmp_addrmask_reply );
for(i = 0;i < 5;i++){
