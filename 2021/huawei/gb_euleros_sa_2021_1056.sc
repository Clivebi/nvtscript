if(description){
	...
	script_xref( name: "Advisory-ID", value: "EulerOS-SA-2021-1056" );
	...
	script_tag( name: "summary", value: "The remote host is missing an update for the Huawei EulerOS 'kernel' package(s) announced via the EulerOS-SA-2021-1056 advisory." );
...
The Linux kernel through 5.7.11 allows remote attackers to make observations that help to obtain ... [Please see the references for more information on the vulnerabilities]" );
...
if(release == "EULEROSVIRT-3.0.2.6"){
	if(!isnull( res = isrpmvuln( pkg: "kernel", rpm: "kernel~3.10.0~862.14.1.6_81", rls: "EULEROSVIRT-3.0.2.6" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "kernel-devel", rpm: "kernel-devel~3.10.0~862.14.1.6_81", rls: "EULEROSVIRT-3.0.2.6" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "kernel-headers", rpm: "kernel-headers~3.10.0~862.14.1.6_81", rls: "EULEROSVIRT-3.0.2.6" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "kernel-tools", rpm: "kernel-tools~3.10.0~862.14.1.6_81", rls: "EULEROSVIRT-3.0.2.6" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "kernel-tools-libs", rpm: "kernel-tools-libs~3.10.0~862.14.1.6_81", rls: "EULEROSVIRT-3.0.2.6" ) )){
		report += res;
	}
	if( report != "" ){
