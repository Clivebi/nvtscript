if(description){
	...
	script_tag( name: "summary", value: "Plex Media Server installations in a specific (and uncommon)
  network position could potentially be used to reflect UDP traffic on certain device-discovery
  ports as part of a possible DDoS (distributed denial-of-service) attack." );
	script_tag( name: "insight", value: "Plex Media Server instances which have either been deployed on a
  public-facing network DMZ or in an Internet Data Center (IDC), or with manually configured
  port-forwarding rules which forward specific UDP ports from the public Internet to devices running
  Plex Media Server, can potentially be abused as part of possible DDoS attacks.

  These actions can have the effect of exposing a Plex UPnP-enabled service registration responder
  to the general Internet, where it can be abused to generate reflection/amplification DDoS
  attacks." );
  ...
  Global variable settings (OID: 1.3.6.1.4.1.25623.1.0.12288)" );
	...
	script_tag( name: "solution", value: "Update to version 1.21.3.4014 or later. For mitigation steps
  see the referenced vendor response." );
