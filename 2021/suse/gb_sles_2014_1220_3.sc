if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2014.1220.3" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2014:1220-3)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES10\\.0SP3|SLES11\\.0SP1)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2014:1220-3" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2014/suse-su-20141220-3/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'mozilla-nss' package(s) announced via the SUSE-SU-2014:1220-3 advisory." );
 ...
 * CVE-2014-1568" );
	script_tag( name: "affected", value: "'mozilla-nss' package(s) on SUSE Linux Enterprise Server 10 SP3, SUSE Linux Enterprise Server 11 SP1." );
...
if(release == "SLES10.0SP3"){
	if(!isnull( res = isrpmvuln( pkg: "mozilla-nss", rpm: "mozilla-nss~3.16.5~0.5.1", rls: "SLES10.0SP3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "mozilla-nss-32bit", rpm: "mozilla-nss-32bit~3.16.5~0.5.1", rls: "SLES10.0SP3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "mozilla-nss-devel", rpm: "mozilla-nss-devel~3.16.5~0.5.1", rls: "SLES10.0SP3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "mozilla-nss-tools", rpm: "mozilla-nss-tools~3.16.5~0.5.1", rls: "SLES10.0SP3" ) )){
		report += res;
	}
	if( report != "" ){
