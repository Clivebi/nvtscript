if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2020.1695.2" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2020:1695-2)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES15\\.0SP2)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2020:1695-2" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2020/suse-su-20201695-2/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'osc' package(s) announced via the SUSE-SU-2020:1695-2 advisory." );
...
Added --ccache option to osc getbinaries" );
	script_tag( name: "affected", value: "'osc' package(s) on SUSE Linux Enterprise Module for Development Tools 15-SP2." );
...
if(release == "SLES15.0SP2"){
	if(!isnull( res = isrpmvuln( pkg: "osc", rpm: "osc~0.169.1~3.20.1", rls: "SLES15.0SP2" ) )){
		report += res;
	}
	if( report != "" ){
