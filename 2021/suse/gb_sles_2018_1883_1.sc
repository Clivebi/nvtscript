if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2018.1883.1" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2018:1883-1)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES15\\.0)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2018:1883-1" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2018/suse-su-20181883-1/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'unzip' package(s) announced via the SUSE-SU-2018:1883-1 advisory." );
...
+- Allow processing of Windows zip64 archives (Windows archivers set
 ...
 (bnc#910683)" );
	script_tag( name: "affected", value: "'unzip' package(s) on SUSE Linux Enterprise Module for Basesystem 15." );
...
report = "";
if(release == "SLES15.0"){
	if(!isnull( res = isrpmvuln( pkg: "unzip", rpm: "unzip~6.00~4.3.1", rls: "SLES15.0" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "unzip-debuginfo", rpm: "unzip-debuginfo~6.00~4.3.1", rls: "SLES15.0" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "unzip-debugsource", rpm: "unzip-debugsource~6.00~4.3.1", rls: "SLES15.0" ) )){
		report += res;
	}
	if( report != "" ){
