if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2020.0493.1" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2020:0493-1)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES15\\.0SP1)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2020:0493-1" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2020/suse-su-20200493-1/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'squid' package(s) announced via the SUSE-SU-2020:0493-1 advisory." );
...
Improved cache handling with chunked responses." );
	script_tag( name: "affected", value: "'squid' package(s) on SUSE Linux Enterprise Module for Server Applications 15-SP1." );
...
if(release == "SLES15.0SP1"){
	if(!isnull( res = isrpmvuln( pkg: "squid", rpm: "squid~4.10~5.14.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "squid-debuginfo", rpm: "squid-debuginfo~4.10~5.14.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "squid-debugsource", rpm: "squid-debugsource~4.10~5.14.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if( report != "" ){
