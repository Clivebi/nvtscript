if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2014.0335.1" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2014:0335-1)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES10\\.0SP3)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2014:0335-1" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2014/suse-su-20140335-1/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'openssl-certs' package(s) announced via the SUSE-SU-2014:0335-1 advisory." );
 ...
 * revoke mis-issued intermediate CAs from TURKTRUST" );
	script_tag( name: "affected", value: "'openssl-certs' package(s) on SUSE Linux Enterprise Server 10 SP3." );
...
if(release == "SLES10.0SP3"){
	if(!isnull( res = isrpmvuln( pkg: "openssl-certs", rpm: "openssl-certs~1.96~0.6.1", rls: "SLES10.0SP3" ) )){
		report += res;
	}
	if( report != "" ){
