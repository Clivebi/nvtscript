if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2013.0595.1" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2013:0595-1)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES10\\.0SP4)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2013:0595-1" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2013/suse-su-20130595-1/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'poppler' package(s) announced via the SUSE-SU-2013:0595-1 advisory." );
 ...
 * CVE-2013-1790
>" );
	script_tag( name: "affected", value: "'poppler' package(s) on SLE SDK 10 SP4, SUSE Linux Enterprise Desktop 10 SP4, SUSE Linux Enterprise Server 10 SP4." );
...
if(release == "SLES10.0SP4"){
	if(!isnull( res = isrpmvuln( pkg: "poppler", rpm: "poppler~0.4.4~19.29.1", rls: "SLES10.0SP4" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "poppler-glib", rpm: "poppler-glib~0.4.4~19.29.1", rls: "SLES10.0SP4" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "poppler-qt", rpm: "poppler-qt~0.4.4~19.29.1", rls: "SLES10.0SP4" ) )){
		report += res;
	}
	if( report != "" ){
