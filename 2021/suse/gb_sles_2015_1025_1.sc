if(description){
	...
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2015/suse-su-20151025-1/" );
	...
	script_tag( name: "insight", value: "This update for xorg-x11-server fixes a regression introduced with the fix for CVE-2014-8092:
...
if(release == "SLES11.0SP3"){
	if(!isnull( res = isrpmvuln( pkg: "xorg-x11-Xvnc", rpm: "xorg-x11-Xvnc~7.4~27.105.1", rls: "SLES11.0SP3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "xorg-x11-server", rpm: "xorg-x11-server~7.4~27.105.1", rls: "SLES11.0SP3" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "xorg-x11-server-extra", rpm: "xorg-x11-server-extra~7.4~27.105.1", rls: "SLES11.0SP3" ) )){
		report += res;
	}
	if( report != "" ){
