if(description){
	script_oid( "1.3.6.1.4.1.25623.1.1.4.2019.2913.1" );
	...
	script_name( "SUSE: Security Advisory (SUSE-SU-2019:2913-1)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2021 Greenbone Networks GmbH" );
	script_family( "SuSE Local Security Checks" );
	script_dependencies( "gather-package-list.sc" );
	script_mandatory_keys( "ssh/login/suse_sles", "ssh/login/rpms",  "ssh/login/release=(SLES15\\.0SP1)" );
	script_xref( name: "Advisory-ID", value: "SUSE-SU-2019:2913-1" );
	script_xref( name: "URL", value: "https://www.suse.com/support/update/announcement/2019/suse-su-20192913-1/" );
	script_tag( name: "summary", value: "The remote host is missing an update for the 'gdb' package(s) announced via the SUSE-SU-2019:2913-1 advisory." );
...
Handle most of new s390 arch13 instructions. [fate#327369, jsc#ECO-368]" );
	script_tag( name: "affected", value: "'gdb' package(s) on SUSE Linux Enterprise Module for Development Tools 15-SP1, SUSE Linux Enterprise Module for Open Buildservice Development Tools 15-SP1." );
...
if(release == "SLES15.0SP1"){
	if(!isnull( res = isrpmvuln( pkg: "gdb", rpm: "gdb~8.3.1~8.8.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "gdb-debuginfo", rpm: "gdb-debuginfo~8.3.1~8.8.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "gdb-debugsource", rpm: "gdb-debugsource~8.3.1~8.8.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "gdbserver", rpm: "gdbserver~8.3.1~8.8.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if(!isnull( res = isrpmvuln( pkg: "gdbserver-debuginfo", rpm: "gdbserver-debuginfo~8.3.1~8.8.1", rls: "SLES15.0SP1" ) )){
		report += res;
	}
	if( report != "" ){
