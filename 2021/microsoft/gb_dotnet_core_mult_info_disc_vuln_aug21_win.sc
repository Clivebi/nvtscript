...
path = infos["location"];
if(!coreVers || !IsMatchRegexp( coreVers, "^([3|2]\\.1|5\\.0)" )){
	exit( 0 );
}
...
if( corerunVer ){
	if( version_in_range( version: corerunVer, test_version: "5.0", test_version2: "5.0.8" ) ){
		...
		if( version_in_range( version: corerunVer, test_version: "3.1", test_version2: "3.1.17" ) ){
			fix = "3.1.18";
		}
		else {
			if(version_in_range( version: corerunVer, test_version: "2.1", test_version2: "2.1.28" )){
				fix = "2.1.29";
			}
		}
	}
	...
	if(codesdkVer){
		if( version_in_range( version: codesdkVer, test_version: "5.0", test_version2: "5.0.205" ) ){
			...
			if( version_in_range( version: codesdkVer, test_version: "3.1", test_version2: "3.1.117" ) ){
				fix1 = "3.1.118 (for Visual Studio 2019 v16.4) or 3.1.412 (for Visual Studio 2019 v16.7 or later) ";
			}
			else {
				if(version_in_range( version: codesdkVer, test_version: "2.1", test_version2: "2.1.524" )){
...
if( fix ){
	report = report_fixed_ver( installed_version: "ASP .NET Core With Microsoft .NET Core runtimes " + corerunVer, fixed_version: "ASP .NET Core With Microsoft .NET Core runtimes version " + fix, install_path: path );
	security_message( data: report );
	exit( 0 );
}
else {
	if(fix1){
		report = report_fixed_ver( installed_version: "ASP .NET Core With Microsoft .NET Core SDK " + codesdkVer, fixed_version: "ASP .NET Core With Microsoft .NET Core SDK version " + fix1, install_path: path );
		security_message( data: report );
		exit( 0 );
	}
}
