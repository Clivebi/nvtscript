...
for dir in nasl_make_list_unique( "/admin", http_cgi_dirs( port: appPort ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/", port: appPort );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "<title>EvolutionScript - Login</title>" )){
		Ver = "unknown";
		...
		log_message( data: build_detection_report( app: "Evolution Script CMS", version: Ver, install: install, cpe: cpe, concluded: tmpVer[0] ), port: appPort );
	}
}
exit( 0 );
