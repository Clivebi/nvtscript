if(description){
	...
	script_tag( name: "summary", value: "This script finds the running Flashlight version." );
...
for dir in nasl_make_list_unique( "/", "/flashlight", "/flash", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/README.txt", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "Flashlight Free Edition" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Flash light", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
