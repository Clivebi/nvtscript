if(description){
	...
	script_tag( name: "last_modification", value: "2021-06-01 14:33:10 +0000 (Tue, 01 Jun 2021)" );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!app_name || !IsMatchRegexp( app_name, "PuTTY Session Manager" )){
			continue;
		}
		...
		register_and_report_cpe( app: app_name, ver: version, concluded: concluded, base: "cpe:/a:putty_session_manager:putty_session_manager:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
