...
if(description){
  ...
  SSL-VPN 200, SSL-VPN 2000 and SSL-VPN 4000 devices which are detected and reported as well." );
	...
	script_dependencies( "gb_dell_sonicwall_sma_sra_snmp_detect.sc" );
	if(FEED_NAME == "GSF" || FEED_NAME == "SCM"){
		script_dependencies( "gsf/gb_dell_sonicwall_sma_sra_http_detect.sc" );
	}
...
if( detected_product != "unknown" ){
	...
	if( detected_series != "unknown" ){
		...
		if( detected_series != "Virtual Appliance" ) {
			hw_cpe = "cpe:/h:sonicwall:" + tolower( detected_product ) + "_" + tolower( str_replace( string: detected_series, find: " ", replace: "_" ) );
		}
		else {
			app_cpe = "cpe:/a:sonicwall:" + tolower( detected_product ) + "_" + tolower( str_replace( string: detected_series, find: " ", replace: "_" ) );
		}
	}
	else {
...
os_register_and_report( os: os_name + " Firmware", cpe: os_cpe, desc: "SonicWall / Dell SonicWALL SRA / SMA Detection Consolidation", runs_key: "unixoide" );
report = build_detection_report( app: os_name + " Firmware", version: detected_version, install: location, cpe: os_cpe );
if(hw_cpe){
	report += "\n\n" + build_detection_report( app: os_name + " Appliance", skip_version: TRUE, install: location, cpe: hw_cpe );
}
if(app_cpe){
	report += "\n\n" + build_detection_report( app: os_name, skip_version: TRUE, install: location, cpe: app_cpe );
}
if(extra){
	report += "\n\nDetection methods:\n";
	report += "\r\n" + extra;
