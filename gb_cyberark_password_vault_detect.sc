...
if(!IsMatchRegexp( res, "^HTTP/1\\.[01] 302" ) || !ContainsString( res, "ASP.NET_SessionId=" ) || !IsMatchRegexp( res, "Location: /PasswordVault/auth/(ldap|radius|cyberark|pki)" )){
	exit( 0 );
}
...
res = http_keepalive_send_recv( port: port, data: req );
if(!ContainsString( res, "CyberArk Password Vault" )){
	exit( 0 );
}
version = "unknown";
...
log_message( data: build_detection_report( app: "CyberArk Enterprise Password Vault", version: version, install: install, cpe: cpe, concluded: vers[0], concludedUrl: concUrl ), port: port );
exit( 0 );
