...
install = port + "/tcp";
version = "unknown";
vers = eregmatch( string: found_banner, pattern: "Server\\s*:.*mod_python/([0-9.]+)", icase: TRUE );
if(vers[1]){
	version = vers[1];
}
...
log_message( data: build_detection_report( app: "Apache mod_python", version: version, install: install, cpe: cpe, concludedUrl: conclurl, concluded: concluded ), port: port );
exit( 0 );
