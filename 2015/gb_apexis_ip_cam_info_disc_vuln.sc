...
for dir in nasl_make_list_unique( "/", "/cgi-bin", http_cgi_dirs( port: apexCamPort ) ) {
	if(dir == "/"){
		dir = "";
	}
	sndReq = http_get( item: NASLString( dir, "/get_status.cgi" ), port: apexCamPort );
	rcvRes = http_keepalive_send_recv( port: apexCamPort, data: sndReq );
	if(ContainsString( rcvRes, "ret_prot_mode='APM-H" )){
		url = dir + "/get_tutk_account.cgi";
		if(http_vuln_check( port: apexCamPort, url: url, check_header: TRUE, pattern: "ret_tutk_user=", extra_check: "ret_tutk_pwd=" )){
