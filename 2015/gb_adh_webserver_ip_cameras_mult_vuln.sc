...
if(ContainsString( rcvRes, "ipCamera" ) && ContainsString( rcvRes, "Server: ADH-Web" )){
	url = "/variable.cgi?variable=camconfig[0]&slaveip=127.0.0.1";
	if(http_vuln_check( port: ipCamPort, url: url, check_header: TRUE, pattern: "telm_cam_protocol=", extra_check: make_list( "supported_streams=",
		 "aspect_ratio=",
		 "lens_type=" ) )){
