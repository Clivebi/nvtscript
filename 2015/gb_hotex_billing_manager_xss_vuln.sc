...
for dir in nasl_make_list_unique( "/", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( rcvRes, "> Login<" ) && ContainsString( rcvRes, "hotspot_popup" )){
		url = "/cgi-bin/hotspotlogin.cgi?res=failed&reply=" + "<script>alert%28document.cookie%29<%2fscript>" + "%2c%20Invalid%20username%20or%20Password";
		if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "_script_alert\\(document\\.cookie\\)_/script_", extra_check: "> Login<" )){
