...
if( IsMatchRegexp( dllVer, "^(6\\.0\\.6002\\.2)" ) ){
	...
	if( IsMatchRegexp( dllVer, "^(6\\.0\\.6002\\.)" ) ){
		...
		if( IsMatchRegexp( dllVer, "^(6\\.1\\.7601\\.2)" ) ){
			...
			if( IsMatchRegexp( dllVer, "^(6\\.1\\.7601\\.)" ) ){
				...
				if( IsMatchRegexp( dllVer, "^(6\\.2\\.9200\\.2)" ) ){
					...
					if( IsMatchRegexp( dllVer, "^(6\\.2\\.9200\\.)" ) ){
						Vulnerable_range = "Less than 6.2.9200.17467";
					}
					else {
						if(IsMatchRegexp( dllVer, "^(6\\.3\\.9600\\.)" )){
...
if( hotfix_check_sp( winVista: 3, win2008: 3 ) > 0 ){
	if(version_is_less( version: dllVer, test_version: "6.0.6002.19465" ) || version_in_range( version: dllVer, test_version: "6.0.6002.23000", test_version2: "6.0.6002.23773" )){
		VULN = TRUE;
	}
}
else {
	if( hotfix_check_sp( win7: 2, win7x64: 2, win2008r2: 2 ) > 0 ){
		if(version_is_less( version: dllVer, test_version: "6.1.7601.18951" ) || version_in_range( version: dllVer, test_version: "6.1.7601.22000", test_version2: "6.1.7601.23153" )){
			VULN = TRUE;
		}
	}
	else {
		if( hotfix_check_sp( win8: 1, win8x64: 1, win2012: 1 ) > 0 ){
			if(version_is_less( version: dllVer, test_version: "6.2.9200.17467" ) || version_in_range( version: dllVer, test_version: "6.2.9200.20000", test_version2: "6.2.9200.21580" )){
				VULN = TRUE;
			}
		}
		else {
