if(description){
	...
	script_dependencies( "gb_cisco_wsa_web_detect.sc" );
...
if(!ContainsString( res, "authenticated" )){
	exit( 99 );
}
...
cookie = ac[1] + cookie;
if(http_vuln_check( port: port, url: "/monitor/wsa_user_report", pattern: "action=Logout", extra_check: make_list( "System Status",
	 "Logged in as" ), cookie: cookie )){
