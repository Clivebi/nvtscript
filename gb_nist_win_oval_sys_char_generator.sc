if(description){
	...
	script_add_preference( name: "Create OVAL System Characteristics for NIST Windows OVAL Definitions", type: "checkbox", value: "no", id: 1 );
...
create_sc = script_get_preference( name: "Create OVAL System Characteristics for NIST Windows OVAL Definitions", id: 1 );
if(create_sc == "no"){
	exit( 0 );
}
var sys_char_id;
...
func get_system_info_using_wmi( host_ip ){
	sys_info = "";
	...
	query = "SELECT IPAddress, MacAddress, Description FROM Win32_NetworkAdapter" + "Configuration Where IPEnabled = True";
	net_adpt_info = wmi_query( wmi_handle: handle, query: query );
	net_adpt_info_list = split( buffer: net_adpt_info, sep: "\n", keep: FALSE );
	for interface_info in net_adpt_info_list {
		if(ContainsString( interface_info, host_ip )){
			interface_info_list = split( buffer: interface_info, sep: "|", keep: FALSE );
	...
	sys_info = make_list( host_ip,
		 interface_name,
		 mac_address );
	return ( sys_info );
}
func create_registry_system_data_char_xml( reg_key, reg_item, reg_type, reg_hive ){
	var status, reg_xml, reg_value, reg_hive, ret_list, key_exists;
	status = "";
	reg_xml = "";
	reg_value = "";
	if(!reg_hive){
	...
	if( !key_exists ){
		...
		reg_xml = "\t\t<registry_item" + status + " xmlns=\"http://oval.mitre.org/" + "XMLSchema/oval-system-characteristics-5#windows\" id=\"" + sys_char_id + "\">";
...
func create_collected_obj_xml( comment, flag, obj_id, version, item_ref, variable_id, variable_value ){
	var coll_obj_xml;
	coll_obj_xml = "";
	if( comment ){
...
func create_file_item_sys_data_xml( path, file_name ){
	var status, file_xml, file_ver, c_path, ret_list;
	status = "";
	file_xml = "";
	file_ver = "";
	sys_char_id = sys_char_id + 1;
	if(!path || !file_name){
		ret_list = make_list( file_xml,
			 file_ver,
			 sys_char_id );
		return ( ret_list );
	}
	...
	file_xml = "\t\t<file_item" + status + " xmlns=\"http://oval.mitre.org/XMLSchema/" + "oval-system-characteristics-5#windows\" id=\"" + sys_char_id + "\">";
...
complete_xml = NASLString( complete_xml, "\t<generator>\t\t<oval:product_name>", PLUGIN_FEED, "</oval:product_name>\t\t<oval:product_version>", PLUGIN_SET, "</oval:product_version>\t\t<oval:schema_version>5.10</oval:schema_", "version>\t\t<oval:timestamp>", l_time["year"], "-", month, "-", day, "T", hour, ":", minute, ":", sec, "</oval:timestamp>\t\t<vendor>", FEED_VENDOR, "</vendor>\t</generator>" );
sys_data_xml = NASLString( sys_data_xml, "\\t<system_data>" );
collected_obj_xml = NASLString( collected_obj_xml, "\t<collected_objects>" );
if( get_kb_item( "SMB/WindowsVersion" ) ){
	...
	system_info_xml = NASLString( "\\t<system_info>\\t\\t<os_name>", win_os_comp_name, "</os_name>", "\\t\\t<os_version>", win_os_ver, "</os_version>", "\\t\\t<architecture>", win_arch, "</architecture>", "\\t\\t<primary_host_name>", host_name, "</primary_host_name>", "\\t\\t<interfaces>\\t\\t\\t<interface>", "\\t\\t\\t\\t<interface_name>", interface_name, "</interface_name>", "\\t\\t\\t\\t<ip_address>", host_ip, "</ip_address>", "\\t\\t\\t\\t<mac_address>", mac_address, "</mac_address>", "\\t\\t\\t</interface>\\t\\t</interfaces>", "</system_info>" );
	...
	if( ms_exchange_keyExists ){
		...
		if(msi_prd_mjr_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, msi_prd_mjr_sys_data_xml );
		}
		...
		if(msi_prd_min_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, msi_prd_min_sys_data_xml );
		}
		...
		if(msi_build_mjr_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, msi_build_mjr_sys_data_xml );
		}
		...
		if(msi_build_min_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, msi_build_min_sys_data_xml );
		}
		...
		ms_exchange_key_xml = "<registry_item  status=\"does not exist\" xmlns=\"http:" + "//oval.mitre.org/XMLSchema/oval-system-characteristi" + "cs-5#windows\" id=\"" + ms_exchange_item_ref + "\">";
		...
		sys_data_xml = NASLString( sys_data_xml, ms_exchange_key_xml );
	...
	if(msi_prd_mjr_coll_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, msi_prd_mjr_coll_obj_xml );
	}
	if(msi_prd_min_coll_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, msi_prd_min_coll_obj_xml );
	}
	if(msi_build_mjr_coll_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, msi_build_mjr_coll_obj_xml );
	}
	if(msi_build_min_coll_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, msi_build_min_coll_obj_xml );
	}
	...
	if(ie_reg_version_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, ie_reg_version_sys_data_xml );
	}
	...
	if(ie_reg_version_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, ie_reg_version_obj_xml );
	}
	...
	if(processor_arch_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, processor_arch_sys_data_xml );
	}
	...
	if(processor_arch_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, processor_arch_obj_xml );
	}
	...
	if(csd_version_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, csd_version_sys_data_xml );
	}
	...
	if(csd_version_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, csd_version_obj_xml );
	}
	...
	if(product_os_name_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, product_os_name_sys_data_xml );
	}
	...
	if(product_os_name_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, product_os_name_obj_xml );
	}
	...
	if(common_files_dir_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, common_files_dir_sys_data_xml );
	}
	...
	if(common_files_dir_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, common_files_dir_obj_xml );
	}
	if(common_files_dir_value){
		...
		if(vgx_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, vgx_file_sys_data_xml );
		}
		if(vgx_file_value){
		...
		if(vgx_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, vgx_file_obj_xml );
		}
	}
	...
	if(system_root_sys_data_xml){
		sys_data_xml = NASLString( sys_data_xml, system_root_sys_data_xml );
	}
	...
	if(system_root_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, system_root_obj_xml );
	}
	if(system_root_value){
		...
		if(win32k_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, win32k_file_sys_data_xml );
		}
		...
		if(win32k_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, win32k_file_obj_xml );
		}
		...
		if(user32_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, user32_file_sys_data_xml );
		}
		...
		if(user32_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, user32_file_obj_xml );
		}
		...
		if(csrsrv_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, csrsrv_file_sys_data_xml );
		}
		...
		if(csrsrv_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, csrsrv_file_obj_xml );
		}
		...
		if(winsrv_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, winsrv_file_sys_data_xml );
		}
		...
		if(winsrv_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, winsrv_file_obj_xml );
		}
		...
		if(mshtml_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, mshtml_file_sys_data_xml );
		}
		...
		if(mshtml_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, mshtml_file_obj_xml );
		}
		...
		if(iexplore_file_sys_data_xml){
			sys_data_xml = NASLString( sys_data_xml, iexplore_file_sys_data_xml );
		}
		...
		if(iexplore_file_obj_xml){
			collected_obj_xml = NASLString( collected_obj_xml, iexplore_file_obj_xml );
		}
	}
	sys_char_id = sys_char_id + 1;
	family_xml = "\t\t<family_item xmlns=\"http://oval.mitre.org/XMLSchema/oval-sys" + "tem-characteristics-5#independent\" id=\"" + sys_char_id + "\">";
	family_xml = NASLString( family_xml, "\\t\\t\\t<family>windows</family>" );
	family_xml = NASLString( family_xml, "\\t\\t</family_item>" );
	sys_data_xml = NASLString( sys_data_xml, family_xml );
	...
	if(family_obj_xml){
		collected_obj_xml = NASLString( collected_obj_xml, family_obj_xml );
	}
}
else {
...
collected_obj_xml = NASLString( collected_obj_xml, "\\t</collected_objects>" );
if(!system_info_xml){
	system_info_xml = NASLString( "\\t<system_info>\\t\\t<os_name></os_name>\\t\\t<os_version>", "</os_version>\\t\\t<architecture></architecture>\\t\\t<primary_host_name>", "</primary_host_name>\\t\\t<interfaces>\\t\\t\\t<interface>\\t\\t\\t\\t<interfa", "ce_name></interface_name>\\t\\t\\t\\t<ip_address></ip_address>\\t\\t\\t\\t", "<mac_address></mac_address>\\t\\t\\t</interface>\\t\\t</interfaces>\\t", "</system_info>" );
}
complete_xml = NASLString( complete_xml, system_info_xml );
complete_xml = NASLString( complete_xml, collected_obj_xml );
complete_xml = NASLString( complete_xml, sys_data_xml );
complete_xml = NASLString( complete_xml, "</oval_system_characteristics>" );
