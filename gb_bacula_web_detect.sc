...
for dir in nasl_make_list_unique( "/", "/bacula-web", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( port: port, item: dir + "/" );
	if(ContainsString( res, "<title>Bacula-Web - Dashboard</title>" ) && ContainsString( res, ">Jobs<" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Bacula-Web", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
