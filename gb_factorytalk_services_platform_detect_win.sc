...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!app_name || !IsMatchRegexp( app_name, "FactoryTalk Services Platform" )){
			continue;
		}
		...
		register_and_report_cpe( app: "Rockwell Automation " + app_name, ver: version, concluded: concluded, base: "cpe:/a:rockwellautomation:factorytalk_services_platform:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
