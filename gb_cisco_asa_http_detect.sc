if(description){
	...
	script_tag( name: "last_modification", value: "2021-07-05 11:21:05 +0000 (Mon, 05 Jul 2021)" );
...
if(!buf || !ContainsString( buf, "CSCO_WebVPN" )){
	exit( 0 );
}
...
if(!buf || !IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" )){
	...
	req = http_get( item: url, port: port );
	buf = http_send_recv( port: port, data: req );
	if(!buf || !IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) || !IsMatchRegexp( buf, "Cisco Secure Desktop" )){
		exit( 0 );
	}
	conclurl = http_report_vuln_url( port: port, url: url, url_only: TRUE );
}
...
report = build_detection_report( app: "Cisco Adaptive Security Appliance (ASA) SSL VPN Portal", version: vers, install: install, cpe: app_cpe );
report += "\n\n";
report += build_detection_report( app: "Cisco Adaptive Security Appliance (ASA)", version: vers, install: install, cpe: os_cpe );
report += "\n\n";
if(version[0]){
