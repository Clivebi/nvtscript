if(description){
	script_oid( "1.3.6.1.4.1.25623.1.0.150185" );
	script_version( "2020-07-17T10:50:12+0000" );
	script_tag( name: "last_modification", value: "2020-07-17 10:50:12 +0000 (Fri, 17 Jul 2020)" );
	script_tag( name: "creation_date", value: "2020-04-02 13:06:00 +0000 (Thu, 02 Apr 2020)" );
	script_tag( name: "cvss_base", value: "0.0" );
	script_tag( name: "cvss_base_vector", value: "AV:L/AC:H/Au:S/C:N/I:N/A:N" );
	script_tag( name: "qod", value: "97" );
	script_name( "GaussDB: Read Access Permissions for GSDB_HOME (KB)" );
	script_category( ACT_GATHER_INFO );
	script_copyright( "Copyright (C) 2020 Greenbone Networks GmbH" );
	script_family( "Policy" );
	script_dependencies( "gather-package-list.sc", "compliance_tests.sc" );
	script_mandatory_keys( "Compliance/Launch" );
	script_exclude_keys( "ssh/no_linux_shell" );
	script_xref( name: "URL", value: "https://support.huawei.com/enterprise/en/doc/EDOC1100098622" );
	script_tag( name: "summary", value: "The home directory is a database installation directory
specified by the environment variable {GSDB_HOME}.

Access permissions to files and directories within {GSDB_HOME} need to be set carefully.

Note: This script only stores information for other Policy Controls. This script can be performed
only by a database installation user." );
	exit( 0 );
}
require("ssh_func.inc.sc");
require("policy_functions.inc.sc");
if(!get_kb_item( "login/SSH/success" ) || !sock = ssh_login_or_reuse_connection()){
	set_kb_item( name: "Policy/linux/gaussdb/${GSDB_HOME}/ssh/ERROR", value: TRUE );
	exit( 0 );
}
if( gsdb_home = get_kb_item( "Policy/gaussdb/gsdb_home" ) ){
	files = make_list( gsdb_home,
		 gsdb_home + "/lib",
		 gsdb_home + "/add-ons",
		 gsdb_home + "/bin",
		 gsdb_home + "/cacert.pem" );
	for file in files {
		kb_name = ereg_replace( string: file, pattern: gsdb_home, replace: "${GSDB_HOME}" );
		policy_linux_stat_file( socket: sock, file: file, kb_name: kb_name );
	}
}
else {
	files = make_list( "${GSDB_HOME}",
		 "${GSDB_HOME}/lib",
		 "${GSDB_HOME}/add-ons",
		 "${GSDB_HOME}/bin",
		 "${GSDB_HOME}/cacert.pem" );
	for file in files {
		policy_linux_stat_file( socket: sock, file: file );
	}
}
exit( 0 );

