...
for dir in nasl_make_list_unique( "/base", "/snort/base", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>Basic Analysis and Security Engine \\(BASE\\)", string: buf, icase: TRUE )){
		...
		set_kb_item( name: NASLString( "www/", port, "/BASE" ), value: tmp_version );
		...
		log_message( data: build_detection_report( app: "Basic Analysis and Security Engine (BASE)", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
