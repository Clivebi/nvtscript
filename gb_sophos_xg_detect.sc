...
if(!IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) || !ContainsString( buf, "<title>Sophos</title>" )){
	exit( 0 );
}
url1 = "/javascript/lang/English/common.js";
buf1 = http_get_cache( item: url1, port: port );
if(!IsMatchRegexp( buf1, "Sophos [^ ]*Firewall" )){
	exit( 0 );
}
...
os_register_and_report( os: "Sophos XG Firewall Firmware", cpe: cpe, desc: "Sophos XG Firewall Detection (HTTP)", runs_key: "unixoide" );
register_product( cpe: cpe, location: "/webconsole", port: port, service: "www" );
log_message( data: build_detection_report( app: "Sophos XG Firewall", version: version, install: "/webconsole", cpe: cpe, concluded: vers[0], concludedUrl: concUrl ), port: port );
exit( 0 );
