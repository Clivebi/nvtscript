...
if(ContainsString( buf, "SolarWinds Platform" ) || ContainsString( buf, "SolarWinds Orion" ) || ContainsString( buf, "Orion Platform" )){
	...
	if(!isnull( sam )){
		...
		set_kb_item( name: NASLString( "www/", port, "/orion_sam" ), value: vers );
		...
		log_message( data: build_detection_report( app: "SolarWinds Server and Application Monitor", version: vers, install: dir, cpe: cpe, concluded: sam[0] ), port: port );
	}
}
exit( 0 );
