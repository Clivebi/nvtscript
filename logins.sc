...
if(description){
	...
	script_add_preference( name: "NTLMSSP", type: "checkbox", value: "yes", id: 1 );
...
http_login = script_get_preference( name: "HTTP account :", id: 2 );
http_password = script_get_preference( name: "HTTP password (sent in clear) :", id: 3 );
if(http_login && http_password){
	...
	userpass = NASLString( http_login, ":", http_password );
	userpass64 = base64( str: userpass );
	authstr = "Authorization: Basic " + userpass64;
	set_kb_item( name: "http/auth", value: authstr );
}
nntp_login = script_get_preference( name: "NNTP account :", id: 4 );
nntp_password = script_get_preference( name: "NNTP password (sent in clear) :", id: 5 );
...
ftp_login = script_get_preference( name: "FTP account :", id: 6 );
if(!ftp_login){
	ftp_login = default_ftp_login;
}
ftp_password = script_get_preference( name: "FTP password (sent in clear) :", id: 7 );
if(!ftp_password){
	ftp_password = default_ftp_password;
}
ftp_w_dir = script_get_preference( name: "FTP writeable directory :", id: 8 );
if(!ftp_w_dir){
	ftp_w_dir = default_ftp_w_dir;
}
...
pop2_login = script_get_preference( name: "POP2 account :", id: 9 );
pop2_password = script_get_preference( name: "POP2 password (sent in clear) :", id: 10 );
...
pop3_login = script_get_preference( name: "POP3 account :", id: 11 );
pop3_password = script_get_preference( name: "POP3 password (sent in clear) :", id: 12 );
...
imap_login = script_get_preference( name: "IMAP account :", id: 13 );
imap_password = script_get_preference( name: "IMAP password (sent in clear) :", id: 14 );
...
smb_ctxt = script_get_preference( name: "Never send SMB credentials in clear text", id: 15 );
if(!smb_ctxt){
	smb_ctxt = "yes";
}
if(smb_ctxt == "yes"){
...
smb_ntv1 = script_get_preference( name: "Only use NTLMv2", id: 16 );
if(!smb_ntv1){
	smb_ntv1 = "no";
}
if(smb_ntv1 == "yes"){
...
smb_ntlmssp = script_get_preference( name: "NTLMSSP", id: 1 );
if(!smb_ntlmssp){
	smb_ntlmssp = "yes";
}
if(smb_ntlmssp == "yes"){
