...
for dir in nasl_make_list_unique( "/", "/ossec", "/ossec-wui", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	buf = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( buf, "<title>OSSEC Web Interface" ) || ContainsString( buf, "<!-- OSSEC" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "OSSEC Web UI", version: version, concluded: ver[0], install: install, cpe: cpe ), port: port );
	}
}
exit( 0 );
