if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of Pilz PNOZmulti Configurator." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "PNOZmulti Configurator" )){
			continue;
		}
		version = "unknown";
		concluded = appName;
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:pilz:pnozmulti_configurator:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
