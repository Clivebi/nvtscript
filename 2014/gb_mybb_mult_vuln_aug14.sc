...
url = dir + "/search.php";
payload = "action=do_search&keywords=%3Cfoo%3E+%3Ch1%3E+%3Cscript%3E+" + "alert+%28bar%29+%28%29+%3B+%2F%2F+%27+%22+%3E+%3C+prompt+%" + "5Cx41+%2542+constructor+onload&postthread=1&author=&matchu" + "sername=1&forums%5B%5D=all&findthreadst=1&numreplies=&post" + "date=0&pddir=1&sortby=lastpost&sortordr=desc&showresults=t" + "hreads&submit=Search";
req = http_post( item: url, port: port, data: payload );
res = http_send_recv( port: port, data: req );
if(res && IsMatchRegexp( res, "You have an error in your SQL syntax.*constructor onload" )){
