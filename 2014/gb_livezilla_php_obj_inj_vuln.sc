...
if(!lz_port = get_app_port( cpe: CPE )){
	exit( 0 );
}
if(!dir = get_app_location( cpe: CPE, port: lz_port )){
	exit( 0 );
}
...
post_data = NASLString( "p_request=extern&p_action=mail" );
post_data_len = strlen( post_data );
lz_path = dir + "/server.php";
referer = NASLString( "http://", host, dir, "/chat.php" );
...
if(ContainsString( lz_res, "Cannot use object of type __PHP_Incomplete_Class as array" ) && ContainsString( lz_res, "_lib/functions.global.inc.php" )){
