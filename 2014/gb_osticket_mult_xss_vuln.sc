...
req = http_get( item: NASLString( dir, "/upload/open.php" ), port: port );
res = http_keepalive_send_recv( port: port, data: req );
if(res && ContainsString( res, "powered by osTicket<" )){
	...
	res = http_keepalive_send_recv( port: port, data: req );
	if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "></script><script>alert(document.cookie)</script>" ) && ContainsString( res, "osTicket<" )){
