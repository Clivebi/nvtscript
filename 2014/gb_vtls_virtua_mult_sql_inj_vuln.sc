...
for dir in nasl_make_list_unique( "/", "/virtua", "/vlts", "/mgmt", "/libmgmt", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	...
	if(ContainsString( rcvRes, "Virtua<" ) && ContainsString( rcvRes, ">InfoStation - Log In<" )){
		postData = "mod=login&func=process&database=1&lang_code=en&report_group" + "=Adm&filter=test&username=%27SQL-Injection-Test&password=%27";
		...
		if(rcvRes && IsMatchRegexp( rcvRes, "SQL error.*SQL command not properly ended.*SQL-Injection-Test" )){
