...
url = dir + "/wp-content/plugins/keyword-strategy-internal-links/inpage.tpl." + "php?sort='><script>alert(document.cookie)</script>&search='><sc" + "ript>alert(document.cookie)</script>&dir='><script>alert(docume" + "nt.cookie)</script>";
if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "<script>alert\\(document.cookie\\)</script>", extra_check: ">Keyword Strategy<" )){
