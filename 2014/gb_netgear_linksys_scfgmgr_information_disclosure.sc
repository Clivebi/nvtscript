if(description){
	...
	script_version( "2020-05-05T10:31:34+0000" );
	script_name( "Netgear/Linksys Routers Backdoor" );
	script_xref( name: "URL", value: "https://github.com/elvanderb/TCP-32764" );
	script_tag( name: "last_modification", value: "2020-05-05 10:31:34 +0000 (Tue, 05 May 2020)" );
...
reqs = make_list( raw_string( "MMcS",
	 1,
	 0x00,
	 0x00,
	 0x00,
...
for req in reqs {
	...
	close( soc );
	if(!ContainsString( recv, "ScMM" ) && !ContainsString( recv, "MMcS" )){
		exit( 0 );
	}
	if(strlen( recv ) > 12){
		data = substr( recv, 12 );
