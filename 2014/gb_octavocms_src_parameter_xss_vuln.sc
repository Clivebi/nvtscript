...
for dir in nasl_make_list_unique( "/", "/octavocms", "/cms", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	sndReq = http_get( item: NASLString( dir, "/admin/login.php" ), port: http_port );
	rcvRes = http_keepalive_send_recv( port: http_port, data: sndReq );
	if(ContainsString( rcvRes, "Octavo Content Management<" )){
		url = dir + "/admin/viewer.php?src=\"><script>alert(document.cook" + "ie)</script>";
		if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "<script>alert\\(document.cookie\\)</script>", extra_check: "Octavo Content Management<" )){
