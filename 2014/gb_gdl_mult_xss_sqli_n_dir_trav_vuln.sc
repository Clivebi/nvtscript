...
for dir in nasl_make_list_unique( "/", "/gdl", "/diglib", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	gdlRes = http_get_cache( item: NASLString( dir, "/" ), port: http_port );
	if(ContainsString( gdlRes, "ITB. All rights reserved" ) || ContainsString( gdlRes, "Powered By GDL" )){
		url = dir + "/gdl.php?mod=search&action=folks&keyword=''%22%3E%3Cscript" + "%3Ealert(document.cookie)%3C/script%3E&type=all&submit=OK";
		if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "<script>alert\\(document\\.cookie\\)</script>", extra_check: "GDL" )){
