if(description){
	...
	script_dependencies( "gb_snmp_sysdescr_detect.sc", "telnetserver_detect_type_nd_version.sc", "gb_default_credentials_options.sc" );
...
telnet_port = 23;
if(!get_port_state( telnet_port )){
	exit( 0 );
}
if(IsMatchRegexp( sysdesc, "(ZXV|N12E|SpeedSurf|RTA|DG-)" )){
	device = TRUE;
}
if(!device){
	banner = telnet_get_banner( port: telnet_port );
	if(banner && ( ContainsString( banner, "User Access Verification" ) && ContainsString( banner, "Username:" ) ) || IsMatchRegexp( banner, "(ZXV|N12E|SpeedSurf|RTA|DG-)" )){
		device = TRUE;
	}
}
if(!device){
...
for(i = 0;i < 3;i++){
	...
	sendata = raw_string( 0x30, len, 0x02, 0x01, i, 0x04, sz ) + community + raw_string( 0xa0, 0x1f, 0x02, 0x04, 0x2d, 0xc7, 0xb1, 0x92, 0x02, 0x01, 0x00, 0x02, 0x01, 0x00, 0x30, 0x11, 0x30, 0x0f, 0x06, 0x0b, 0x2b, 0x06, 0x01, 0x02, 0x01, 0x02, 0x02, 0x01, 0x06, 0xce, 0x10, 0x05, 00 );
	...
	recv = recv( socket: soc, length: 2048 );
	if(!ContainsString( recv, "ADSL#" )){
		exit( 0 );
	}
	send( socket: soc, data: "login show\r\n" );
	recv = recv( socket: soc, length: 2048 );
	close( soc );
	if(ContainsString( recv, "Username" ) && ContainsString( recv, "Password" ) && ContainsString( recv, "Priority" )){
