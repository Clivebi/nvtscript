...
for dir in nasl_make_list_unique( "/", "/phpSound", "/sound", http_cgi_dirs( port: http_port ) ) {
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: NASLString( dir, "/index.php" ), port: http_port );
	if(ContainsString( rcvRes, "phpSound<" ) && ContainsString( rcvRes, "Explore new music" )){
		url = dir + "/index.php?a=explore&filter=%3C/title%3E%3Cscript%3Ealert(document.cookie);%3C/script%3E";
		if(http_vuln_check( port: http_port, url: url, check_header: TRUE, pattern: "<script>alert\\(document\\.cookie\\);</script>", extra_check: ">Search Results<" )){
