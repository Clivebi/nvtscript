...
if(!ContainsString( banner, "Server: httpd" ) || !IsMatchRegexp( banner, "HTTP/1\\.. 401" )){
	exit( 0 );
}
if(!ContainsString( tolower( banner ), "ip camera" ) && !ContainsString( banner, "IP SPEED DOME" ) && !ContainsString( tolower( banner ), "IP Video Server" )){
	exit( 0 );
}
credentials = make_list( "3sadmin:27988303",
	 "root:root" );
