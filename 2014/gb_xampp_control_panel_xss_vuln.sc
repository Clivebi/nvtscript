...
if(res && IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, ">CD Collection" )){
	if(http_vuln_check( port: port, url: NASLString( dir, "/cds-fpdf.php" ), check_header: TRUE, pattern: "<script>alert\\(document.cookie\\)</script>" )){
		delId = eregmatch( string: res, pattern: "alert.document.cookie.&lt;/script&gt;</b>" + "</td><td class=tabval>title&nbsp;</td><td class=tabval>1&nbsp;<" + "/td><td class=tabval><a onclick=\"return confirm..Sure...;\" href" + "=cds.php.action=del&id=([0-9]*)" );
		req = http_get( item: NASLString( dir, "/cds.php?action=del&id=", delId[1] ), port: port );
		res = http_keepalive_send_recv( port: port, data: req, bodyonly: FALSE );
		if( res && IsMatchRegexp( res, "^HTTP/1.[01] 200" ) && !ContainsString( res, "alert(document.cookie)" ) ){
