if(description){
	...
	script_version( "2021-06-30T05:18:54+0000" );
	script_tag( name: "last_modification", value: "2021-06-30 05:18:54 +0000 (Wed, 30 Jun 2021)" );
	...
	script_dependencies( "webmirror.sc", "DDI_Directory_Scanner.sc", "global_settings.sc", "gb_twonky_server_detect.sc", "gb_owncloud_detect.sc", "gb_adobe_aem_http_detect.sc", "gb_libreoffice_online_detect.sc", "gb_apache_activemq_http_detect.sc", "gb_orientdb_server_detect.sc" );
...
func prepend_max_items_text( curReport, currentItems, maxItems ){
	var curReport, currentItems, maxItems, report;
...
if(!isnull( coffeecupList )){
	...
	for coffeecup in coffeecupList {
		currentItems++;
		if(currentItems >= maxItems){
			continue;
		}
...
if(!isnull( frontpageList )){
	...
	for frontpage in frontpageList {
		currentItems++;
		if(currentItems >= maxItems){
			continue;
		}
...
if(!isnull( chOptOutList ) || !isnull( chOptInList ) || !isnull( chNoOptOutList ) || !isnull( chObfuscatedList )){
	currentItems = 0;
	tmpreport = "The Coinhive JavaScript Miner was found embedded into the following pages:\n\n";
	if(chOptOutList){
		chOptOutList = sort( chOptOutList );
	}
	if(chOptInList){
		chOptInList = sort( chOptInList );
	}
	if(chNoOptOutList){
		chNoOptOutList = sort( chNoOptOutList );
	}
	if(chObfuscatedList){
		chObfuscatedList = sort( chObfuscatedList );
	}
	for chOptOut in chOptOutList {
		currentItems++;
		if(currentItems >= maxItems){
			continue;
		}
	...
	for chNoOptOut in chNoOptOutList {
		currentItems++;
		if(currentItems >= maxItems){
			continue;
		}
	...
	for chObfuscated in chObfuscatedList {
		currentItems++;
		if(currentItems >= maxItems){
			continue;
		}
