if(description){
	...
	script_tag( name: "summary", value: "SMB login-based detection of FUJITSU Paperstream IP (TWAIN) Software." );
...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!appName || !IsMatchRegexp( appName, "PaperStream IP" )){
			continue;
		}
		...
		register_and_report_cpe( app: appName, ver: version, concluded: concluded, base: "cpe:/a:fujitsu:paperstream_ip_%28twain%29:", expr: "^([0-9.]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
