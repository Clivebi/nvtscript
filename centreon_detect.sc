...
for dir in nasl_make_list_unique( "/centreon", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>Centreon - IT & Network Monitoring</title>", string: buf, icase: TRUE ) && ContainsString( buf, "LoginInvitVersion" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "Centreon", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
