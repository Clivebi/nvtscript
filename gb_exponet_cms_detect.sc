...
for dir in nasl_make_list_unique( "/", "/exponent", "/cms", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	for page in make_list( "/index.php",
		 "/login.php",
		...
		if(egrep( pattern: "meta name=\"Generator\" content=\"Exponent", string: buf, icase: TRUE ) || ( ContainsString( buf, ">Exponent CMS" ) && ContainsString( buf, "EXPONENT.LANG" ) )){
			vers = "unknown";
			...
			log_message( data: build_detection_report( app: "Exponent Content Management System", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
			exit( 0 );
		}
	}
}
