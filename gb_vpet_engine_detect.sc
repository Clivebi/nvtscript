...
if(!http_can_host_php( port: vpet_Port )){
	exit( 0 );
}
for dir in nasl_make_list_unique( "/", "/vpet", "/vPetEngine", http_cgi_dirs( port: vpet_Port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: vpet_Port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && ContainsString( rcvRes, "<TITLE>vPetOnline - Home</TITLE>" ) && ContainsString( rcvRes, ">Login<" )){
		...
		log_message( data: build_detection_report( app: "vPet Engine", version: version, install: install, cpe: cpe, concluded: version ), port: vpet_Port );
	}
}
exit( 0 );
