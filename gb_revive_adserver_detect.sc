...
for dir in nasl_make_list_unique( "/", "/adserver", "/radserver", "/revive-adserver", "/ads", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(!ContainsString( res, "Home\">Authentication" ) || !ContainsString( res, "content=\"Revive Adserver" )){
	...
	log_message( data: build_detection_report( app: "Revive Adserver", version: version, install: install, cpe: cpe, concludedUrl: conclUrl, concluded: ver[0] ), port: port );
}
exit( 0 );
