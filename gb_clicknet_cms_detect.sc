if(description){
	...
	script_tag( name: "summary", value: "This script detects the installed version of Clicknet CMS." );
...
for dir in nasl_make_list_unique( "/clicknet", "/cms", "/clicknetcms", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( rcvRes, "Powered by" ) && ContainsString( rcvRes, "Clicknet CMS" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "Clicknet CMS", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
