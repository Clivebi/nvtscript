...
for dir in nasl_make_list_unique( "/board", "/mercuryboard", "/forums", "/forum", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "Powered by.*MercuryBoard.*\\[v[0-9.]+\\]", string: buf, icase: TRUE )){
		...
		log_message( data: build_detection_report( app: "MercuryBoard", version: version, install: install, cpe: cpe, concluded: vers[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
