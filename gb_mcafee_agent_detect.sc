...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(ContainsString( agentName, "McAfee Agent" )){
			...
			if(agentVer){
				...
				log_message( data: build_detection_report( app: "McAfee Agent", version: agentVer, install: agentPath, cpe: cpe, concluded: agentVer ) );
				exit( 0 );
			}
		}
	}
