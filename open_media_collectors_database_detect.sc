...
for dir in nasl_make_list_unique( "/opendb", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	...
	if(egrep( pattern: "<title>Open Media Collectors Database - Login</title>", string: buf, icase: TRUE ) && ContainsString( buf, "Powered by OpenDb" )){
		vers = "unknown";
		...
		log_message( data: build_detection_report( app: "Open Media Collectors Database", version: vers, install: install, cpe: cpe, concluded: version[0] ), port: port );
		exit( 0 );
	}
}
exit( 0 );
