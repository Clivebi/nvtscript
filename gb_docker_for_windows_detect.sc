...
for key in key_list {
	for item in registry_enum_keys( key: key ) {
		...
		if(!app_name || !IsMatchRegexp( app_name, "Docker for Windows" )){
			continue;
		}
		...
		register_and_report_cpe( app: app_name, ver: version, concluded: concluded, base: "cpe:/a:docker:docker:", expr: "^([0-9.a-z-]+)", insloc: location, regService: "smb-login", regPort: 0 );
		exit( 0 );
	}
}
exit( 0 );
