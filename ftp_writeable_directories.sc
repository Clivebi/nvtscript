if(description){
	...
	script_version( "2021-02-18T10:48:36+0000" );
	script_cve_id( "CVE-1999-0527" );
	script_tag( name: "last_modification", value: "2021-02-18 10:48:36 +0000 (Thu, 18 Feb 2021)" );
...
func crawl_dir( socket, directory, level ){
	var port, soc2, r, dirs, array, dir, sep, str, alreadyadded;
	if(level > 20){
		return 0;
	}
...
if(!ftp_authenticate( socket: soc, user: login, pass: pwd )){
	exit( 0 );
}
crawl_dir( socket: soc, directory: "/", level: 0 );
ftp_close( socket: soc );
if(isnull( WriteableDirs )){
	exit( 0 );
}
for dir in keys( WriteableDirs ) {
	report += " - " + dir + "\n";
}
if(report){
