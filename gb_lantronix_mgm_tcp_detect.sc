...
for(i = 4;i < 8;i++){
	...
	ip += NASLString( ord( getconf_recv[i] ) );
}
info["IP"] = ip;
for(i = 16;i < 20;i++){
	if(i != 16){
	...
	gateway += NASLString( ord( getconf_recv[i] ) );
}
...
set_kb_item( name: "lantronix_device/lantronix_remote_conf_tcp/" + port + "/type", value: "unknown" );
...
service_register( port: port, proto: "lantronix_remote_conf" );
