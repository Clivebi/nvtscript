...
for item in registry_enum_keys( key: key ) {
	...
	if(ContainsString( hpName, "HP Support Assistant" )){
		...
		if(hpVer){
			...
			log_message( data: build_detection_report( app: "HP Support Assistant", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
	if(ContainsString( hpName, "HP Support Solutions Framework" )){
		hpVer = registry_get_sz( key: key + item, item: "DisplayVersion" );
		if(hpVer){
			hpPath = registry_get_sz( key: key + item, item: "InstallLocation" );
			if(!hpPath){
			...
			register_product( cpe: cpe, location: hpPath );
			log_message( data: build_detection_report( app: "HP Support Assistant Framework", version: hpVer, install: hpPath, cpe: cpe, concluded: hpVer ) );
		}
	}
}
