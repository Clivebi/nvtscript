...
if(ContainsString( res, "<title>Secure Messaging Gateway Messaging Security</title>" ) && ContainsString( res, "case 1600000004" )){
	version = "unknown";
	set_kb_item( name: "microfocus_smg/installed", value: TRUE );
	cpe = "cpe:/a:microfocus:secure_messaging_gateway";
	register_product( cpe: cpe, location: "/", port: port, service: "www" );
	log_message( data: build_detection_report( app: "Micro Focus Secure Messaging Gateway", version: version, install: "/", cpe: cpe ), port: port );
	exit( 0 );
}
exit( 0 );
