...
for dir in nasl_make_list_unique( "/", "/oscmax/catalog", "/oscmax", "/ecommerce", http_cgi_dirs( port: port ) ) {
	install = dir;
	if(dir == "/"){
		dir = "";
	}
	rcvRes = http_get_cache( item: dir + "/admin/login.php", port: port );
	if(IsMatchRegexp( rcvRes, "^HTTP/1\\.[01] 200" ) && IsMatchRegexp( rcvRes, "Powered by.*osCmax" )){
		version = "unknown";
		...
		log_message( data: build_detection_report( app: "osCMax e-commerce/shopping-cart", version: version, install: install, cpe: cpe, concluded: ver[0] ), port: port );
	}
}
exit( 0 );
