...
for dir in nasl_make_list_unique( "/cms", "/cncms", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( res, "Powered By" ) && ContainsString( res, "The Social Networking CMS" ) && ContainsString( res, ">ShopBlueSoft.com<" )){
		exploit = NASLString( dir, "/user_profile.php?view=photo&photo_id='" );
		req = http_get( item: exploit, port: port );
		res = http_keepalive_send_recv( port: port, data: req );
		if(ContainsString( res, "error in your SQL syntax;" )){
