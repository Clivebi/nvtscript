...
dllFile = registry_get_sz( key: "SOFTWARE\\Microsoft\\COM3\\Setup", item: "Install Path" );
if(!dllFile){
	exit( 0 );
}
share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: dllFile );
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: dllFile + "\\quartz.dll" );
dllVer = GetVer( file: file, share: share );
if(!dllVer){
	exit( 0 );
}
if(hotfix_check_sp( win2k: 5 ) > 0){
	if( IsMatchRegexp( directXver, "^4\\.07" ) ){
		...
		if( IsMatchRegexp( directXver, "^4\\.08" ) ){
...
dllFile = registry_get_sz( key: "SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion", item: "PathName" );
if(!dllFile){
	exit( 0 );
}
share = ereg_replace( pattern: "([A-Z]):.*", replace: "\\1$", string: dllFile );
file = ereg_replace( pattern: "[A-Z]:(.*)", replace: "\\1", string: dllFile + "\\system32\\quartz.dll" );
dllVer = GetVer( file: file, share: share );
if(!dllVer){
	exit( 0 );
}
