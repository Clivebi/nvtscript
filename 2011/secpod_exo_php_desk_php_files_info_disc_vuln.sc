...
for dir in nasl_make_list_unique( "/ExoPHPDesk", "/", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( res, "<title>EXO PHPDesk<" ) || ContainsString( res, ">Powered by ExoPHPDesk" )){
		url = dir + "/upgrades/upgrade9.php";
		if(http_vuln_check( port: port, url: url, pattern: "<b>Fatal error</b>:  " + "Call to a member function query().*upgrades/upgrade9.php" )){
