...
for dir in nasl_make_list_unique( "/bst", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	buf = http_get_cache( item: dir + "/index.php", port: port );
	if(!buf || !IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) || ( !ContainsString( buf, "by BST" ) && !ContainsString( buf, "Powered by BST" ) && !ContainsString( buf, "www.bst.pl" ) )){
		continue;
	}
	url = NASLString( dir, "/nowosci.php?a=1&str=%3Cscript%3Ealert(/", vt_strings["lowercase"], "/)%3C/script%3E" );
