...
for dir in nasl_make_list_unique( "/arsc", "/chat", "/", http_cgi_dirs( port: port ) ) {
	...
	if(ContainsString( res, "Powered by ARSC" )){
		attack = "/base/admin/login.php?arsc_message=<script>alert" + "('" + vt_strings["lowercase"] + "')</script>";
		req = http_get( item: dir + attack, port: port );
		res = http_keepalive_send_recv( port: port, data: req );
		if(IsMatchRegexp( res, "^HTTP/1\\.[01] 200" ) && ContainsString( res, "<script>alert('" + vt_strings["lowercase"] + "')</script>" )){
