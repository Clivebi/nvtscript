...
for dir in nasl_make_list_unique( "/eyeos", "/", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: dir + "/index.php", port: port );
	if(ContainsString( res, "<title>Welcome to eyeos" )){
		url = dir + "/eyeos/apps/rmail/webmail/program/lib/Net/SMTP.php";
		if(http_vuln_check( port: port, url: url, pattern: "<b>Fatal error</b>:  " + "require_once() \\[<a href='function.require'>function." + "require</a>\\]: Failed opening required 'PEAR.php'.*" + "apps/rmail/webmail/program/lib/Net/SMTP.php" )){
