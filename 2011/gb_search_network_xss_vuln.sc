...
for dir in nasl_make_list_unique( "/sn", "/search_network", "/", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	res = http_get_cache( item: NASLString( dir, "/index.php" ), port: port );
	if(ContainsString( res, "www.searchnetworkhq.com" )){
		url = NASLString( dir, "/index.php?searchType=Videos&query=\"<script>alert(document.cookie)<%2Fscript>" );
