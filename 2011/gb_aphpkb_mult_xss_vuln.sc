...
data = NASLString( "username=\"><script>alert(\"VT-XSS-Test\")</script>", "&password=&submit=Login" );
url = dir + "/login.php";
...
if(ereg( pattern: "^HTTP/[0-9]\\.[0-9] 200 .*", string: res ) && ContainsString( res, "><script>alert(\"VT-XSS-Test\")</script>" )){
