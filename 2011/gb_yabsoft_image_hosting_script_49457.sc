...
for dir in nasl_make_list_unique( "/images", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	buf = http_get_cache( item: dir + "/index.php", port: port );
	if(!buf || !IsMatchRegexp( buf, "^HTTP/1\\.[01] 200" ) || ( !ContainsString( buf, ">AIH " ) && !ContainsString( buf, "javascript:toggle('uploadtip')" ) )){
		continue;
	}
	url = NASLString( dir, "/report.php?img_id=%22%3E%3Cscript%3Ealert(/", vt_strings["lowercase"], "/)%3C/script%3E" );
