...
if(jwspaths){
	for executableFile in jwspaths {
...
if(javapaths){
	for executableFile in javapaths {
		executableFile = chomp( executableFile );
		if(!executableFile){
			continue;
		}
		...
		if( ContainsString( javaVer[1], "gcj" ) ){
			...
			if( ContainsString( javaVer[1], "IBM" ) ){
				...
				if(IsMatchRegexp( javaVer[1], "([0-9]\\.[0-9._]+)-([b0-9]+)" ) || IsMatchRegexp( javaVer[1], "([0-9.]+\\+)" )){
					...
					if( version_is_less( version: jvVer, test_version: "1.4.2.38" ) || version_in_range( version: jvVer, test_version: "1.5", test_version2: "1.5.0.33" ) || version_in_range( version: jvVer, test_version: "1.6", test_version2: "1.6.0.18" ) ){
						java_name = "Sun Java";
						if( ( ContainsString( executableFile, "jdk" ) && !ContainsString( executableFile, "jre" ) ) || ( ContainsString( executableFile, "jdk" ) && ContainsString( executableFile, "jre" ) ) ){
