...
query = "Select Manufacturer, Version from CIM_DataFile Where FileName =" + raw_string( 0x22 ) + "fhtml2pdf" + raw_string( 0x22 ) + " AND Extension =" + raw_string( 0x22 ) + "exe" + raw_string( 0x22 );
appConfirm = wmi_query( wmi_handle: handle, query: query );
wmi_close( wmi_handle: handle );
if(ContainsString( appConfirm, "Foxit Software Inc" )){
	...
	if(version[1]){
		...
		log_message( data: build_detection_report( app: "Foxit PDF Toolkit", version: version[1], install: path, cpe: cpe, concluded: version[1] ) );
	}
}
exit( 0 );
